"""
 DWARF D=768 L=20 Triadic J=96 — HISA/DSQG hybrid v2 L3, Bible-audited variant.

Architecture: D=768, H=16 (hd=48), L=20, FFN=1536, tied lm_head
...
Layout:
  ...
  L16: DSQGBlock(GROUP_A)
  L17: DSQGBlock(GROUP_B)
  L18: DSQGBlock(GROUP_C)
  L19: DSQGBlock(GROUP_A)
"""

import contextlib, hashlib, json, math, os, random, subprocess, sys, time
...
NUM_CHUNKS       = 32
TOP_K_CHUNKS     = 4
HISA_TOP_M_TOKENS = int(os.environ.get('DWARF_HISA_TOP_M', '64'))  # 64 preserves exact self-chunk token coverage at N=2048,C=32

SCALE_EMBED_INIT_VAL = 0.15
# Conservative audited default; override with DWARF_SCALE_EMBED_LR_MULT.
SCALE_EMBED_LR_MULT  = float(os.environ.get('DWARF_SCALE_EMBED_LR_MULT', '18.37'))
EMA_INIT  = 0.020833
EMA_FLOOR = 0.00001
LR        = float(os.environ.get('DWARF_LR', '3e-4'))
WEIGHT_DECAY = float(os.environ.get('DWARF_WEIGHT_DECAY', '0.1'))
PHASE_LR_MULT = float(os.environ.get('DWARF_PHASE_LR_MULT', '50.0'))
LR_WARMUP_STEPS = int(os.environ.get('DWARF_LR_WARMUP_STEPS', '1000'))
...
BATCH_SIZE     = int(os.environ.get('DWARF_BS', '24'))
...
CKPT_BASE_NAME    = 'd768_l20_hisa_h16_v2_l3_bible'

CHECKPOINT_STRATEGY = os.getenv('DWARF_CKPT', 'all').lower()
DATASET_PATH = os.environ.get('DWARF_DATASET', 'datasets/mixed_encoded_2048_v6_modular_fw1000000.pt')

# =============================================================================
# LAYER LAYOUT: L=20, DSR/HISA@L3
# Pre-DSR: 1 triad (L0-2), Post-DSR: 5 full triads + A (L4-19)
# =============================================================================
...
def train():
    ...
    print('=' * 70)
    print(f'  DWARF D768-L20 Triadic J=96 — DSR/HISA@L{DSR_LAYER} + R_PLANES=4, TIED LM_HEAD')
    print(f'  DSR@L{DSR_LAYER}: HierarchicalSparseAttentionV15HISA(C={NUM_CHUNKS}, top_k={TOP_K_CHUNKS}, HISA_m={HISA_TOP_M_TOKENS})')
    print('  PURE LM loss only. No distillation. No teacher. Random init. Bible-audited defaults.')
    print('=' * 70)
    ...
    print(f'  Per-layer bandwidth ratio: {(len(GROUP_A)*(EMBEDDING_DIM//NUM_HEADS))/EMBEDDING_DIM:.2f}x  (safe <= 3.0x)')
    ...
    scale_embed_params = optimizer_groups[2]['params']
    phase_params = optimizer_groups[3]['params']
    _opt_cls = torch.optim.AdamW
    if _BNB_AVAILABLE:
        _opt_cls = getattr(bnb.optim, 'PagedAdamW8bit', bnb.optim.AdamW8bit)
    optimizer = _opt_cls(optimizer_groups, betas=(0.9, 0.95), eps=1e-8)
    print(f'  Optimizer: {_opt_cls.__name__}')
    for group in optimizer.param_groups:
        n_group = sum(p.numel() for p in group['params'])
        print(f"  opt_group[{group.get('name', '?')}]: params={n_group:,} lr={group['lr']:.2e} wd={group.get('weight_decay', 0.0):.2g}")
    ...
    def _lr_lambda(step, group_idx):
        ...
        if SCALE_EMBED_CONSTANT_LR and group_name == 'scale_embed':
            return 1.0
    ...
    print('\n' + '=' * 70)
    print(f'  DSR + R_PLANES={R_PLANES} Bible Variant Summary (D{EMBEDDING_DIM}-L{NUM_LAYERS}, tied lm_head)')
    print('=' * 70)
