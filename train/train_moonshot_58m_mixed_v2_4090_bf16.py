...
Config:
  ...
  - SCALE_EMBED_INIT = 0.15, LR_MULT = 18.0 (v2: harder percolation push on mixed dataset)
...
TODO (audit Mar 20 2026):
  - dropout=0.0 (confirmed: relay chain severance during training degrades percolation; modern LLMs use 0.0)
  - grad_clip=1.0 never varied — flag for future experiment
  - No LR warmup — stable so far but untested at >100M scale

Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_moonshot_58m_mixed_v2_4090_bf16.py \\
    > logs/run_moonshot_58m_mixed_v2.log 2>&1 &
"""
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":           FFN_DIM,
            "seq_len":           MAX_SEQ_LEN,
            "source_script":     "train/train_moonshot_58m_mixed_v2_4090_bf16.py",
    ...
    out_path = os.path.join(checkpoint_dir, f"moonshot_58m_mixed_v2_ep{epoch}_full_attn.pt")
...
def train():
    ...
    # ── Cold start — no warm-start checkpoint ────────────────────────────────
    best_ckpt_name = 'moonshot_58m_mixed_v2_best.pt'
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        resume_path  = os.path.join(CHECKPOINT_DIR, f'moonshot_58m_mixed_v2_ep{epoch}_resume.pt')
