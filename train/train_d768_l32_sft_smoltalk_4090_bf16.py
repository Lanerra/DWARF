"""
🔬 DWARF D=768 L=32 — SFT on retrieval-aligned dataset (SQuAD + TriviaQA + Dolly)
...
Purpose: Test whether relay mechanism survives SFT.
  ...
  - Dataset: smoltalk_sft_encoded.pt (50K chat examples — ablation: scale_embed frozen)
...
Key hypothesis: SQuAD/TriviaQA passages keep FA retrieval gradient alive.
  Ablation: frozen scale_embed on smoltalk to test whether dataset composition or freeze caused Mar21 collapse.
...
Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_d768_l32_sft_4090_bf16.py \\
    2>&1 | tee logs/run_d768_l32_sft_smoltalk.log
"""
...
CE_CHUNK      = 512
MAX_SEQ_LEN   = 512   # SFT sequences are QA pairs; cap saves ~4x logit VRAM

RESUME_CHECKPOINT = 'autoresearch/checkpoints/d768_l32_fa8_best.pt'
SFT_DATASET_PATH  = 'logs/smoltalk_sft_encoded.pt'
...
def main():
    ...
    for epoch in range(1, SFT_EPOCHS + 1):
        ...
        torch.save({
            ...
            'git_hash': git_hash,
        }, os.path.join(CHECKPOINT_DIR, f'd768_l32_sft_ep{epoch}_resume.pt'))
        ...
        if val_ppl <= best_val_ppl:
            torch.save({
                ...
                'epoch': epoch,
                'git_hash': git_hash,
            }, os.path.join(CHECKPOINT_DIR, 'd768_l32_sft_best.pt'))
