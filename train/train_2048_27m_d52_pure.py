"""
d52_pure_27m — Pure DSQG scaled to 27M parameters.

Architecture (from d51 14M, scaled up):
  D=384, H=8, HD=48, L=8, FFN=1536 (4×D), interference=3
  → 27,288,392 parameters (verified by architecture_search.rs)
  → Interference layers at positions 2 and 5 (n_if=2, same as 14M)
  → Two additional standard TransformerBlocks at positions 6 and 7

Scaling rationale:
  - HD=48 vs 14M's HD=32: +50% head dimension → more scale_embed capacity per offset
  - L=8 vs 6: deeper network, same interference density (25% IF vs 33%)
  - FFN=1536=4×D: standard ratio maintained
  - Same J=44 geometry: [0..40, 128, 384, 1536] (d51 winning config)
  - Same uniform linspace pos_bias init (no θ asymmetry)
  - Same kernel: dsqg_attention_v3 from dsqg_d50_pure

Purpose:
  Intermediate datapoint between 14M (0% passkey) and 35M pure DSQG (33% passkey).
  Tests whether passkey retrieval emerges at 27M scale.
  Expected epoch time: ~900–1100s (~15–18 min/epoch on RTX 4090).
  Expected VRAM: ~18–20 GB.

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_2048_27m_d52_pure.py \\
    2>&1 | tee logs/27m_d52_pure_run.log
"""
...
EXPERIMENT_NAME = 'd52_pure_27m'
SAVE_DIR        = 'checkpoints/d52_pure_27m'
RESULT_FILE     = 'benchmarks/logs/d52_pure_27m_results.json'
