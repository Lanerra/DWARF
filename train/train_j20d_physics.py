"""
🔬 DWARF Autoresearch — J20D Full-Dataset + Physics

Builds on the J16D physics result. Extends to V7 kernel with J=20 offsets
(finer local coverage: δ=2,6,8,16 added → J_SMALL=10, J_LARGE=10).
...
Key changes from J16D:
  - V7 kernel (J=20) vs V6 kernel (J=16)
  - torch.compile applied to FullAttnBlock for speed

Offsets:
  ALL_OFFSETS = [1,2,3,4,6,8,13,15,16,21,23,28,48,64,96,192,384,512,768,1024]
  J_SMALL=10 (indices 0-9, δ≤21, no MOVT)
  J_LARGE=10 (indices 10-19, δ≥23, MOVT applied)

Run (from /tmp/dwarf-j17d):
  CUDA_VISIBLE_DEVICES=1 .venv/bin/python3 -u autoresearch/train_j20d_physics.py \
    > autoresearch/run_j20d_physics.log 2>&1 &
"""
...
OFFSETS = [1, 2, 3, 4, 6, 8, 13, 15, 16, 21, 23, 28, 48, 64, 96, 192, 384, 512, 768, 1024]
...
from dsqg_attention_v7 import DSQGAttentionV7 as DSQGAttentionV6, npci_rotate
...
class DSQGBlockV6Physics(nn.Module):
    """V7 DSQG attention + condV interference (EMA + KdV + AGC)."""
...
def train():
    ...
    print('=' * 70)
    print('  🔬 DWARF Autoresearch — J20D Full-Dataset + Physics')
    print('  V7 kernel (J=20) + condV: Kalman-EMA (α₀=0.003) + KdV + AGC')
    print('=' * 70)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(CHECKPOINT_DIR,
                                    f'{git_hash}_j20d_physics_best.pt'))
            marker = ' *'
    ...
    print(f'description:    J20D physics — {n_params/1e6:.1f}M, V7+condV, '
