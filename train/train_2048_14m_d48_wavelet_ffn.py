"""
Wavelet FFN — DWT-Domain Transformation + All-DSQG (14M scale, 3-Epoch Probe)

Replaces the standard position-wise FFN with a wavelet-domain transformation:
  DWT(x) → selective GELU at alternating levels → IDWT → learned scale + bias

Key properties:
  - D4 Daubechies wavelet (4-tap)
  ...
  - 512 learned params per block (gamma + beta) vs 525,568 standard FFN params
  - NO coupling to attention outputs (unlike FiLM)
...
Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_2048_14m_d48_wavelet_ffn.py \
    2>&1 | tee benchmarks/logs/d48_wavelet_ffn_14m.log
"""
...
EXPERIMENT_NAME = 'd48_wavelet_ffn_14m'
...
SAVE_DIR    = 'checkpoints/d48_wavelet_ffn_14m'
RESULT_FILE = 'benchmarks/logs/d48_wavelet_ffn_14m_results.json'
...
D4_LOWPASS  = torch.tensor([0.48296291314469025, 0.83651630373780772,
                             0.22414386804185735, -0.12940952255092145])
D4_HIGHPASS = torch.tensor([0.12940952255092145,  0.22414386804185735,
...
class WaveletFFN(nn.Module):
    """
    DWT along the embedding dimension (D=256), selective GELU at alternating levels.
    512 learned parameters total (scale + bias per coefficient).

    Uses a precomputed orthogonal DWT matrix W so that IDWT = W^T exactly.
    """
    def __init__(self, embedding_dim: int, levels_with_gelu: list = None):
        super().__init__()
        self.embedding_dim = embedding_dim
        self.number_of_levels = int(math.log2(embedding_dim)) - 1
        self.levels_gelu = set(levels_with_gelu if levels_with_gelu is not None else [0, 2, 4, 6])

        self.gamma = nn.Parameter(torch.ones(embedding_dim))
        self.beta  = nn.Parameter(torch.zeros(embedding_dim))

        W = self._build_dwt_matrix(embedding_dim, self.number_of_levels)
        self.register_buffer('W', W)
        self.register_buffer('W_T', W.T.contiguous())

        gelu_mask = self._build_gelu_mask(embedding_dim, self.number_of_levels)
        self.register_buffer('gelu_mask', gelu_mask)

    def _build_dwt_matrix(self, N: int, number_of_levels: int) -> torch.Tensor:
        """
        Build the full N×N orthogonal DWT matrix for D4 decomposition.
        DWT(x) = W @ x, IDWT(y) = W^T @ y
        """
    ...
    def _build_gelu_mask(self, N: int, number_of_levels: int) -> torch.Tensor:
        """
        Build a boolean mask of shape (N,) marking coefficient positions
        where GELU is applied (detail coefficients at self.levels_gelu).

        Coefficient layout after DWT:
          [0 : N//2^L]              = approximation (deepest level)
          [N//2^L : N//2^(L-1)]     = detail level (L-1)
          ...
          [N//2 : N]                = detail level 0
        """
        mask = torch.zeros(N, dtype=torch.bool)
        deepest_approximation_size = N // (2 ** number_of_levels)
        cursor = deepest_approximation_size

        for level in range(number_of_levels - 1, -1, -1):
            detail_size = N // (2 ** (level + 1))
            if level in self.levels_gelu:
                mask[cursor:cursor + detail_size] = True
            cursor += detail_size
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: (B, N, D) — apply wavelet FFN to each token embedding independently.
        """
        batch_size, sequence_length, dimension = x.shape
        x_flat = x.reshape(batch_size * sequence_length, dimension)

        coefficients = x_flat @ self.W_T
        activated = coefficients.clone()
        activated[:, self.gelu_mask] = F.gelu(coefficients[:, self.gelu_mask])
        reconstructed = activated @ self.W

        output = reconstructed * self.gamma + self.beta
        return output.reshape(batch_size, sequence_length, dimension)
...
class DSQGBlock(nn.Module):
    def __init__(self, embedding_dim, num_heads, ffn_dim, seq_len,
        ...
        self.wavelet_ffn = WaveletFFN(embedding_dim)
    ...
    def forward(self, x):
        ...
        ffn_out = self.wavelet_ffn(self.norm2(x))
        x = x + ffn_out
...
class CondMTransformer(nn.Module):
    ...
    def attn_summary(self):
        ...
        return {
            ...
            'wavelet_gamma_abs_mean': sum(
                b.wavelet_ffn.gamma.abs().mean().item() for b in dsqg_blocks) / n,
        }
...
def dwt_invertibility_check(device):
    """Verify that WaveletFFN roundtrips correctly when nonlinearity is disabled."""
    print('Running DWT invertibility check...')
    wffn = WaveletFFN(256, levels_with_gelu=[]).to(device)
    with torch.no_grad():
        wffn.gamma.fill_(1.0)
        wffn.beta.fill_(0.0)
    x_test = torch.randn(4, 16, 256, device=device)
    with torch.no_grad():
        out = wffn(x_test)
    max_error = (out - x_test).abs().max().item()
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        print(f'  Wavelet:       gamma |mean|={ss["wavelet_gamma_abs_mean"]:.4f}')
        ...
        per_epoch_results.append({
            ...
            'wavelet_gamma_abs_mean': ss['wavelet_gamma_abs_mean'],
        })
        sys.stdout.flush()
    ...
    print(f'  Final scale_embed: |mean|={ss["scale_embed_abs_mean"]:.4f} '
          f'|max|={ss["scale_embed_abs_max"]:.4f}')
    print(f'  Final wavelet gamma: |mean|={ss["wavelet_gamma_abs_mean"]:.4f}')
    if ss['ema_factors']:
...
def main():
    ...
    print('=' * 70)
    print(f'  {EXPERIMENT_NAME} -- Wavelet FFN + Pure DSQG (14M scale, FineWeb-Edu)')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}  '
              f'({torch.cuda.get_device_properties(0).total_memory/1e9:.1f} GB)')
    print(f'  Kernel: dsqg_attention_v3 (d48 offsets)')
    print(f'  Wavelet FFN: D4, alternating levels [0,2,4,6], full D=256 transform')
    ...
    layer_types = ['FULL' if i == FULL_ATTN_LAYER else 'DSQG'
                   for i in range(NUM_LAYERS)]
    wavelet_params = sum(p.numel() for n, p in model.named_parameters() if 'wavelet' in n)
    print(f'\n{EXPERIMENT_NAME}: {n_params:,} parameters ({wavelet_params:,} wavelet FFN)')
    print(f'  Layer types: {layer_types}')
    print(f'  Standard FFN would be ~3,153,408 params; wavelet FFN is {wavelet_params:,} params')
