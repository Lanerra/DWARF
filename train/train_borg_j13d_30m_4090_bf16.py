"""
🔬 DWARF J13D — 30M Science Experiment
...
Run (from repo root, on 4090):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_borg_j13d_30m_4090_bf16.py > logs/run_j12_30m.log 2>&1 &
"""
...
def save_resume_checkpoint(model, optimizer, scheduler, epoch, git_hash, checkpoint_dir):
    ...
    out_path = os.path.join(checkpoint_dir, f"j13d_30m_ep{epoch}_resume.pt")
    torch.save({
        "epoch":                epoch,
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":           FFN_DIM,
            "seq_len":           MAX_SEQ_LEN,
            "source_script":     "train/train_borg_j13d_30m_4090_bf16.py",
    ...
    out_path = os.path.join(checkpoint_dir, f"j13d_30m_ep{epoch}_full_attn.pt")
...
def train():
    ...
    best_ckpt_name = 'j13d_30m_best.pt'
    ...
    for ep in range(SCREEN_EPOCHS, 0, -1):
        candidate = os.path.join(CHECKPOINT_DIR, f"j13d_30m_ep{ep}_resume.pt")
