...
Baselines:
  Moonshot-58M ep2     (PPL=35.04, passkey=99.2%, ar_score= 0.00)
  FFN=1×D (512) ep2    (PPL=62.50, passkey=80.8%,  ar_score=−27.30)  — threshold crossed ep2
  Depth-16 FA@L4 ep3   (PPL=44.49, passkey=99.2%, ar_score=−4.62)
...
Config:
  ...
  - FFN = 768 (1.5×D) — testing sweet spot between 1×D (threshold in ep2) and 2×D (threshold in ep1)
...
Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_ffn768_44m_l16_4090_bf16.py \\
    > logs/run_ffn768_44m_l16.log 2>&1 &
"""
...
FFN_DIM          = 768        # 1.5×D — sweet spot test: 1×D threshold in ep2, 2×D in ep1
NUM_LAYERS       = 16
FULL_ATTN_LAYER  = 4          # 11 post-FA relay layers (L5-15); 3 pre-FA warm-up = 25% of depth

MAX_TRAIN_SEQS       = 121_232   # ~22% Chinchilla for 50.9M params (20T/seq, seq_len=2048); overridden below after imports if MAX_TRAIN_SEQS_OVERRIDE env var is set
SCALE_EMBED_INIT_VAL = 0.15  # slight push toward ep1 crossing
...
VOCAB_SIZE     = 32000
BATCH_SIZE     = 16
GRAD_ACCUM     = 8    # effective batch = 128 (matches Moonshot baseline)
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":       FFN_DIM,
            "seq_len":       MAX_SEQ_LEN,
            "source_script": "train/train_ffn768_44m_l16_4090_bf16.py",
    ...
    out_path = os.path.join(checkpoint_dir, f"ffn768_44m_l16_ep{epoch}_full_attn.pt")
...
def train():
    ...
    _WARMUP_BS   = BATCH_SIZE  # MUST match training BATCH_SIZE — Triton specialises on (B,N)
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(clean_state,
                       os.path.join(CHECKPOINT_DIR, 'ffn768_44m_l16_best.pt'))
            marker = ' *'
        ...
        torch.save({
            ...
            'val_loss': val_loss,
        }, os.path.join(CHECKPOINT_DIR, f'ffn768_44m_l16_ep{epoch}_resume.pt'))
