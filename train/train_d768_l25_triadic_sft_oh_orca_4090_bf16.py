"""
🔧 DWARF D=768 L=25 Triadic J=96 — OH+Orca SFT, UNFROZEN scale_embed (freak flag)

Architecture: D=768, H=12 (hd=64), L=25, FFN=1536, TIED lm_head
  Triadic partitioning: 96 offsets split into 3 pure groups of 32 (no tail)
  Per-layer bandwidth ratio at D=768: (32×64)/768 = 2.67x (safe ≤ 3.0x)

SFT notes:
  - Resumes from d768_l25_triadic_j96_best.pt (ep3, PPL=30.92, passkey=96.7%)
...
Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_d768_l25_triadic_sft_oh_orca_4090_bf16.py
"""
...
SCALE_EMBED_LR_MULT  = 2.0   # SFT: reduced from 18.37 — relay topology already crystallized
EMA_INIT  = 0.020833
EMA_FLOOR = 0.00001
...
MAX_TRAIN_SEQS = int(os.environ.get('DWARF_MAX_TRAIN_SEQS', '75000'))   # SFT: full OH+Orca 75K
MAX_SEQ_LEN    = 2048
MAX_VAL_SEQS   = 5_582
CE_CHUNK       = 512
...
CKPT_BASE_NAME    = 'd768_l25_triadic_sft_oh_orca'
...
def _collate_sft(examples, device, pad_id=0):
    """Collate SFT (ids_list, loss_mask_list) tuples into (x, y) tensors.
...
def train():
    ...
    print('=' * 70)
    print('  🚀 DWARF D768-L25 Triadic J=96 — phase-informed partitioning, cold start')
    ...
    encoded_path = 'logs/oh_orca_75k_sft_encoded.pt'
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer,
        lr_lambda=[_cosine_lambda, _cosine_lambda])
    ...
    resume_path = os.getenv('DWARF_RESUME', 'autoresearch/checkpoints/d768_l25_triadic_j96_best.pt')
