"""
🚀 DWARF Autoresearch — J24D-int2 100M H100 SXM Optimized

100M parameter variant (~108M actual) optimized for H100 80GB SXM.

Architecture scale-up from 35M baseline:
  - D=1024 (was 512), head_dim=128
  - FFN=4096 (was 2048), 4×D ratio
  - L=6, H=8, V=32000 (unchanged)

H100 optimizations:
  - BATCH_SIZE=16 (was 8)
  - Flash attention via SDPA with enable_flash=True
  - torch.compile with max-autotune for FullAttnBlock
  - DataLoader: num_workers=4, pin_memory=True

Run (from /workspace/DWARF on RunPod):
  python -u train/train_j24d_int2_100m_h100.py > logs/run_j24d_100m.log 2>&1 &
"""
...
class FullCausalAttentionH100(nn.Module):
    """Full causal attention optimized for H100 with flash attention SDPA."""
...
class FullAttentionBlockH100(nn.Module):
    def __init__(self, embedding_dim, num_heads, ffn_dim, dropout=0.1):
        super().__init__()
        self.norm1 = nn.LayerNorm(embedding_dim)
        self.norm2 = nn.LayerNorm(embedding_dim)
        self.attn  = FullCausalAttentionH100(embedding_dim, num_heads, dropout)
        self.ffn   = FFN(embedding_dim, ffn_dim, dropout)
...
class AutoresearchTransformer100M(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads,
        ...
        for i in range(num_layers):
            if i == full_attn_layer:
                blocks.append(FullAttentionBlockH100(
...
def train():
    ...
    print('=' * 70)
    print('  J24D-int2 100M — H100 SXM Optimized')
    print('=' * 70)
    ...
    print(f'  Batch={BATCH_SIZE}, BF16, flash-attn SDPA')
    ...
    model = AutoresearchTransformer100M(
    ...
    try:
        model.blocks[FULL_ATTN_LAYER] = torch.compile(
            model.blocks[FULL_ATTN_LAYER],
            mode="max-autotune",
            fullgraph=False
        )
        print(f"  compiled FullAttnBlock at layer {FULL_ATTN_LAYER} (max-autotune)")
    ...
    print(f'description:    J24D-int2 100M H100 — D={EMBEDDING_DIM}/H={NUM_HEADS}/FFN={FFN_DIM}/L={NUM_LAYERS}, int=2 fa=5, J24 offsets, condV physics, {n_params/1e6:.1f}M')
