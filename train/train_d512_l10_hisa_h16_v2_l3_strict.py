"""
DWARF D=512 L=10 Triadic J=96 — strict-causal HISA/DSQG hybrid v2, relocated HISA to L3.
...
Architecture: D=512, H=8 (hd=64), L=10, FFN=1536, tied lm_head
  ...
  HISA block: strict HISA (V16 reference or V17 fused Triton candidate), C=32, chunk_size=64, top_k=4, HISA_m=32
...
from hierarchical_sparse_attn_v16_hisa_strict import HierarchicalSparseAttentionV16HISAStrict
from hierarchical_sparse_attn_v17_hisa_strict_triton import HierarchicalSparseAttentionV17HISAStrictTriton

HISA_IMPL = os.getenv('DWARF_HISA_IMPL', 'triton').strip().lower()
if HISA_IMPL not in {'ref', 'triton'}:
    raise ValueError(f"Unsupported DWARF_HISA_IMPL={HISA_IMPL!r}; expected 'ref' or 'triton'")

HISA_BACKWARD_IMPL = os.getenv('DWARF_HISA_BACKWARD_IMPL', 'triton').strip().lower()
if HISA_BACKWARD_IMPL not in {'eager', 'triton'}:
    raise ValueError(
...
HISA_IMPL_CLS = (
    ...
    if HISA_IMPL == 'ref' else
    HierarchicalSparseAttentionV17HISAStrictTriton
)
HISA_IMPL_LABEL = (
    'V16HISA_STRICT reference'
    if HISA_IMPL == 'ref' else
    'V17HISA_STRICT Triton (fused strict core + Stage-2 builder)'
)
if HISA_IMPL == 'ref':
    HISA_BACKWARD_IMPL = 'eager'
...
if _compile_env is None:
    # Strict HISA path currently runs faster in eager mode on this trainer; opt in explicitly.
...
TOKENIZER_CANDIDATES = ['tokenizers/mixed_tokenizer_32k.json']
...
CKPT_BASE_NAME    = 'd512_l10_hisa_h16_v2_l3_strict'

CHECKPOINT_STRATEGY = os.getenv('DWARF_CKPT', 'all').lower()
if CHECKPOINT_STRATEGY not in {'auto', 'none', 'all', 'every_other', 'full_attn'}:
    raise ValueError(
        f"Unsupported DWARF_CKPT={CHECKPOINT_STRATEGY!r}; "
        "expected one of auto/none/all/every_other/full_attn"
    )


def _resolve_checkpoint_strategy(requested: str, device: str) -> tuple[str, str]:
    if requested != 'auto':
        return requested, 'explicit'
    if device != 'cuda' or not torch.cuda.is_available():
        return 'all', 'auto->all (non-CUDA safety fallback)'

    total_gb = torch.cuda.get_device_properties(0).total_memory / (1024 ** 3)
    if HISA_IMPL == 'triton' and total_gb >= 21.0:
        return 'none', f'auto->none ({total_gb:.1f} GB GPU, strict Triton path fits without ckpt)'
    if HISA_IMPL == 'triton':
        return 'full_attn', f'auto->full_attn ({total_gb:.1f} GB GPU, preserve memory while avoiding blanket ckpt)'
    return 'all', f'auto->all ({total_gb:.1f} GB GPU, reference strict path safety default)'
...
LAYER_LAYOUT = [
    ...
    ('DSR', None, 0, 0, False),                    # L03: strict HISA (V16 ref / V17 Triton candidate)
...
class DSRBlock(nn.Module):
    """DSR block: strict HISA attention + FFN + SDPA output gate."""
    def __init__(self, embedding_dim, num_heads, ffn_dim, head_dim,
                 num_chunks, top_k_chunks, dropout=0.1, hisa_top_m_tokens=32, chunk_size=64):
        super().__init__()
        self.norm1 = _LayerNorm(embedding_dim)
        self.norm2 = _LayerNorm(embedding_dim)
        self.attn = HISA_IMPL_CLS(
            ...
            hisa_top_m_tokens=hisa_top_m_tokens, chunk_size=chunk_size,
            **(
                {
...
def train():
    ...
    resolved_checkpoint_strategy, checkpoint_strategy_reason = _resolve_checkpoint_strategy(
        CHECKPOINT_STRATEGY,
        device,
    )
    ...
    print(
        f'  checkpoint_strategy={resolved_checkpoint_strategy} '
        f'(requested={CHECKPOINT_STRATEGY}; {checkpoint_strategy_reason})'
    )
    ...
    encoded_path = 'datasets/mixed_encoded_2048_v6_modular_fw1000000.pt'
    ...
    model.checkpoint_strategy = resolved_checkpoint_strategy
    ...
    print('\n' + '=' * 70)
    print(f'  STRICT HISA + R_PLANES={R_PLANES} Summary (D{EMBEDDING_DIM}-L{NUM_LAYERS}, tied lm_head)')
    print('=' * 70)
