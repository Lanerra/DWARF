"""
FiLM-Pure — FiLM-Conditioned FFN + All-DSQG (14M, 3-Epoch Probe)

FiLM v2 — Per-Head conditioning via IF gains:
  s_h = if_gain.detach()                        (H=8 values, one per head; no grad through s)
  gamma_h = (1 + W_gamma[h] * s_h[h])          (per-head scale, shape H×head_dim)
  beta_h  = W_beta[h] * s_h[h]                 (per-head shift)
  ffn_out reshaped to (B,N,H,HD), conditioned per head, reshaped back

Upgrade from v1: scalar mean → per-head (8× more informative, 9× better head differentiation)
  IF gains already differentiate h0 (global, ~1.15) from h7 (local, ~1.0)
  h0 gets different gamma/beta than h7 → FiLM can reinforce head specialization

At init (W_gamma=0.1, W_beta=0.0): ~10% modulation at typical if_gain≈1.0 (normalized init)
its output based on which temporal offsets the attention layer selected.
...
Run:
  CUDA_VISIBLE_DEVICES=1 .venv/bin/python3 -u train/train_2048_14m_d48_film_pure.py \
    2>&1 | tee benchmarks/logs/d48_film_pure_14m.log
"""
...
EXPERIMENT_NAME = 'd48_film_pure_14m_v2'
...
SAVE_DIR    = 'checkpoints/d48_film_pure_14m_v2'
RESULT_FILE = 'benchmarks/logs/d48_film_pure_14m_v2_results.json'
...
class DSQGAttentionQW(nn.Module):
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        self.scale_embed = nn.Parameter(torch.zeros(52, HD))
...
# -- DSQGBlock with FiLM conditioning -----------------------------------------
...
class DSQGBlock(nn.Module):
    def __init__(self, embedding_dim, num_heads, ffn_dim, seq_len,
        ...
        # FiLM v2: per-head conditioning via IF gains (H×head_dim shape)
        # Normalized init: W_gamma=0.1 → ~10% modulation at if_gain≈1.0
        self.num_heads = num_heads
        self.head_dim_film = embedding_dim // num_heads
        self.film_gamma = nn.Parameter(torch.full((num_heads, embedding_dim // num_heads), 0.1))
        self.film_beta  = nn.Parameter(torch.zeros(num_heads, embedding_dim // num_heads))
    ...
    def forward(self, x):
        ...
        # FiLM v2: per-head conditioning via IF gains
        B_f, N_f, D_f = ffn_out.shape
        H_f, HD_f = self.num_heads, self.head_dim_film
        s_h = self.attn.if_gain.detach()           # (H,) — no grad through signal
        gamma_h = 1.0 + self.film_gamma * s_h.unsqueeze(-1)  # (H, HD)
        beta_h  = self.film_beta  * s_h.unsqueeze(-1)        # (H, HD)
        ffn_out = ffn_out.view(B_f, N_f, H_f, HD_f)
        ffn_out = ffn_out * gamma_h + beta_h
        ffn_out = ffn_out.view(B_f, N_f, D_f)
        x = x + ffn_out
    ...
    def attn_summary(self):
        ...
        if not dsqg_blocks:
            return {'pos_bias_abs_mean': 0.0, 'pos_bias_abs_max': 0.0,
                    ...
                    'film_gamma_abs_mean': 0.0, 'film_beta_abs_mean': 0.0}
        ...
        return {
            ...
            'film_gamma_abs_mean': sum(b.film_gamma.abs().mean().item() for b in dsqg_blocks) / n,
            'film_beta_abs_mean':  sum(b.film_beta.abs().mean().item()   for b in dsqg_blocks) / n,
        }
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        print(f'  FiLM:          gamma |mean|={ss["film_gamma_abs_mean"]:.4f}  '
              f'beta |mean|={ss["film_beta_abs_mean"]:.4f}')
        ...
        per_epoch_results.append({
            ...
            'film_gamma_abs_mean': ss['film_gamma_abs_mean'],
            'film_beta_abs_mean':  ss['film_beta_abs_mean'],
        })
        sys.stdout.flush()
    ...
    print(f'  Final scale_embed: |mean|={ss["scale_embed_abs_mean"]:.4f} '
          f'|max|={ss["scale_embed_abs_max"]:.4f}')
    print(f'  Final FiLM: gamma |mean|={ss["film_gamma_abs_mean"]:.4f}  '
          f'beta |mean|={ss["film_beta_abs_mean"]:.4f}')
    if ss['ema_factors']:
...
def main():
    ...
    print('=' * 70)
    print(f'  {EXPERIMENT_NAME} -- FiLM-Conditioned FFN + Pure DSQG (14M, FineWeb-Edu)')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}  '
              f'({torch.cuda.get_device_properties(0).total_memory/1e9:.1f} GB)')
    print(f'  Kernel: dsqg_attention_v3 (d48 offsets)')
    print(f'  FiLM v2 conditioning: per-head via IF gains, normalized init (0.1), detached signal')
    ...
    layer_types = ['FULL' if i == FULL_ATTN_LAYER else 'DSQG'
                   for i in range(NUM_LAYERS)]
    film_params = sum(p.numel() for n, p in model.named_parameters() if 'film' in n)
    print(f'\n{EXPERIMENT_NAME}: {n_params:,} parameters ({film_params:,} FiLM)')
