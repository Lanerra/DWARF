"""
🚀 DWARF D=768 L=24 — ~154M params, depth-scaling test, cold-start

Architecture: D=768, H=12 (hd=64), L=24, FFN=1536, J=24 (se015 offsets), TIED lm_head
  L0-L4:  DSQGBlockV6Physics  IF=False  ← 5 pre-FA warm-up relay layers
  L5:     DSQGBlockV6Physics  IF=True   ← preIF@L5
  L6:     FullAttentionBlock            ← FA@L6 (25% depth, 17 post-FA relay layers)
...
Run (from repo root, on H100 pod):
  .venv/bin/python3 -u train/train_d768_l24_h100_bf16.py \\
    2>&1 | tee logs/run_d768_l24.log
"""
...
NUM_LAYERS       = 24
FULL_ATTN_LAYER  = 6          # 25% of 24 = L6; 17 post-FA relay layers (L7-23); preIF@L5

MAX_TRAIN_SEQS       = 331_000   # 22% Chinchilla for 154M params (0.22 × 154M × 20 / 2048 ≈ 331K)
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":       FFN_DIM,
            "seq_len":       MAX_SEQ_LEN,
            "source_script": "train/train_d768_l24_h100_bf16.py",
            ...
            "git_hash":      git_hash,
            "note": (
                f"D768-L24-FA6: D={EMBEDDING_DIM} H={NUM_HEADS} L={NUM_LAYERS} "
                f"FFN={FFN_DIM} J={len(OFFSETS)} FA@L{FULL_ATTN_LAYER} "
                f"preIF@L{FULL_ATTN_LAYER-1}. Epoch {epoch}/3. Cold start."
            ),
        },
    }
    out_path = os.path.join(checkpoint_dir, f"d768_l24_fa6_ep{epoch}_full_attn.pt")
...
def train():
    ...
    print('=' * 70)
    print('  🚀 DWARF D768-L24 FA@L6 — D=768 H=12 hd=64 L=24 FFN=1536 J=24, cold start')
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(clean_state,
                       os.path.join(CHECKPOINT_DIR, 'd768_l24_fa6_best.pt'))
            marker = ' *'
        ...
        torch.save({
            ...
            'val_loss': val_loss,
            'val_ppl':  val_ppl,
        }, os.path.join(CHECKPOINT_DIR, f'd768_l24_fa6_ep{epoch}_resume.pt'))
    ...
    print(f'description: D768-L24-FA6 D={EMBEDDING_DIM} H={NUM_HEADS} hd=64 L={NUM_LAYERS} '
