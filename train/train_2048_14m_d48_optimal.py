"""
d48_optimal_14m — First synthesis run with March 10 autoresearch physics insights.

Based on condV (d41s3) with three key changes from autoresearch:

  1. d48 offset set (J=51):
     Dense local W=48, dyadic long-range [96, 128, 384].
     Winner from autoresearch sweep; within J ≤ 52 budget.

  2. Two-regime pos_bias initialization:
     Local regime (δ ≤ 128): standard negative distance penalty.
     Distal regime (δ = 384): positive init (anti-phase complement).
     Motivated by binary local/distal architecture: +0.9996 cross-correlation
     in local regime, -0.9822 in distal regime, with 16,000× phase
     discontinuity at the mid→long boundary.

  3. Scale_embed soft L2 regularization:
     Prevents abs_max overshoot beyond 1.0 after the ep3 phase transition.
     Optimal range is abs_mean ~0.21–0.23. Overshoot kills long-range
     retrieval at later epochs.

All other components retained from condV (d41s3):
  - Q-weighted scale gains (V3 kernel)
  - IF amplifier (per-head gain)
  - Huygens K/V injection
  - Kalman-EMA + KdV soliton + AGC interference

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_2048_14m_d48_optimal.py \
    2>&1 | tee benchmarks/logs/d48_optimal_14m.log
"""
...
EXPERIMENT_NAME = 'd48_optimal_14m'
SAVE_DIR        = 'checkpoints/d48_optimal_14m'
RESULT_FILE     = 'benchmarks/logs/d48_optimal_14m_results.json'
...
assert len(_COND_N_OFFSETS) == 52  # range(0,49)=49 dense + [96,128,384]=3 sparse

# -- Two-regime pos_bias configuration ----------------------------------------

DISTAL_THRESHOLD = 200

# -- Scale_embed regularization configuration ----------------------------------
...
from dsqg_probe_d48_s96_128_384 import dsqg_attention_v3  # 52-offset d48 kernel


# -- DSQGAttentionQW -- Q-Weighted scale gains + IF amplifier ------------------

_NUM_OFFSETS = len(_COND_N_OFFSETS)

class DSQGAttentionQW(nn.Module):
    """
    DSQG attention with Q-weighted scale gains + IF amplifier.

    Two-regime pos_bias initialization:
      Local regime (δ ≤ DISTAL_THRESHOLD): negative distance penalty (standard).
      Distal regime (δ > DISTAL_THRESHOLD): positive init (anti-phase complement).
    """
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        distal_mask = (torch.tensor(_COND_N_OFFSETS, dtype=torch.float32) > DISTAL_THRESHOLD)
        pos_bias_init[distal_mask] = -pos_bias_init[distal_mask]
        self.pos_bias = nn.Parameter(pos_bias_init)

        self.scale_embed = nn.Parameter(torch.zeros(_NUM_OFFSETS, HD))
...
def _compute_scale_embed_regularization(model):
    """Soft L2 penalty on scale_embed values exceeding SCALE_EMBED_MAX_ABS."""
    penalty = 0.0
    for block in model.blocks:
        if hasattr(block, 'attn') and hasattr(block.attn, 'scale_embed'):
            se = block.attn.scale_embed
            excess = (se.abs() - SCALE_EMBED_MAX_ABS).clamp(min=0.0)
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
        ...
        for acc_step in range(steps_per_epoch):
            for ga in range(GRAD_ACCUM):
                ...
                with torch.amp.autocast('cuda'):
                    ...
                    # ⚡ Scale_embed soft regularization
                    scale_embed_reg = _compute_scale_embed_regularization(model)
                    loss = loss + SCALE_EMBED_REG_LAMBDA * scale_embed_reg / GRAD_ACCUM
    ...
    results = {
        ...
        'offset_set':              f'd{_DENSE_LOCAL_W} + {_DYADIC_LONG_RANGE} = J={_NUM_OFFSETS}',
        'two_regime_pos_bias':     True,
        'distal_threshold':        DISTAL_THRESHOLD,
        'scale_embed_reg_lambda':  SCALE_EMBED_REG_LAMBDA,
        'scale_embed_max_abs':     SCALE_EMBED_MAX_ABS,
...
def main():
    ...
    print('=' * 70)
    print(f'  {EXPERIMENT_NAME} — d48 Optimal Synthesis (14M, FineWeb-Edu)')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}  '
              f'({torch.cuda.get_device_properties(0).total_memory/1e9:.1f} GB)')
    print(f'  Kernel: dsqg_probe_d48_s96_128_384 (52-offset d48 kernel)')
    print(f'  Offset set: d{_DENSE_LOCAL_W} dense + {_DYADIC_LONG_RANGE} = J={_NUM_OFFSETS}')
    print(f'  Two-regime pos_bias: ENABLED (δ=384 init flipped, threshold={DISTAL_THRESHOLD})')
    print(f'  Scale_embed reg:     ENABLED (lambda={SCALE_EMBED_REG_LAMBDA}, max_abs={SCALE_EMBED_MAX_ABS})')
    print(f'  ISO-COMPUTE CAP:     MAX_TRAIN_SEQS={MAX_TRAIN_SEQS:,}')
    print(f'  Retained: Q-weighted scale gains, IF amplifier, Huygens K/V inject')
    print(f'  Retained: Kalman-EMA + KdV soliton + AGC (interference)')
    print(f'  Architecture: INTERFERENCE=3, gate=0, PURE DSQG (FULL_ATTN_LAYER=-1)')
