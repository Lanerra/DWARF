...
Config:
  ...
  - Chunked-lm_head CE: [B,N,D] hidden → per-chunk out proj → no [B,N,V] materialization
  - No dq/dk fp32 cast in backward (Spike A fix)
  - BF16 probe einsum (Spike B fix)
...
class AutoresearchTransformerPhysics(nn.Module):
    ...
    def forward_hidden(self, idx):
        """Return normed hidden states [B, N, D] without the lm_head projection.
        Use with chunked_ce_loss to avoid materialising the full [B, N, V] logits tensor."""
        B, N = idx.shape
        x    = self.drop(self.embedding(idx))
        for i, block in enumerate(self.blocks):
            if self.training and self._should_checkpoint_block(i):
                ...
                x = block(x)
        return self.norm(x)   # [B, N, D], still on the compute graph
...
def train():
    ...
    _whid_flat = _whidden.reshape(-1, _whidden.size(-1))   # [B*N, D]
    _wy_flat   = _wy.reshape(-1)
    _wT        = _whid_flat.size(0)
    _wgrad_hid = torch.zeros_like(_whid_flat)
    for _wcs in range(0, _wT, CE_CHUNK):
        _wce    = min(_wcs + CE_CHUNK, _wT)
        _wchunk = _whid_flat[_wcs:_wce].detach().requires_grad_(True)
        _wlogit = model.out(_wchunk)
        _wloss  = F.cross_entropy(_wlogit.float(), _wy_flat[_wcs:_wce], reduction='sum')
        _wloss.backward()
        _wgrad_hid[_wcs:_wce] = _wchunk.grad
    _whid_flat.backward(_wgrad_hid / _wT)
    optimizer.zero_grad(set_to_none=True)
    del _wx, _wy, _whidden, _whid_flat, _wy_flat, _wloss
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        for acc_step in range(steps_per_epoch):
            for ga in range(GRAD_ACCUM):
                ...
                with _amp_context(device):
                    hidden = model.forward_hidden(x)   # [B, N, D]
                ...
                for chunk_start in range(0, T, CE_CHUNK):
                    ...
                    chunk_hid   = hidden_flat[chunk_start:chunk_end].detach().requires_grad_(True)
                    chunk_logit = model.out(chunk_hid)
                    chunk_loss  = F.cross_entropy(
                        chunk_logit.float(), y_flat[chunk_start:chunk_end], reduction='sum')
                    chunk_loss.backward()
                    grad_hidden[chunk_start:chunk_end] = chunk_hid.grad
