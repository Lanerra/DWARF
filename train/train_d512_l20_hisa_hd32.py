"""
DWARF D=512 L=8 Triadic J=96 — DSR/HISA@L3 + R_PLANES=4, TIED LM_HEAD

Architecture: D=512, H=8 (hd=64), L=8, FFN=2048, TIED lm_head
...
def _grad_norm_from_parameters(parameters) -> float:
    total_sq = 0.0
    for p in parameters:
        if p.grad is not None:
            total_sq += p.grad.detach().float().square().sum().item()
...
CKPT_BASE_NAME    = 'd512_l24_hisa_hd32'

CHECKPOINT_STRATEGY = os.getenv('DWARF_CKPT', 'all').lower()

# =============================================================================
# LAYER LAYOUT: L=8, DSR@L3
# Pre-DSR: 1 triad (L0-2), Post-DSR: 4 triads (L4-15)
# =============================================================================
...
LAYER_LAYOUT = [
    ...
    ('C', GROUP_C, J_SMALL_C, J_LARGE_C, True),    # L08
    ('DSR', None, 0, 0, False),                    # L09: HierarchicalSparseAttentionV15HISA
...
def train():
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        for acc_step in range(steps_per_epoch):
            ...
            grad_norm_snapshot = _grad_norm_from_parameters(model.parameters())
            optimizer.step()
            optimizer.zero_grad(set_to_none=True)
            scheduler.step()
            step += 1
