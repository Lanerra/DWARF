"""
🔬 DWARF Autoresearch — J16D Full-Dataset + Physics

Builds on the confirmed J16D fulldata result (ar_score=+57.53, 75.8% passkey ep3).
Adds condV physics to the interference layers:
  1. Kalman-EMA  — causal exponential moving average (replaces cumsum mean)
  2. KdV         — soliton field correction (zero-init, activates during training)
  3. AGC         — automatic gain control (no params, normalises K/V injection)

Key fix vs prior d41-J16D failure (EMA collapsed to 10t window):
  EMA α₀ = 0.003  (window ≈ 333t at init, vs 0.03 → 33t that collapsed)
  EMA floor = 0.0003  (was 0.001 — give more room to find long-window attractor)
  Full dataset (121,232 seqs) so relay gradient signal is strong from ep1.

Hypothesis: strong relay chains (proven at fulldata) give the EMA gradient
the right direction signal to keep the window long. The capped-data collapse
happened because local gradients dominated — that bottleneck is now removed.

Run (from /tmp/dwarf-j17d):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u autoresearch/train_j16d_physics.py \
    > autoresearch/run_j16d_physics.log 2>&1 &
"""
...
EMA_INIT  = 0.003
EMA_FLOOR = 0.0003   # was 0.001 — allow model to find long-window attractor
...
def train():
    ...
    print('=' * 70)
    print('  🔬 DWARF Autoresearch — J16D Full-Dataset + Physics')
    print('  condV: Kalman-EMA (α₀=0.003) + KdV (zero-init) + AGC')
    print('=' * 70)
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(CHECKPOINT_DIR,
                                    f'{git_hash}_j16d_physics_best.pt'))
            marker = ' *'
        ...
        # Log EMA window — this is the diagnostic for collapse vs stability
    ...
    print(f'description:    J16D physics — {n_params/1e6:.1f}M, V6+condV, '
          f'EMA_α₀={EMA_INIT}, 121232 seqs, scale_embed_init={SCALE_EMBED_INIT_VAL}')
