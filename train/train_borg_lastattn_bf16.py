...
EXTRACTED_CKPT = "autoresearch/checkpoints/j26d_full_attn_extracted.pt"
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    out_path = os.path.join(checkpoint_dir, f"borg_lastattn_ep{epoch}_full_attn.pt")
...
def train(no_warmstart: bool = False):
    ...
    print('=' * 70)
    print('  🔬 DWARF Autoresearch — BORG LAST-ATTN RUN')
    print('  FullAttn at L5 (last), warm-started from J26D (5 DSQG blocks before FA — matches J26D topology)')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}')
    print(f'  D={EMBEDDING_DIM}, H={NUM_HEADS}, L={NUM_LAYERS}, FFN={FFN_DIM}')
    print(f'  IF interval={INTERFERENCE}, Full attn layer={FULL_ATTN_LAYER} (LAST, L5 of 6)')
    ...
    best_ckpt_name = f'borg_lastattn_best.pt'
    if no_warmstart:
    ...
    if not no_warmstart and os.path.exists(EXTRACTED_CKPT):
        ...
        for k, v in full_attn_weights.items():
            new_key = k.replace(f"blocks.5._orig_mod.", f"blocks.{FULL_ATTN_LAYER}.")
            new_key = new_key.replace(f"blocks.5.", f"blocks.{FULL_ATTN_LAYER}.")
            remapped[new_key] = v
...
if __name__ == '__main__':
    ...
    if args.no_warmstart:
        ...
        print("  Checkpoint: borg_lastattn_best.pt\n")
