"""
DWARF D=512 L=11 Dual-HISA Triadic J=96 — HISA@L03 (C=64) + HISA@L07 (C=32), R_PLANES=4, TIED LM_HEAD

Architecture: D=512, H=8 (hd=64), L=11, FFN=1536, TIED lm_head
  Triadic partitioning: 96 offsets split into 3 pure groups of 32 (no tail)
  Two HISA layers with DIFFERENT chunk sizes:
    L03: HierarchicalSparseAttentionV15HISA (C=64, top_k=4, HISA_m=32)  -- coarse selection
    L07: HierarchicalSparseAttentionV15HISA (C=32, top_k=4, HISA_m=32)  -- fine selection
  preIF at L06 (information-forcing, between the two HISA layers).
All DSQG blocks use V20 R_PLANES=4 Triton kernel (MOVT matmul rotation, grouped sparse, SE gates).

  L00: DSQGBlock(GROUP_A)  -- triad 1
  L01: DSQGBlock(GROUP_B)
  L02: DSQGBlock(GROUP_C)
  L03: DSRBlock (C=64)     -- coarse HISA (before preIF triad)
  L04: DSQGBlock(GROUP_A)  -- triad 2
  L05: DSQGBlock(GROUP_B)
  L06: DSQGBlock(GROUP_C)  + preIF
  L07: DSRBlock (C=32)     -- fine HISA (after preIF triad)
  L08: DSQGBlock(GROUP_A)  -- triad 3
  L09: DSQGBlock(GROUP_B)
  L10: DSQGBlock(GROUP_C)

  Multi-scale routing: coarse (C=64) at L03 → DSQG refinement → preIF → fine (C=32) at L07.
  Full J=96 topological coverage within each 3-layer triad.

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_d512_l10_hisa_h8_v3.py
"""
...
# =============================================================================
# DUAL HISA CONFIG (L03: coarse C=64, L07: fine C=32)
# =============================================================================
DSR_LAYERS   = [3, 7]   # indices of HISA layers

HISA1_NUM_CHUNKS    = 64    # L03: coarse chunk selection
HISA1_TOP_K_CHUNKS  = 4
HISA1_TOP_M_TOKENS  = 32

HISA2_NUM_CHUNKS    = 32    # L07: fine chunk selection
HISA2_TOP_K_CHUNKS  = 4
HISA2_TOP_M_TOKENS  = 32

# Backwards-compatible aliases (L07 settings as default)
NUM_CHUNKS       = HISA2_NUM_CHUNKS
TOP_K_CHUNKS     = HISA2_TOP_K_CHUNKS
HISA_TOP_M_TOKENS = HISA2_TOP_M_TOKENS
...
GRAD_ACCUM     = int(os.environ.get('DWARF_GA', '1'))
MAX_TRAIN_SEQS = int(os.environ.get('DWARF_MAX_TRAIN_SEQS', '150000'))
MAX_SEQ_LEN    = 2048
MAX_VAL_SEQS   = 5_582
CE_CHUNK       = 1024
SCREEN_EPOCHS  = 3
...
CKPT_BASE_NAME    = 'd512_l10_hisa_h8_v3'

CHECKPOINT_STRATEGY = os.getenv('DWARF_CKPT', 'all').lower()

# =============================================================================
# LAYER LAYOUT: L=11, DUAL HISA (L03=C64 coarse, L07=C32 fine), preIF at L06
# =============================================================================
LAYER_LAYOUT = [
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L00
    ('B', GROUP_B, J_SMALL_B, J_LARGE_B, False),   # L01
    ('C', GROUP_C, J_SMALL_C, J_LARGE_C, False),   # L02
    ('DSR', 32, 4),                                # L03: HISA coarse (C=32, tk=4)
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L04
    ('B', GROUP_B, J_SMALL_B, J_LARGE_B, False),   # L05
    ('C', GROUP_C, J_SMALL_C, J_LARGE_C, True),    # L06: preIF
    ('DSR', 16, 4),                                # L07: HISA fine (C=16, tk=4)
...
class TriadicJ96Dsr(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_heads, ffn_dim, seq_len,
                 dsr_layers, scale_embed_init_val=0.15, dropout=0.1,
                 num_chunks=32, top_k_chunks=4):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.drop = nn.Dropout(dropout)
        self.dsr_layers = list(dsr_layers)   # e.g. [3, 7]
        self.head_dim = embedding_dim // num_heads

        blocks = []
        for i, entry in enumerate(LAYER_LAYOUT):
            label = entry[0]
            if label == 'DSR':
                # entry format: ('DSR', num_chunks, top_k_chunks)
                # backwards-compat: if len==1 it's a string 'DSR' (old format)
                if len(entry) > 1 and isinstance(entry[1], int):
                    c_val = entry[1]
                    tk_val = entry[2] if len(entry) > 2 else 4
                    # Per-HISA top_m_tokens: L03 → HISA1, L07 → HISA2
                    hisa_m = HISA1_TOP_M_TOKENS if i == 3 else HISA2_TOP_M_TOKENS
                else:
                    c_val = num_chunks
                    tk_val = top_k_chunks
                    hisa_m = HISA_TOP_M_TOKENS
                blocks.append(DSRBlock(
                    embedding_dim, num_heads, ffn_dim,
                    self.head_dim, c_val, tk_val, dropout,
                    hisa_top_m_tokens=hisa_m))
            else:
                offsets, js, jl, has_if = entry[1], entry[2], entry[3], entry[4]
    ...
    def _should_checkpoint_block(self, block_idx):
        if block_idx in set(self.dsr_layers):
            return False
        ...
        if CHECKPOINT_STRATEGY == 'full_attn':
            return block_idx in set(self.dsr_layers)
        return False
...
def train():
    ...
    print('=' * 70)
    print('  DWARF D512-L11 Dual-HISA Triadic J=96 — HISA@L03(C=64) + HISA@L07(C=32), R_PLANES=4')
    print(f'  HISA@L03: HierarchicalSparseAttentionV15HISA(C={HISA1_NUM_CHUNKS}, top_k={HISA1_TOP_K_CHUNKS}, HISA_m={HISA1_TOP_M_TOKENS}) -- COARSE')
    print(f'  HISA@L07: HierarchicalSparseAttentionV15HISA(C={HISA2_NUM_CHUNKS}, top_k={HISA2_TOP_K_CHUNKS}, HISA_m={HISA2_TOP_M_TOKENS}) -- FINE')
    ...
    encoded_path = 'logs/mixed_encoded_2048_v6_modular_fw1000000.pt'
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        for acc_step in range(steps_per_epoch):
            ...
            if should_log:
                ...
                entropy_parts = []
                for dl in DSR_LAYERS:
                    re = getattr(model_ref.blocks[dl].attn, '_routing_entropy', None)
                    if re is not None:
                        entropy_parts.append(f'L{dl}_ent={re:.3f}')
                entropy_str = (' ' + ' '.join(entropy_parts)) if entropy_parts else ''
        ...
        if _is_best:
            ...
            torch.save({
                ...
                'config': {
                    ...
                    'dsr_layers': DSR_LAYERS,
                    'hisa1_num_chunks': HISA1_NUM_CHUNKS, 'hisa1_top_k': HISA1_TOP_K_CHUNKS,
                    'hisa2_num_chunks': HISA2_NUM_CHUNKS, 'hisa2_top_k': HISA2_TOP_K_CHUNKS,
        ...
        for dl in DSR_LAYERS:
            routing_entropy = getattr(
                model_ref.blocks[dl].attn, '_routing_entropy', None)
            max_chunks = model_ref.blocks[dl].attn.num_chunks
            if routing_entropy is not None:
                print(f'  DSR routing entropy (L{dl}): {routing_entropy:.4f} '
                      f'(max={math.log(max_chunks):.2f}, min=0.00)')

        # Per-HISA config summary
        print(f'  HISA@L03: C={HISA1_NUM_CHUNKS}, top_k={HISA1_TOP_K_CHUNKS}, HISA_m={HISA1_TOP_M_TOKENS}')
        print(f'  HISA@L07: C={HISA2_NUM_CHUNKS}, top_k={HISA2_TOP_K_CHUNKS}, HISA_m={HISA2_TOP_M_TOKENS}')
    ...
    print(f'  HISA1 (L03): C={HISA1_NUM_CHUNKS}  top_k={HISA1_TOP_K_CHUNKS}  HISA_m={HISA1_TOP_M_TOKENS}')
    print(f'  HISA2 (L07): C={HISA2_NUM_CHUNKS}  top_k={HISA2_TOP_K_CHUNKS}  HISA_m={HISA2_TOP_M_TOKENS}')
