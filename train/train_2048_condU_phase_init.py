...
SAVE_DIR    = 'checkpoints/condU_phase_init'
RESULT_FILE = 'logs/condU_phase_init_results.json'
...
from dsqg_attention_v4 import dsqg_attention_v4, DSQGAttentionV4 as _DSQGAttentionV4Base


# -- DSQGAttentionQW -- V4: adds learned phase rotation per (offset, head) -----

class DSQGAttentionQW(nn.Module):
    """
    condU-phase: V3 + learned Givens phase rotation on V per (offset, head).
    ...
    Parameters:
      ...
      phase_embed [44, H]   -- Givens rotation angles in radians (N(0,0.01) init)

    Phase init = 0: identity rotation, backward compatible with condU V3.
    Training discovers non-zero phases where constructive/destructive
    interference in output space improves the objective.

    Hypothesis:
      Global heads → φ → 0 for long-range offsets (constructive alignment)
      Local heads  → φ → ±π for long-range offsets (destructive cancellation)
    """
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        HD             = self.head_dim
        assert HD % 2 == 0, "HD must be even for phase rotation"
        ...
        # Phase: small random init N(0, 0.01) — breaks symmetry from step 1.
        # Different heads start at different random angles so gradient descent
        # can pull them apart (constructive vs destructive) rather than waiting
        # for noise to escape the uniform-positive local minimum seen with
        # zero-init. Mean |phase| ≈ 0.008 rad at init — V rotation near-
        # identity but with head-specific starting directions.
        self.phase_embed = nn.Parameter(torch.randn(44, num_heads) * 0.01)
    ...
    def forward(self, x, kv_inject=None):
        ...
        # V4 kernel: Q-weighted scale + phase rotation on V
        out = dsqg_attention_v4(q, k, v, self.pos_bias, self.scale_embed,
                                self.phase_embed)
    ...
    def attn_summary(self):
        ...
        return {
            ...
            'phase_by_head':           ph.abs().mean(0).tolist(),
            'phase_sparse_by_head':    ph_sparse.mean(0).tolist(),  # signed: +→constructive, -→destructive
        }
...
class FullCausalAttention(nn.Module):
    ...
    def attn_summary(self):
        return {'type': 'full_causal',
                ...
                'phase_abs_mean': 0.0, 'phase_abs_max': 0.0,
                'phase_local_mean': 0.0, 'phase_sparse_mean': 0.0,
                'phase_by_head': [0.0] * NUM_HEADS,
                'phase_sparse_by_head': [0.0] * NUM_HEADS}
...
class CondMTransformer(nn.Module):
    ...
    def attn_summary(self):
        ...
        return {
            ...
            'phase_abs_mean':         sum(s['phase_abs_mean']       for s in summaries) / n,
            'phase_abs_max':          max(s['phase_abs_max']        for s in summaries),
            'phase_local_mean':       sum(s['phase_local_mean']     for s in summaries) / n,
            'phase_sparse_mean':      sum(s['phase_sparse_mean']    for s in summaries) / n,
            'phase_by_head':          [
                sum(s['phase_by_head'][h] for s in summaries) / n
                for h in range(NUM_HEADS)
            ],
            'phase_sparse_by_head':   [
                sum(s['phase_sparse_by_head'][h] for s in summaries) / n
                for h in range(NUM_HEADS)
            ],
        }
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # Phase logging — the key new diagnostic
        print(f'  phase:         |mean|={ss["phase_abs_mean"]:.4f} '
              f'|max|={ss["phase_abs_max"]:.4f} '
              f'local={ss["phase_local_mean"]:.4f} '
              f'sparse={ss["phase_sparse_mean"]:.4f}')
        sph = ss['phase_sparse_by_head']
        sph_str = '  '.join(f'h{h}:{sph[h]:+.3f}' for h in range(NUM_HEADS))
        print(f'  phase_sparse:  {sph_str}  (+ constructive / - destructive)')
        ...
        per_epoch_results.append({
            ...
            'if_gain': ss['if_gain'],
            'phase_abs_mean': ss['phase_abs_mean'],
            'phase_abs_max':  ss['phase_abs_max'],
            'phase_sparse_mean': ss['phase_sparse_mean'],
            'phase_sparse_by_head': ss['phase_sparse_by_head'],
        })
        sys.stdout.flush()
    ...
    print(f'\n  condU_phase_init TEST: PPL {test_ppl:.3f} | Loss {test_loss:.4f}')
    ...
    print(f'  Final scale_embed: |mean|={ss["scale_embed_abs_mean"]:.4f} '
          f'|max|={ss["scale_embed_abs_max"]:.4f}')
    sph = ss['phase_sparse_by_head']
    sph_str = '  '.join(f'h{h}:{sph[h]:+.4f}' for h in range(NUM_HEADS))
    print(f'  Final phase: |mean|={ss["phase_abs_mean"]:.4f}  |max|={ss["phase_abs_max"]:.4f}')
    print(f'  Final phase_sparse_by_head: {sph_str}')
    print(f'  (Hypothesis: global heads → positive, local heads → negative)')

    results = {
        'experiment':              'condU_phase_init',
        'kernel':                  'dsqg_attention_v4',
        'changes_vs_condU':        ['add_phase_embed_v4_givens_rotation_on_V',
                                    'phase_embed_randn_init_std0.01',
                                    'retain_scale_embed_v3',
                                    'retain_if_amplifier',
                                    'retain_huygens_kv'],
        'changes_vs_condU_phase':  ['phase_embed_init_zero->randn_std0.01'],
        'hypothesis':              'small random init breaks uniform-positive symmetry; '
                                   'allows heads to differentiate constructive vs '
                                   'destructive sparse-offset phase from step 1',
        'baseline_condU_phase':    {'ppl': 52.237, 'passkey': 0.383},
...
def main():
    ...
    print('=' * 70)
    print('  condU_phase_init -- Phase Givens Rotation, N(0,0.01) init (14M)')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}  '
              f'({torch.cuda.get_device_properties(0).total_memory/1e9:.1f} GB)')
    print(f'  Kernel: dsqg_attention_v4 (phase_embed [44,H], randn std=0.01 init)')
    print(f'  Changes vs condU_phase: phase_embed init zero -> randn(0, 0.01)')
    print(f'  Hypothesis: breaks uniform-positive symmetry; enables head differentiation')
    print(f'  Retained: scale_embed V3, Huygens K/V injection, IF amplifier')
    ...
    layer_types = ['FULL' if i == FULL_ATTN_LAYER else 'DSQG'
                   for i in range(NUM_LAYERS)]
    print(f'\ncondU_phase_init: {n_params:,} parameters')
