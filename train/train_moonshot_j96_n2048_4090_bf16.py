...
MAX_TRAIN_SEQS      = 120_000   # 100% Chinchilla/epoch for ~37M params: 37M×20=740M tok÷3ep÷2048=120K seqs/ep
SCALE_EMBED_INIT_VAL = 0.15   # slightly higher than Moonshot — helps percolation at J=96
SCALE_EMBED_LR_MULT  = 24.0   # J=96: 4× relay paths dilute SE gradient; bumped from 15.0 to compensate
...
VOCAB_SIZE     = 32000
BATCH_SIZE     = 16             # 32K vocab lm_head peak: BS=8 safe on 24GB VRAM
GRAD_ACCUM     = 8           # effective batch = 128
CE_CHUNK       = 512          # standard
MAX_SEQ_LEN    = 2048
MAX_VAL_SEQS   = 5_582        # val set from fineweb_edu_encoded_2048_v2.pt
...
CHECKPOINT_STRATEGY = os.getenv('DWARF_CKPT', 'every_other').lower()  # J=96 backward: checkpoint every other block
...
def train():
    ...
    print('=' * 70)
    print('  🚀 DWARF Moonshot J=96 N=2048 — D=512 H=8 hd=64 L=8 cold start')
    print('  FA@L2, preIF@L1, J=96 (se096@N=4096), N=2048, EMA_INIT=1/48=0.020833')
    print('=' * 70)
    ...
    best_ckpt_name = 'moonshot_j96_n2048_best.pt'
    ...
    # scale_embed uses constant LR (no cosine decay) — percolation threshold
    # requires sustained amplitude; cosine decay fights relay crystallization.
    # Group 0 = normal params (cosine), Group 1 = scale_embed (constant).
    def _lr_lambda(step, group_idx):
        if group_idx == 1:          # scale_embed: constant
            return 1.0
        # cosine decay for normal params
        return 0.5 * (1.0 + math.cos(math.pi * step / total_steps))
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer,
        lr_lambda=[lambda s: _lr_lambda(s, 0), lambda s: _lr_lambda(s, 1)])

    # Freeze scale_embed support: DWARF_FREEZE_SE=1
    # When set, scale_embed.requires_grad=False and LR zeroed — amplitude
    # stays pinned at percolation value; K/Q topology crystallizes freely.
    freeze_se = os.getenv('DWARF_FREEZE_SE', '0') == '1'
    if freeze_se:
        for p in scale_embed_params:
            p.requires_grad_(False)
        # zero out the scale_embed param group LR so optimizer ignores it
        optimizer.param_groups[1]['lr'] = 0.0
        print('  [FREEZE] scale_embed frozen (DWARF_FREEZE_SE=1)')

    # Resume support: DWARF_RESUME=path, DWARF_START_EPOCH=N
    ...
    if resume_path and os.path.isfile(resume_path):
        ...
        if 'model_state_dict' in ckpt:
            ...
            model.load_state_dict(ckpt, strict=False)  # old flat format
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        torch.save(resume_state,
                   os.path.join(CHECKPOINT_DIR, f'moonshot_j96_n2048_ep{epoch}_resume.pt'))
    ...
    print(f'description:    Moonshot J=96 N=2048 — D=512 H=8 L=8 FFN=1024 '
          f'J=96 se096@N4096, N=2048, cold start, fineweb_edu_encoded_2048_v2, '
