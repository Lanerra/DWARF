"""
🚀 DWARF Depth-16 — 67M params, depth ablation, 4090, cold-start

Architecture: D=512, H=8 (hd=64), L=16, FFN=1024, J=24 (se015 offsets)
  L0:   DSQGBlockV6Physics  IF=False  ← pure DSQG relay
  L1:   DSQGBlockV6Physics  IF=True   ← preIF (single layer before FA)
  L2:   FullAttentionBlock            ← FA@L2 (same position as moonshot)
  L3-15: DSQGBlockV6Physics IF=False  ← 13 post-FA relay layers (vs 5 in moonshot)

Hypothesis: More relay depth → better long-range compositionality.
Baseline: Moonshot-58M ep2 (PPL=35.04, passkey=99.2%, ar_score=80.90)
          FFN ablation ep1  (PPL=38.18, passkey=93.3%) — FFN=1024 confirmed viable
...
Config:
  ...
  - ~67.5M parameters (untied lm_head)

Key differences vs moonshot-58M:
  - L=16 vs L=8 (doubled depth, 13 post-FA relay layers vs 5)
  - FFN=1024 vs FFN=2048 (halved, confirmed by ablation to not hurt PPL)
  - BS=64 GRAD_ACCUM=2 vs BS=16 GRAD_ACCUM=8 (same eff_batch=128, better GPU util)
  - Untied lm_head (embedding 16.4M + lm_head 16.4M — dominant cost at this vocab)

Screen: 3 epochs × 121,232 seqs (same as autoresearch standard)

Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_depth16_67m_4090_bf16.py \\
    > logs/run_depth16_67m.log 2>&1 &
"""
...
FFN_DIM          = 1024       # 2×D — confirmed viable by FFN ablation (ep1: PPL=38.18 vs moonshot 41.48)
NUM_LAYERS       = 16
FULL_ATTN_LAYER  = 2          # same FA placement as moonshot

MAX_TRAIN_SEQS       = 121_232   # standard 3-epoch screen (autoresearch budget)
...
VOCAB_SIZE     = 32000
BATCH_SIZE     = 16   # L=16 has 2× activation memory vs L=8 moonshot
GRAD_ACCUM     = 8    # effective batch = 128 (same as moonshot)
...
class AutoresearchTransformerPhysics(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads,
        ...
        # Untied lm_head — embedding table is the dominant cost at V=32K
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":       FFN_DIM,
            "seq_len":       MAX_SEQ_LEN,
            "source_script": "train/train_depth16_67m_4090_bf16.py",
            ...
            "git_hash":      git_hash,
            "note": (
                f"Depth16-67M: D={EMBEDDING_DIM} H={NUM_HEADS} L={NUM_LAYERS} "
                f"FFN={FFN_DIM} J={len(OFFSETS)} FA@L{FULL_ATTN_LAYER} "
                f"preIF@L{FULL_ATTN_LAYER-1}. Epoch {epoch}/3. Cold start."
            ),
        },
    }
    out_path = os.path.join(checkpoint_dir, f"depth16_67m_ep{epoch}_full_attn.pt")
...
def train():
    ...
    print('=' * 70)
    print('  🚀 DWARF Depth-16 — D=512 H=8 hd=64 L=16 FFN=1024 J=24, cold start')
    print('  FA@L2, preIF@L1, 13 post-FA relay layers, fineweb_tokenizer_32k')
    print('=' * 70)
    if torch.cuda.is_available():
    ...
    print(f'Warming up Triton kernels (BS={BATCH_SIZE} dummy pass)...')
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(clean_state,
                       os.path.join(CHECKPOINT_DIR, 'depth16_67m_best.pt'))
            marker = ' *'
        ...
        torch.save({
            ...
            'val_loss': val_loss,
        }, os.path.join(CHECKPOINT_DIR, f'depth16_67m_ep{epoch}_resume.pt'))
    ...
    print(f'description: Depth16-67M D={EMBEDDING_DIM} H={NUM_HEADS} hd=64 L={NUM_LAYERS} '
