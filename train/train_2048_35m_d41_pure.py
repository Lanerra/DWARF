...
Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u benchmarks/train_2048_condV.py \
    2>&1 | tee benchmarks/logs/d41_35m_run.log
"""
...
MAX_TRAIN_SEQS  = None      # computed dynamically → Chinchilla at ep 7

# -- Passkey eval config -------------------------------------------------------

PASSKEY_DISTANCES = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 1536]
PASSKEY_TRIALS    = 20   # 20 probes → 5% granularity per distance
...
SAVE_DIR    = 'checkpoints/d41_35m_pure'
RESULT_FILE = 'benchmarks/logs/d41_35m_pure_results.json'
...
from dsqg_attention_d41_35m import dsqg_attention_v3
...
class DSQGAttentionQW(nn.Module):
    ...
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        self.scale_embed = nn.Parameter(torch.zeros(52, HD))  # zero init: safe baseline for first 35M run; validate non-zero init separately
...
def _causal_ema(xi: torch.Tensor, ema_factor: torch.Tensor) -> torch.Tensor:
    ...
    ema_factor: scalar parameter, clamped to [0.001, 0.5] in forward
    Returns: [B, N, D]
    """
    B, N, D = xi.shape
    alpha   = ema_factor.clamp(0.001, 0.5)  # lowered floor: allows window up to ~1000 tokens
...
class DSQGBlock(nn.Module):
    ...
    def __init__(self, embedding_dim, num_heads, ffn_dim, seq_len,
        ...
        if interference:
            ...
            self.ema_factor   = nn.Parameter(torch.full((1,), 0.03))   # original init: narrow window (≈33t) bootstraps short-range passkey first
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for acc_step in range(steps_per_epoch):
            ...
            # ── gradient norm + param value diagnostics (every 10 steps until 500) ──
            if step < 500 and step % 10 == 0:
                gn = {}
                for name, p in model.named_parameters():
                    if p.grad is not None:
                        key = name.split('.')[-2] if '.' in name else name
                        gn[key] = max(gn.get(key, 0.0), p.grad.detach().float().norm().item())
                top = sorted(gn.items(), key=lambda x: -x[1])[:4]
                gn_str = '  '.join(f'{k}:{v:.3f}' for k,v in top)
                # collect key parameter values
                pv = {}
                for name, p in model.named_parameters():
                    if any(t in name for t in ('pos_bias','scale_embed','if_gain','ema_factor','kdv_alpha')):
                        pv[name.split('.')[-1]] = p.detach().float().abs().max().item()
                pv_str = '  '.join(f'{k}:{v:.4f}' for k,v in sorted(pv.items(), key=lambda x: -x[1])[:4])
                print(f'  [step {step:4d}] gn={gn_str}  pv={pv_str}')
        ...
        if ss['ema_factors']:
            ema_str = '  '.join(f'b{i}:{v:.4f} (w≈{1/max(v,1e-6):.0f}t)' for i, v in enumerate(ss['ema_factors']))
    ...
    results = {
        'experiment':              'd41_35m_pure_dsqg',
        'kernel':                  'dsqg_attention_v3',
        'changes':                 ['pure_dsqg_no_full_attn', 'dense_w48_sparse96_128_384_J52', 'scale_embed_init_zero', 'ema_clamp_floor_0.001'],
        ...
        'd41s3_reference_ppl':      52.457,
        'd41s3_reference_passkey':  0.767,  # training-log passkey
...
def main():
    ...
    print('=' * 70)
    print('  d41_35m_pure -- PURE DSQG (no full attention), D=512, dense=48, sparse=[96,128,384]')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}  '
              f'({torch.cuda.get_device_properties(0).total_memory/1e9:.1f} GB)')
    print(f'  Kernel: dsqg_attention_v3')
    print(f'  Changes vs d41_35m: FULL_ATTN_LAYER=-1 (all 6 layers are DSQG)')
    print(f'  Physics: Kalman-EMA + KdV soliton + AGC + scale_embed + IF amplifier + Huygens K/V')
    print(f'  Architecture: INTERFERENCE=3, gate=0, NO full attention')
    print(f'  References: condU_35M_pure=48.271/33.3% | d41_35m_hybrid=32.136/94.2%')
    ...
    layer_types = ['FULL' if i == FULL_ATTN_LAYER else 'DSQG'
                   for i in range(NUM_LAYERS)]
    print(f'\nd41_35m_pure: {n_params:,} parameters')
