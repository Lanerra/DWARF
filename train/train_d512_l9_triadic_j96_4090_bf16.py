"""
🚀 DWARF D=512 L=9 Triadic J=96 — offset partitioning, 4090, cold start

Architecture: D=512, H=8 (hd=64), L=9, FFN=1024, TIED lm_head
  ...
  L7: DSQGBlock(GROUP_T)   -- tail (48 long-range + 3 local anchors, J=51)
  L8: DSQGBlock(GROUP_T)

  Full J=96 topological coverage achieved within each 3-layer triad.

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_d512_l9_triadic_j96_4090_bf16.py
"""
...
MAX_TRAIN_SEQS = int(os.environ.get('DWARF_MAX_TRAIN_SEQS', '120000'))
MAX_SEQ_LEN    = 2048
MAX_VAL_SEQS   = 5_582
CE_CHUNK       = 512
SCREEN_EPOCHS  = 3
...
CKPT_BASE_NAME    = 'd512_l9_triadic_j96'

CHECKPOINT_STRATEGY = os.getenv('DWARF_CKPT', 'none').lower()

# =============================================================================
# LAYER LAYOUT: L=9, FA@L3
# =============================================================================
...
LAYER_LAYOUT = [
    ...
    ('T', GROUP_T, J_SMALL_T, J_LARGE_T, False),   # L7: tail
    ('T', GROUP_T, J_SMALL_T, J_LARGE_T, False),   # L8: tail
]
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "source_script": "train/train_d512_l9_triadic_j96_4090_bf16.py",
            "source_layer": FULL_ATTN_LAYER, "num_layers": NUM_LAYERS,
            "epoch": epoch, "git_hash": git_hash,
            "note": (f"Triadic J=96 L=9: D={EMBEDDING_DIM} H={NUM_HEADS} "
...
def train():
    ...
    print(f'description: Triadic J=96 L=9 D={EMBEDDING_DIM} H={NUM_HEADS} '
