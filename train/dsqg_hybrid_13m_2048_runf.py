"""
Run F — DSQG Hybrid 13M @ N=2048, Dense 1-15 + GAP 16-31 + Octave Blocks 32-512
==================================================================================

What changes vs Run E (plateau at ~20% passkey):
  - Offset set: dense 1-15 + GAP 16-31 + octave blocks [32-39][64-71]
                [128-135][256-263][512-519] + gap/tail (J=63)
  - pos_bias init: NEAR-ZERO (zeros) — same as Run D and Run E
  - All other hyperparameters identical to Run E

Key insight from Run E: "distraction by density"
  Dense 1-39 taught DSQG smooth local attention (good PPL) but prevented hard
  commitment to specific retrieval offsets. The passkey signal was washed out
  because the model never needed to commit to any single offset — 40 nearby
  offsets all gave equally good loss.

  Run F removes the 16-31 range, creating a "cliff edge" at δ=31→32.
  The model is forced to use [32-39] for d=32 retrieval — no soft alternative
  at δ=16-31. This is the same structure that drove Run C's rapid d=32
  commitment (60% by ep5), now extended to all 5 octave distances.

Offset design (J=63 including δ=0):
  Base:        {0}                               (1)
  Dense 1-15:  {1,2,...,15}                     (15) — NL + δ_eff for d=1-8
  GAP 16-31:   (empty — commitment pressure zone) (0)
  Block d≈32:  {32-39}                            (8) — δ_eff for d=32, cliff edge
  {48}:        combination-tone anchor             (1)
  Block d≈64:  {64-71}                            (8) — δ_eff for d=64
  {96}:        combination-tone anchor             (1)
  Block d≈128: {128-135}                          (8) — δ_eff for d=128
  {192}:       combination-tone anchor             (1)
  Block d≈256: {256-263}                          (8) — δ_eff for d=256
  {384}:       combination-tone anchor             (1)
  Block d≈512: {512-519}                          (8) — δ_eff for d=512
  Tail:        {768, 1024, 1536}                  (3)

Rust verification (Test 9: run_f_analysis) — ALL ASSERTIONS PASS:
  - Short-range density = Run C (15 offsets, dense 1-15)
  - Gap 16-31 confirmed empty
  - δ_eff direct for d=32,64,128,256,512
  - d=16 multi-hop only (same as Run C — acceptable)
  - RF path-count: 6.91× Run C, 4.83× Run D, 207× Run C at d=1536

Predictions:
  PPL:     ~52-53 (dense 1-15 matches Run C's NL power exactly)
  d=1-8:   δ_eff in dense 1-15 → signal from ep2
  d=16:    δ_eff=23 in gap → multi-hop only (minor weakness, same as Run C)
  d=32:    cliff edge forces commitment → 60%+ by ep5 (Run C rate)
  d=64-512: octave blocks → 60%+ by ep7 (Run D rate)
  d=1024+: multi-hop synthesis (proven in Run D: 60%)
  Speed:   J=63 vs Run E J=79 → ~20% faster per epoch (~410s vs 510s)

Kernel: kernels/dsqg_attention_v3_runf.py (J=63, max δ=1536)

Smoke test kernel before first run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 kernels/dsqg_attention_v3_runf.py

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/dsqg_hybrid_13m_2048_runf.py \\
    2>&1 | tee benchmarks/logs/dsqg_hybrid_13m_2048_runf_run.log
"""
...
# J=63: dense 1-15 + GAP 16-31 + octave blocks 32-512
_BASE           = [0]
_DENSE          = list(range(1, 16))     # dense 1-15: NL + δ_eff for d=1-8
# GAP 16-31: intentionally empty
_D32            = list(range(32, 40))    # cliff-edge block: δ_eff for d=32
_D64            = list(range(64, 72))    # octave block: δ_eff for d=64
_D128           = list(range(128, 136))  # octave block: δ_eff for d=128
_D256           = list(range(256, 264))  # octave block: δ_eff for d=256
_D512           = list(range(512, 520))  # octave block: δ_eff for d=512
_TAIL           = [768, 1024, 1536]
_RUNF_OFFSETS   = sorted(_BASE + _DENSE + _D32 + [48] + _D64 + [96] +
                          _D128 + [192] + _D256 + [384] + _D512 + _TAIL)
NUM_OFFSETS     = len(_RUNF_OFFSETS)     # 63
assert NUM_OFFSETS == 63, f"Expected 63, got {NUM_OFFSETS}"
# Sanity: gap 16-31 must be empty
assert all(x not in _RUNF_OFFSETS for x in range(16, 32)), "Gap 16-31 must be empty"

MAX_TRAIN_SEQS = 52_716   # iso-compute with condU 13M N=2048 baseline
...
SAVE_DIR    = 'checkpoints/2048_dsqg_hybrid_13m_runf'
RESULT_FILE = 'benchmarks/logs/dsqg_hybrid_13m_2048_runf_results.json'

import pathlib as _pl
_kernel_dir = str(_pl.Path(__file__).parent.parent / 'kernels')
if _kernel_dir not in sys.path:
    sys.path.insert(0, _kernel_dir)
from dsqg_attention_v3_runf import dsqg_attention_v3_runf

def dsqg_attention_backend(q, k, v, pos_bias, scale_embed):
    return dsqg_attention_v3_runf(q, k, v, pos_bias, scale_embed)
...
class DSQGAttentionQW(nn.Module):
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        # Run F: near-zero init — all heads start equal; specialization from gradient dynamics.
        # Same as Run D and Run E. The cliff edge at δ=31→32 (not init) drives commitment.
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
    ...
    print(f'\n  Run F (dense 1-15 + gap 16-31 + octave blocks) TEST PPL: {test_ppl:.3f}')
    print(f'  condU 13M 2048 baseline:   52.206 PPL  delta={test_ppl-52.206:+.3f}')
    print(f'  Run C (interleaved J=40):  52.857 PPL  delta={test_ppl-52.857:+.3f}')
    print(f'  Run D (octave J=53):       55.488 PPL  delta={test_ppl-55.488:+.3f}')
    print(f'  Run E (dense 1-39 J=79):   ~52   PPL  (in progress)')
    ...
    print(f'  Run C pk:                     45.0%')
    print(f'  Run D pk:                     35.0%')

    results = {
        'experiment':      'run_f_dsqg_hybrid_13m_2048_runf',
        'description':     'Dense 1-15 + GAP 16-31 + octave blocks 32-512 (J=63); N=2048; near-zero init',
        'offset_set':      _RUNF_OFFSETS,
        'offset_design':   'dense 1-15 | GAP 16-31 | dense 32-39 | 48 | dense 64-71 | 96 | dense 128-135 | 192 | dense 256-263 | 384 | dense 512-519 | 768 1024 1536',
        'change_vs_base':  'dense 1-15 + gap 16-31 + all-octave blocks (J=63) + near-zero pos_bias init',
        ...
        'refs': {
            ...
            'run_d_ppl':           55.488,
            'run_d_pk':            0.350,
            'run_e_ppl':           'in progress',
            'run_e_pk':            'in progress',
        },
    }
...
def main():
    ...
    print('=' * 70)
    print('  Run F — DSQG Hybrid 13M @ N=2048 — Dense 1-15 + GAP + Octave (J=63)')
    print(f'  D={EMBEDDING_DIM}, H={NUM_HEADS}, L={NUM_LAYERS}, N={MAX_SEQ_LEN}')
    print(f'  Offsets ({NUM_OFFSETS}): dense 1-15 | GAP 16-31 | blocks 32-39,64-71,128-135,256-263,512-519 | tail')
    print(f'  Init: near-zero pos_bias (cliff edge drives commitment, not init)')
    print('=' * 70)
