"""
🔬 DWARF No-FFN Scale-Up — D=512 L=16 FA@L4 FFN=0, 32K vocab, ~34M params

Scale-up from 16M (8K vocab, L=8) which showed relay formation (scale_embed
|max|=1.65 at ep1) but failed to cross percolation threshold. Hypothesis:
more depth (L=16, 15 DSQG layers + 1 FA) provides enough representational
capacity to activate relay without FFN.

Architecture: D=512, H=8 (hd=64), L=16, FFN=0, J=24 (se015 offsets)
  L0-L2:  DSQGBlockNoFFN  IF=False  ← pre-FA warm-up layers
  L3:     DSQGBlockNoFFN  IF=True   ← preIF
  L4:     FullAttentionBlockNoFFN   ← FA@L4 (25% depth), no FFN
  L5-L15: DSQGBlockNoFFN IF=False   ← 11 post-FA relay layers, no FFN

Config:
  - Tokenizer: results/fineweb_tokenizer_32k.json  (32K BPE, FineWeb proper)
  - Dataset:   logs/fineweb_edu_encoded_2048_v2.pt  (~2M seqs, 4.13B tokens)
  - EMA_INIT = 0.0208 (= 1/δ_relay_min = 1/48, se015 validated)
  - SCALE_EMBED_INIT = 0.1, LR_MULT = 15.0
  - Batch: BS=16 × GRAD_ACCUM=8 → eff_batch=128
  - Cold start, 3 epochs, 121K seqs (Chinchilla screen)
  - ~34.3M parameters

Baseline comparison:
  - Moonshot-58M FFN=2048:  35.04 PPL, 99.2% passkey  (ar_score = 0.0)
  - 16M no-FFN ep1: PPL 91.87, passkey 12.5%, scale_embed |max|=1.65 (sub-threshold)
  - condX-v2 35M (with FFN=2048): 38.17 PPL, 96.7% passkey — direct comparison target

EMA_INIT RULE: 1/δ_relay_min = 1/48 = 0.0208 (J24D se015 validated).

ar_score = (passkey_final - 99.2) + (35.04 - ppl_final) * 0.5
  Zero = ties Moonshot-58M; positive = beats it; negative = worse.

Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_no_ffn_34m_4090_bf16.py \\
    2>&1 | tee logs/run_no_ffn_34m.log
"""
...
FFN_DIM          = 0          # ← THE ABLATION: no FFN anywhere
NUM_LAYERS       = 16
FULL_ATTN_LAYER  = 4          # 25% depth (FA placement rule)

# Dataset: 32K-tokenized FineWeb-Edu, 2M+ seqs
# Chinchilla-optimal for ~34.3M params: 20 × 34.3M / 2048 = 335,156 seqs
# Screen run: 121,232 seqs = 36% Chinchilla (same budget as depth16_55m_fa4 screen)
...
BATCH_SIZE  = 16    # 4090: L=16 + gradient checkpointing + 32K vocab logits
GRAD_ACCUM  = 8     # eff_batch=128 preserved
MAX_SEQ_LEN = 2048
...
# ar_score baseline: Moonshot-58M ep2 (35.04 PPL, 99.2% passkey)
PPL_BASELINE     = 35.04
...
# =============================================================================
# Model — FFN-free variants
# =============================================================================

class DSQGBlockNoFFN(nn.Module):
    """DSQG attention block with NO FFN sublayer.
    Each block is: x = x + attn(norm1(x))
    That's it. No norm2, no FFN.
    """
    def __init__(self, embedding_dim, num_heads, seq_len,
    ...
    def forward(self, x):
        ...
        if self.interference:
            ...
            v_delta = inter.view(B, N, H, HD).permute(0, 2, 1, 3).contiguous()
...
class FullAttentionBlockNoFFN(nn.Module):
    """Full causal attention block with NO FFN sublayer.
    Each block is: x = x + attn(norm1(x))
    """
    def __init__(self, embedding_dim, num_heads, dropout=0.0):
        super().__init__()
...
class DWARFNoFFN(nn.Module):
    """DWARF architecture with FFN completely removed from all layers."""
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads,
                 seq_len, full_attn_layer, scale_embed_init_val=0.1):
        super().__init__()
        ...
        for i in range(num_layers):
            if i == full_attn_layer:
                blocks.append(FullAttentionBlockNoFFN(embedding_dim, num_heads))
            else:
                has_if = (i == full_attn_layer - 1)
                blocks.append(DSQGBlockNoFFN(
                    embedding_dim, num_heads, seq_len,
                    dropout=0.0, interference=has_if))
    ...
    def forward(self, idx):
        ...
        for i, block in enumerate(self.blocks):
            if self.training and i % 2 == 0:  # every_other checkpointing
    ...
    def physics_summary(self):
        entries = []
        for i, block in enumerate(self.blocks):
            if isinstance(block, DSQGBlockNoFFN) and block.interference:
...
def evaluate(model, data, device):
    ...
    bs = min(4, len(data))  # small batch to avoid 32K vocab logit OOM
...
def train(resume_from=None, start_epoch=1):
    ...
    print('=' * 70)
    print('  🔬 DWARF No-FFN Scale-Up — D=512 H=8 L=16 FFN=0, 32K vocab (~34M)')
    print('  Testing FFN-free relay at depth: FA@L4, 11 post-FA relay layers')
    if resume_from:
        print(f'  RESUMING from ep{start_epoch - 1} checkpoint: {resume_from}')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}')
        _cc   = torch.cuda.get_device_capability()
        _path = 'sm_89 (4090 Ada)' if (_cc[0]==8 and _cc[1]==9) else \
                'sm_90 (H100/H200)' if (_cc[0]==9) else f'sm_{_cc[0]}{_cc[1]}'
    ...
    print(f'  Dataset: {len(train_data):,} train / {len(val_data):,} val seqs')
    ...
    print(f'  D={EMBEDDING_DIM}, H={NUM_HEADS}, L={NUM_LAYERS}, FFN=NONE')
    ...
    steps_per_epoch_calc = math.ceil(len(train_data) / BATCH_SIZE / GRAD_ACCUM)
    total_steps = SCREEN_EPOCHS * steps_per_epoch_calc
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=total_steps)

    # Fast-forward scheduler if resuming
    if resume_from and start_epoch > 1:
        completed_steps = (start_epoch - 1) * steps_per_epoch_calc
        for _ in range(completed_steps):
            scheduler.step()
        print(f'  Scheduler fast-forwarded {completed_steps} steps (ep{start_epoch-1} complete)')
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(CHECKPOINT_DIR, 'no_ffn_34m_best.pt'))
            marker = ' *'

        print(f'Ep {epoch}/{SCREEN_EPOCHS} | Val PPL {val_ppl:.3f}{marker}')
        ...
        torch.save(model.state_dict(),
                   os.path.join(CHECKPOINT_DIR, f'no_ffn_34m_ep{epoch}_resume.pt'))
        print(f'  Saved resume checkpoint: no_ffn_34m_ep{epoch}_resume.pt')
    ...
    for ep in range(1, SCREEN_EPOCHS + 1):
        print(f'ppl_ep{ep}:        {ppl_results.get(ep, 999.0):.3f}')
    ...
    print(f'description: No-FFN scale-up — D={EMBEDDING_DIM} H={NUM_HEADS} L={NUM_LAYERS} '
          f'FFN=0 J=24 se015, FA@L{FULL_ATTN_LAYER}, fineweb_tokenizer_32k, EMA_INIT=1/δ_relay_min')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume-from', type=str, default=None,
                        help='Path to ep checkpoint to resume from')
    parser.add_argument('--start-epoch', type=int, default=1,
                        help='Epoch to start from (2 if resuming after ep1)')
