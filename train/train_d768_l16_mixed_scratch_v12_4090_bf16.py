...
Run from repo root:
  tmux new-session -d -s d768_l16_mixed_v12 \\
    ".venv/bin/python3 -u train/train_d768_l16_mixed_scratch_v12_4090_bf16.py \\
     2>&1 | tee logs/run_d768_l16_mixed_v12.log"
"""
...
MAX_TRAIN_SEQS       = 234_418   # all available ≈ 22% Chinchilla for ~111M params
...
_USE_V13 = os.getenv('DWARF_USE_V13', '0') == '1'
_USE_V12 = os.getenv('DWARF_USE_V12', '1') == '1'   # V12 on by default for this script
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":       FFN_DIM,
            "seq_len":       MAX_SEQ_LEN,
            "source_script": "train/train_d768_l16_mixed_scratch_v12_4090_bf16.py",
    ...
    out_path = os.path.join(checkpoint_dir, f"d768_l16_mixed_scratch_v12_ep{epoch}_full_attn.pt")
...
def train():
    ...
    if os.path.exists(_encoded_cache):
        ...
        raise FileNotFoundError(
            ...
            f'Expected: logs/mixed_encoded_2048_fineweb_tok.pt')
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(clean_state,
                       os.path.join(CHECKPOINT_DIR, 'd768_l16_mixed_scratch_v12_best.pt'))
            marker = ' *'
        ...
        torch.save({
            ...
            'val_loss': val_loss,
            'val_ppl':  val_ppl,
        }, os.path.join(CHECKPOINT_DIR, f'd768_l16_mixed_scratch_v12_ep{epoch}.pt'))
