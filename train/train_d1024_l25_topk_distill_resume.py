#!/usr/bin/env python3
"""
🔬 DWARF D=1024 L=25 TopK Sparse Attention — distillation from cached FA teacher
RESUME version: continue training from a saved checkpoint.
...
Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_d1024_l25_topk_distill_resume.py --resume-from 2
"""

import math, os, subprocess, sys, time, types, argparse
...
CKPT_NAME       = 'd1024_l25_topk_distill'
...
def train():
    ...
    parser.add_argument('--resume-from', type=int, default=0,
                        help='Resume from this epoch number (0 = fresh start)')
    ...
    print('=' * 70)
    print('  🔬 DWARF D1024-L25 TopK Distillation (cached FA teacher)')
    ...
    initial_epoch = resume_from + 1 if resuming else 1
    global_step = resume_from * steps_per_epoch if resuming else 0

    # ── Cache FA activations (skip if resuming — already cached) ──
    ...
    # ── Resume: load checkpoint state ──
    if resuming:
        ckpt_path = os.path.join(CHECKPOINT_DIR, f'{CKPT_NAME}_ep{resume_from}.pt')
        print(f'\n  📂 Loading checkpoint: {ckpt_path}')
        ckpt = torch.load(ckpt_path, map_location=device)
        state = ckpt.get('model_state_dict', ckpt)
        state = {k.replace('._orig_mod', ''): v for k, v in state.items()}
        missing2, unexpected2 = model.load_state_dict(state, strict=False)
        del ckpt, state
        print(f'  Resumed weights: {len(missing2)} missing, '
              f'{len(unexpected2)} unexpected')
        for k in missing2:
            print(f'    [missing] {k}')
    ...
    # ── Resume: load optimizer/scheduler state ──
    if resuming:
        ckpt_path = os.path.join(CHECKPOINT_DIR, f'{CKPT_NAME}_ep{resume_from}.pt')
        ckpt = torch.load(ckpt_path, map_location=device)
    ...
    print(f'\n{"=" * 70}')
    print('  🔬 TopK Distillation Summary (D1024-L25)')
    print(f'{"=" * 70}')
