"""
condU 35M — Q-Weighted Scale Gains, Scaling Validation (FineWeb-Edu)

Scales condU from 13M (D=256) to 35M (D=512) while preserving the full
condU physics stack:
...
planned condU 85M run. Allows:
  1. Scaling curve fit: 13M → 35M → 85M — does condU follow the same
     power-law as condM (13M=54.529, 27M=44.500, 85M=36.042)?
  2. Compare against condM 27M (44.500 PPL, 83.3% passkey):
     does condU's physics advantage persist at 35M scale?
  3. Check IF gain evolution at larger HD (50 vs 32): does the
     global>local differentiation strengthen with more head capacity?

ARCHITECTURE
  D=512, H=8, d_head=64, L=6, FFN=2048 (clean power-of-2 HD; ~35M params)
  ...
  ~35M parameters (D=512 is the smallest H=8 config with HD=64, a power-of-2)
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
    ...
    print(f'\n  condU 35M TEST: PPL {test_ppl:.3f} | Loss {test_loss:.4f}')
    ...
    results = {
        'experiment':              'condU_35m_qweighted_scale',
...
def main():
    ...
    print('=' * 70)
    print('  condU 35M — Q-Weighted Scale Gains + IF Amplifier + Huygens K/V')
