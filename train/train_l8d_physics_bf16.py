"""
🔬 DWARF Autoresearch — L8D Physics (First 8-Layer DWARF Run)

Pure depth test based on J24D physics BF16. Same D=512, H=8, FFN=2048.
Only changes: L=6→L=8 and interference structure.

Architecture:
  - NUM_LAYERS = 8 (was 6)
  - INTERFERENCE_INTERVAL = 4 (was 3)
  - FULL_ATTN_LAYER = 7 (was 5)

With L=8 and interference_interval=4:
  - IF blocks at layers 0 and 4 (where layer_idx % 4 == 0)
  - DSQG layers: 0(IF), 1, 2, 3, 4(IF), 5, 6, 7(FullAttn)
  - 2 IF blocks before full-attn, 2-layer staging gap (layers 5,6) before full-attn at 7

Run (from /tmp/dwarf-j17d):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u autoresearch/train_l8d_physics_bf16.py \
    > autoresearch/run_l8d_physics_bf16.log 2>&1 &
"""
...
CHECKPOINT_NAME = "l8d_physics_best.pt"
CHECKPOINT_DIR  = "/tmp/dwarf-j17d/autoresearch/checkpoints"
RESULTS_FILE    = "/tmp/dwarf-j17d/autoresearch/results/l8d_physics_bf16_results.json"
        ...
        for i in range(num_layers):
            if i == full_attn_layer:
                ...
                has_if = (interference_interval is not None and
                          i % interference_interval == 0)
                if has_if:
                    self.if_layers.append(i)
...
def train():
    ...
    if_layers = [i for i in range(NUM_LAYERS)
                 if i != FULL_ATTN_LAYER and i % INTERFERENCE_INTERVAL == 0]
    staging_layers = [i for i in range(NUM_LAYERS)
                      if i > max(if_layers) and i < FULL_ATTN_LAYER]

    print('=' * 70)
    print('  🔬 L8D Physics (first 8-layer DWARF run)')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}')
    print(f'  D={EMBEDDING_DIM}, H={NUM_HEADS}, L={NUM_LAYERS}, FFN={FFN_DIM}, V={VOCAB_SIZE}')
    print(f'  head_dim={head_dim}')
    print(f'  IF interval={INTERFERENCE_INTERVAL}, Full attn layer={FULL_ATTN_LAYER}')
    print(f'  IF blocks at: {if_layers}')
    print(f'  Staging layers before full-attn: {staging_layers}')
    ...
    print(f'  Params: {n_params / 1e6:.1f}M ({n_params:,})')
    ...
    description = (f"L8D physics — ~{n_params/1e6:.0f}M, D={EMBEDDING_DIM}/H={NUM_HEADS}/"
                   f"FFN={FFN_DIM}/L={NUM_LAYERS}, int={INTERFERENCE_INTERVAL} fa={FULL_ATTN_LAYER}, "
                   f"condV EMA+KdV+AGC, {MAX_TRAIN_SEQS} seqs")
    ...
    results = {
        ...
        'passkey_results': {str(k): v for k, v in passkey_results.items()},
        'ppl_results': {str(k): v for k, v in ppl_results.items()},
        'ar_score': ar_score,
        'memory_mb': memory_mb,
        'elapsed_s': elapsed_s,
        'ema_init': EMA_INIT,
        'ema_floor': EMA_FLOOR,
        'scale_embed_init': SCALE_EMBED_INIT_VAL,
