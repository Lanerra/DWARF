"""
🚀 DWARF D=2048 L=24 J=96 — ~970M params, H200/H100, cold start

Architecture: D=2048, H=32 (hd=64), L=24, FFN=4096, J=96 (se096 offsets), TIED lm_head
  L0-L4:  DSQGBlockJ96  IF=False  <- 5 pre-FA warm-up relay layers
  L5:     DSQGBlockJ96  IF=True   <- preIF@L5
  L6:     FullAttentionBlock      <- FA@L6 (L//4 = 6, 17 post-FA relay layers)
  L7-L23: DSQGBlockJ96  IF=False  <- 17 post-FA relay layers

Derived quantities:
  hd = 2048/32 = 64
  Relay/residual ratio = (J=96 x hd=64) / D=2048 = 3.0x
  LR_MULT = 15 x sqrt(2048/512) = 15 x 2 = 30.0
  EMA_INIT = 1/48 = 0.020833 (delta_relay_min=48 for se096)

Chinchilla: ~970M x 20 = 19.4B tokens optimal. At N=2048:
  19.4B / 2048 = ~9.5M sequences for full Chinchilla.
  22% Chinchilla = ~2.1M sequences.
  DWARF_MAX_TRAIN_SEQS default 50000 is intentionally a smoke test.

Kernel: dsqg_attention_v8_dynamic_j.py (J=96 dynamic kernel)

Run (from repo root, on H200):
  .venv/bin/python3 -u train/train_d2048_l24_j96_h200_bf16.py \\
    2>&1 | tee logs/run_d2048_l24_j96.log

Smoke test:
  CUDA_VISIBLE_DEVICES=0 DWARF_MAX_TRAIN_SEQS=200 DWARF_BS=2 DWARF_GA=4 \\
    .venv/bin/python3 -u train/train_d2048_l24_j96_h200_bf16.py
"""
...
EMBEDDING_DIM    = 2048
NUM_HEADS        = 32           # hd = 2048/32 = 64
FFN_DIM          = 4096         # 2xD
NUM_LAYERS       = 24
FULL_ATTN_LAYER  = 6            # L//4 = 6; preIF@L5; 17 post-FA relay layers

SCALE_EMBED_INIT_VAL = 0.15
SCALE_EMBED_LR_MULT  = 30.0    # 15 x sqrt(2048/512) = 15 x 2 = 30

EMA_INIT  = 0.020833            # 1/48, delta_relay_min=48 for se096
EMA_FLOOR = 0.00001

LR            = 2e-4
SCREEN_EPOCHS = 3
VOCAB_SIZE    = 32768           # 32K BPE
MAX_SEQ_LEN   = 2048
...
MAX_TRAIN_SEQS = int(os.environ.get('DWARF_MAX_TRAIN_SEQS', '50000'))
CE_CHUNK       = int(os.environ.get('DWARF_CE_CHUNK', '512'))
MAX_VAL_SEQS   = 5_582
...
from dsqg_attention_v8_dynamic_j import DSQGAttentionV8_H100 as DSQGJ96
...
class D2048L24J96Transformer(nn.Module):
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":       FFN_DIM,
            "seq_len":       MAX_SEQ_LEN,
            "source_script": "train/train_d2048_l24_j96_h200_bf16.py",
            ...
            "git_hash":      git_hash,
            "note": (
                f"D2048-L24-J96: D={EMBEDDING_DIM} H={NUM_HEADS} L={NUM_LAYERS} "
                f"FFN={FFN_DIM} J=96 FA@L{FULL_ATTN_LAYER} "
                f"preIF@L{FULL_ATTN_LAYER-1}. Epoch {epoch}/{SCREEN_EPOCHS}. Cold start."
            ),
        },
    }
    out_path = os.path.join(checkpoint_dir, f"{CHECKPOINT_BASE}_ep{epoch}_full_attn.pt")
...
def train():
    ...
    print('=' * 70)
    print(f'  🚀 DWARF D2048-L24-J96 — D={EMBEDDING_DIM} H={NUM_HEADS} hd=64 '
          f'L={NUM_LAYERS} FFN={FFN_DIM} J=96, cold start')
    ...
    if 'expandable_segments' not in alloc_conf:
        print('  HINT: export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True')
    if USE_LIGER_CE:
    ...
    print(f'  EMA alpha_0={EMA_INIT} (window~{round(1/EMA_INIT)}t)')
    print(f'  MAX_TRAIN_SEQS={MAX_TRAIN_SEQS:,}, Epochs={SCREEN_EPOCHS}')
    print(f'  Batch: BS={BATCH_SIZE} x GRAD_ACCUM={GRAD_ACCUM} = '
          f'eff_batch={BATCH_SIZE*GRAD_ACCUM}')
    print(f'  Gradient checkpointing: every layer (D=2048 VRAM tight)')
    print(f'  CE chunk size: {CE_CHUNK}')
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(clean_state,
                       os.path.join(CHECKPOINT_DIR, f'{CHECKPOINT_BASE}_best.pt'))
            marker = ' *'
        ...
        torch.save({
            ...
            'val_loss': val_loss,
            'val_ppl':  val_ppl,
        }, os.path.join(CHECKPOINT_DIR, f'{CHECKPOINT_BASE}_ep{epoch}_resume.pt'))
    ...
    print(f'num_offsets: 96')
    print(f'scale_embed_lr_mult: {SCALE_EMBED_LR_MULT}')
    print(f'ema_init: {EMA_INIT}')
    print(f'description: D2048-L24-J96 D={EMBEDDING_DIM} H={NUM_HEADS} hd=64 '
          f'L={NUM_LAYERS} FFN={FFN_DIM} J=96 se096, cold start, '
