"""
🚀 DWARF D=768 L=25 Pure DSQG SO(2) — no FA layer, 4090, cold start
...
Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_d768_l25_pure_dsqg_so2_4090_bf16.py
"""
...
EMBEDDING_DIM    = 768
NUM_HEADS        = 12   # hd = 768/12 = 64 (hd>=64 required for relay formation)
FFN_DIM          = 1536  # 2xD
NUM_LAYERS       = 25
# No FA layer — pure DSQG ablation (D=768/L=25 scale)
# Bandwidth ratio: (32 x 64) / 768 = 2.67x  (safe zone 1.5-3.0x)
# Coherence budget: D/35 ~ 22 layers; L=25 provides 3 layers slack
VOCAB_SIZE       = 32768

SCALE_EMBED_INIT_VAL = 0.15
# muP: 15 * sqrt(768/512) = 18.37; bump to 24.0 for mixed-domain gradient dilution
# D=768 mixed-domain validated at 24.0 in hybrid [A,A,B,B,C,C] runs
SCALE_EMBED_LR_MULT  = 24.0
EMA_INIT  = 0.020833  # 1/48; delta_relay_min=48 in se015
EMA_FLOOR = 0.00001
LR        = 3e-4
DROPOUT   = 0.1

# D=768/L=25: ~160M params, mixed corpus 234K seqs
# ~66% Chinchilla/epoch, 3 epochs = ~198% total
BATCH_SIZE     = int(os.environ.get('DWARF_BS', '16'))
GRAD_ACCUM     = int(os.environ.get('DWARF_GA', '8'))  # eff_batch=128
MAX_TRAIN_SEQS = int(os.environ.get('DWARF_MAX_TRAIN_SEQS', '234000'))  # full mixed corpus
MAX_SEQ_LEN    = 2048
MAX_VAL_SEQS   = 5_582
CE_CHUNK       = 512
SCREEN_EPOCHS  = 3
...
CKPT_BASE_NAME    = 'd768_l25_pure_dsqg_so2'
...
LAYER_LAYOUT = [
    # Pre-'FA' section: [A,A,B,B,C+preIF,C+preIF]
    # L4 = C+preIF (pre-FA relay priming, same as hybrid L5)
    # L5 = C+preIF (replaces FA@L6 from hybrid; second preIF for content anchoring)
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L0
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L1
    ('B', GROUP_B, J_SMALL_B, J_LARGE_B, False),   # L2
    ('B', GROUP_B, J_SMALL_B, J_LARGE_B, False),   # L3
    ('C', GROUP_C, J_SMALL_C, J_LARGE_C, True),    # L4 + preIF
    ('C', GROUP_C, J_SMALL_C, J_LARGE_C, True),    # L5 + preIF (was FA@L6)
    # Post-'FA' section: 3x [A,A,B,B,C,C] cycles + tail A
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L6: cycle 1
    ...
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L12: cycle 2
    ...
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L18: cycle 3
...
class TriadicJ96(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_heads, ffn_dim, seq_len,
        ...
        self.persistent_block = DSQGPersistentBlock(
            num_layers=len(LAYER_LAYOUT),
            ...
            checkpoint_every=1,  # checkpoint every layer = minimal activation memory
            ema_init=EMA_INIT,
        )
        # Expose as self.blocks for compatibility with param-group / logging code
        self.blocks = [self.persistent_block]
    ...
    def forward(self, idx):
        ...
        x = self.persistent_block(x)  # checkpoint_every=1 handles activation memory
        return self.out(self.norm(x))

    def forward_hidden(self, idx):
        B, N = idx.shape
        x = self.drop(self.embedding(idx))
        x = self.persistent_block(x)
        return self.norm(x)
    ...
    def scale_embed_parameters(self):
        # DSQGPersistentBlock stores scale_embed as a ParameterList
        yield from self.persistent_block.scale_embed_list.parameters()
    ...
    def physics_summary(self):
        entries = []
        pb = self.persistent_block
        for i, (label, offsets, js, jl, has_if) in enumerate(LAYER_LAYOUT):
            if has_if:
                pif = pb.preif_modules[str(i)]
                alpha = abs(pif.ema_factor.item()) + EMA_FLOOR
                win = round(1.0 / max(alpha, EMA_FLOOR))
                entries.append(f'b{i}[{label}]: α={alpha:.4f}(w≈{win}t)')
        return '  '.join(entries)
...
def save_checkpoint_weights(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "source_script": "train/train_d768_l25_pure_dsqg_so2_4090_bf16.py",
            "num_layers": NUM_LAYERS,
            "epoch": epoch, "git_hash": git_hash,
            "note": (f"Pure DSQG J=96 L=25: D={EMBEDDING_DIM} H={NUM_HEADS}. "
...
def train():
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        for acc_step in range(steps_per_epoch):
            ...
            if step % 100 == 0:
                se_vals = list(model.scale_embed_parameters())
                se_max = torch.cat([p.detach().abs().flatten() for p in se_vals]).max().item() if se_vals else 0.0
        ...
        se_vals = list(model.scale_embed_parameters())
        if se_vals:
            se_all = torch.cat([p.detach().abs().flatten() for p in se_vals])
    ...
    print(f'description: Pure DSQG J=96 L=25 D={EMBEDDING_DIM} H={NUM_HEADS} '
