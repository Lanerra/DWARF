"""
🚀 DWARF Depth-16 FA@L10 — 55M params, FA placement ablation, 4090, cold-start

Architecture: D=512, H=8 (hd=64), L=16, FFN=1024, J=24 (se015 offsets), TIED lm_head
  L0-L8:  DSQGBlockV6Physics  IF=False  ← 9 pre-FA warm-up relay layers
  L9:     DSQGBlockV6Physics  IF=True   ← preIF (single layer before FA)
  L10:    FullAttentionBlock            ← FA@L10 (5 post-FA relay layers, same as moonshot)
  L11-15: DSQGBlockV6Physics  IF=False  ← 5 post-FA relay layers

Hypothesis: post-FA relay depth is the key variable. Moonshot-58M (FA@L2, L=8) had 5
post-FA relay layers and got 99.2% passkey. Depth-16 FA@L2 had 13 post-FA layers and
got 19.2%. Keeping 5 post-FA layers at L=16 (FA@L10) tests whether that was the cause.

Baseline: Moonshot-58M ep2 (PPL=35.04, passkey=99.2%, ar_score=80.90)
          Depth-16 FA@L2 ep3 (PPL=56.22, passkey=19.2%, ar_score=0.87) ← what we're fixing

Changes vs train_depth16_67m_4090_bf16.py:
  - FULL_ATTN_LAYER: 2 → 10  (5 post-FA relay layers, matching moonshot)
  - Tied lm_head:   saves ~16.4M params (55M vs 71M)
  - Everything else identical (same dataset, tokenizer, offsets, physics)
...
Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_depth16_55m_fa10_4090_bf16.py \\
    > logs/run_depth16_55m_fa10.log 2>&1 &
"""
...
NUM_LAYERS       = 16
FULL_ATTN_LAYER  = 10         # 5 post-FA relay layers (L11-15), same count as moonshot-58M
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":       FFN_DIM,
            "seq_len":       MAX_SEQ_LEN,
            "source_script": "train/train_depth16_55m_fa10_4090_bf16.py",
            ...
            "git_hash":      git_hash,
            "note": (
                f"Depth16-55M-FA10: D={EMBEDDING_DIM} H={NUM_HEADS} L={NUM_LAYERS} "
                f"FFN={FFN_DIM} J={len(OFFSETS)} FA@L{FULL_ATTN_LAYER} "
                f"preIF@L{FULL_ATTN_LAYER-1}. Epoch {epoch}/3. Cold start."
            ),
        },
    }
    out_path = os.path.join(checkpoint_dir, f"depth16_55m_fa10_ep{epoch}_full_attn.pt")
...
def train():
    ...
    print('=' * 70)
    print('  🚀 DWARF Depth-16 FA@L10 — D=512 H=8 hd=64 L=16 FFN=1024 J=24, cold start')
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(clean_state,
                       os.path.join(CHECKPOINT_DIR, 'depth16_55m_fa10_best.pt'))
            marker = ' *'
        ...
        torch.save({
            ...
            'val_loss': val_loss,
        }, os.path.join(CHECKPOINT_DIR, f'depth16_55m_fa10_ep{epoch}_resume.pt'))
    ...
    print(f'description: Depth16-55M-FA10 D={EMBEDDING_DIM} H={NUM_HEADS} hd=64 L={NUM_LAYERS} '
