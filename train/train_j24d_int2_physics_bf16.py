"""
🔬 DWARF Autoresearch — J24D Full-Dataset + Physics

Builds on the J20D physics result. Extends to V8 kernel with J=24 offsets
(finer local coverage: δ=5,7,9,10 added → J_SMALL=14, J_LARGE=10).
...
Key changes from J20D:
  - V8 kernel (J=24) vs V7 kernel (J=20)
...
Offsets:
  ...
  J_SMALL=14 (indices 0-13, δ≤21, no MOVT)
  J_LARGE=10 (indices 14-23, δ≥23, MOVT applied)

Run (from /tmp/dwarf-j17d):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u autoresearch/train_j24d_physics_bf16.py \
    > autoresearch/run_j24d_physics_bf16.log 2>&1 &
"""
...
def train():
    ...
    print('=' * 70)
    print('  🔬 DWARF Autoresearch — J24D Full-Dataset + Physics')
    print('  V8 kernel (J=24) + condV: Kalman-EMA (α₀=0.003) + KdV + AGC')
    print('=' * 70)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(CHECKPOINT_DIR,
                                    f'{git_hash}_j24d_int2_physics_best.pt'))
            marker = ' *'
    ...
    print(f'description:    J24D-int2 physics — {n_params/1e6:.1f}M, V8+condV, '
