"""
🔬 DWARF Autoresearch — condDelta: Delta Rule DSQG (V9 kernel)

Builds on V8 (J=24 relay-optimal offsets + MOVT + QK-OVT + NPCI) and adds
per-offset learned delta_beta correction strength.

Delta rule: When beta>0, the gather step applies a content-driven correction
that pushes output toward highest-attended V rather than averaging:
    acc += beta * p * (v_target - acc_normalized)

Prior art: GatedDeltaNet (arXiv:2412.06464, ICLR 2025),
           Fast Weight Programmers (arXiv:2102.11174)

Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_cond_delta_bf16.py \
    > logs/run_cond_delta.log 2>&1 &
"""
...
class DSQGBlockV9Physics(nn.Module):
    """V9 DSQG attention (delta rule) + condV interference (EMA + KdV + AGC)."""
    def __init__(self, embedding_dim, num_heads, ffn_dim, seq_len,
        ...
        self.attn  = DSQGAttentionV9(embedding_dim, num_heads,
...
class AutoresearchTransformerCondDelta(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads,
        ...
        for i in range(num_layers):
            if i == full_attn_layer:
                ...
                blocks.append(DSQGBlockV9Physics(
    ...
    def _init_weights(self, scale_embed_init_val):
        ...
        for m in self.modules():
            if isinstance(m, DSQGAttentionV9):
    ...
    def physics_summary(self):
        ...
        for i, block in enumerate(self.blocks):
            if isinstance(block, DSQGBlockV9Physics) and block.interference:
    ...
    def delta_beta_summary(self):
        """Log delta_beta stats for all V9 attention layers."""
        entries = []
        for i, block in enumerate(self.blocks):
            if isinstance(block, DSQGBlockV9Physics):
                summary = block.attn.attn_summary()
                db_mean = summary.get('delta_beta_abs_mean', 0.0)
                db_top3 = summary.get('delta_beta_top3', [])
                top3_str = ', '.join([f'δ{d}:{v:.3f}' for d, v in db_top3])
                entries.append(f'L{i}: |β|={db_mean:.4f} top3=[{top3_str}]')
        return '  '.join(entries)
...
def train():
    ...
    print('=' * 70)
    print('  🔬 DWARF Autoresearch — condDelta: Delta Rule DSQG (V9 kernel)')
    print('  V9 kernel (J=24) + condV: Kalman-EMA + KdV + AGC + Delta Rule')
    print('=' * 70)
    ...
    model = AutoresearchTransformerCondDelta(
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(CHECKPOINT_DIR,
                                    f'{git_hash}_cond_delta_best.pt'))
            marker = ' *'
        ...
        print(f'  Delta β: {model.delta_beta_summary()}')
    ...
    print(f'description:    condDelta — {n_params/1e6:.1f}M, V9+condV, '
          f'J=24 + Delta Rule, EMA_init={EMA_INIT} floor={EMA_FLOOR}, '
