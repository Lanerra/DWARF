"""
🚀 DWARF D=1536 L=25 Triadic J=96 — phase-informed offset partitioning, H200, cold start

Architecture: D=1536, H=12 (hd=128), L=25, FFN=3072, TIED lm_head
  Triadic partitioning: 96 offsets split into 3 pure groups of 32 (no tail)
  Per-layer bandwidth ratio at D=1536: (32×128)/1536 = 2.67x (safe ≤ 3.0x)
...
Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_d1536_l25_triadic_j96_h200_bf16.py
"""
...
USE_LIGER_CE = _LIGER_AVAILABLE and os.getenv("DWARF_LIGER", "0") != "0"  # disabled; Liger CE crashes under BF16 autocast (H100 + 4090 both affected)
...
# V13 two-pass kernel (drop-in for DSQGAttentionGrouped, ~3x faster)
_USE_V13 = os.getenv('DWARF_USE_V13', '1') != '0'
try:
    from dsqg_attention_v13_dynamic_j import DSQGAttentionV13Dynamic as _V13Cls
except ImportError:
    _V13Cls = None
    _USE_V13 = False
    print('[WARN] dsqg_attention_v13_dynamic_j not found, falling back to V8')
_DSQGAttnCls = _V13Cls if (_USE_V13 and _V13Cls is not None) else None
# _DSQG_TYPES defined after DSQGAttentionGrouped class below
...
SCALE_EMBED_LR_MULT  = 25.98  # μP rule: 15 × √(D/512) = 15 × √(1536/512) = 25.98
EMA_INIT  = 0.020833
EMA_FLOOR = 0.00001
LR        = 2.5e-4
DROPOUT   = 0.1

BATCH_SIZE     = int(os.environ.get('DWARF_BS', '128'))
GRAD_ACCUM     = int(os.environ.get('DWARF_GA', '1'))
MAX_TRAIN_SEQS = int(os.environ.get('DWARF_MAX_TRAIN_SEQS', '452000'))  # 452K seqs = 22% Chinchilla for ~513M params at N=2048
MAX_SEQ_LEN    = 2048
MAX_VAL_SEQS   = 5_582
CE_CHUNK       = 512
SCREEN_EPOCHS  = 3
...
CKPT_BASE_NAME    = 'd1536_l25_triadic_j96'

CHECKPOINT_STRATEGY = os.getenv('DWARF_CKPT', 'none').lower()
# CPU activation offload: if VRAM at the start of a gradient-accumulation step
# exceeds this threshold, save_on_cpu(pin_memory=True) is engaged for that step.
# Activations stream to pinned system RAM during forward and prefetch back
# during backward via async CUDA streams — no recomputation, just PCIe transfers.
# Set 0 to always offload, 9999 to never.  H200 default: 20 GB.
CPU_OFFLOAD_GB = float(os.environ.get('DWARF_OFFLOAD_GB', '20'))
...
LAYER_LAYOUT = [
    ...
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L03: triad 2
    ('B', GROUP_B, J_SMALL_B, J_LARGE_B, False),   # L04
    ('C', GROUP_C, J_SMALL_C, J_LARGE_C, True),    # L05 + preIF
    ('FA', None, 0, 0, False),                       # L06: FullAttention @ 24% depth
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L07: triad 3 (Phase-1)
    ...
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L13: triad 5 (Phase-2)
    ('B', GROUP_B, J_SMALL_B, J_LARGE_B, False),   # L14
    ('C', GROUP_C, J_SMALL_C, J_LARGE_C, False),   # L15
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L16: triad 6
    ('B', GROUP_B, J_SMALL_B, J_LARGE_B, False),   # L17
    ('C', GROUP_C, J_SMALL_C, J_LARGE_C, False),   # L18
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L19: triad 7
    ('B', GROUP_B, J_SMALL_B, J_LARGE_B, False),   # L20
    ('C', GROUP_C, J_SMALL_C, J_LARGE_C, False),   # L21
    ('A', GROUP_A, J_SMALL_A, J_LARGE_A, False),   # L22: triad 8
    ('B', GROUP_B, J_SMALL_B, J_LARGE_B, False),   # L23
...
# Resolve DSQG types tuple now that DSQGAttentionGrouped is defined
_DSQG_TYPES = (DSQGAttentionGrouped, _V13Cls) if _V13Cls is not None else (DSQGAttentionGrouped,)
print(f'[KERNEL] Using {"V13-dynamic-J (two-pass)" if _DSQGAttnCls is not None else "V8-dynamic-J (single-pass)"}')
...
class DSQGBlockTriadic(nn.Module):
    def __init__(self, embedding_dim, num_heads, ffn_dim, seq_len,
        ...
        _Cls = _DSQGAttnCls if _DSQGAttnCls is not None else DSQGAttentionGrouped
        self.attn = _Cls(
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    out_path = os.path.join(checkpoint_dir, f"{CKPT_BASE_NAME}_ep{epoch}_full_attn.pt")  # D=1536
...
def train():
    ...
    print('=' * 70)
    print('  🚀 DWARF D1536-L25 Triadic J=96 — phase-informed partitioning, cold start')
    ...
    print(f'  checkpoint_strategy={CHECKPOINT_STRATEGY}, cpu_offload_threshold={CPU_OFFLOAD_GB:.0f} GB')
    if USE_LIGER_CE:
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        for acc_step in range(steps_per_epoch):
            ...
            # Decide once per optimizer step whether to CPU-offload activations.
            _vram_gb = torch.cuda.memory_allocated(device) / 1e9
            _use_cpu_offload = _vram_gb > CPU_OFFLOAD_GB
            if _use_cpu_offload and step == 1:
                print(f'  [offload] VRAM {_vram_gb:.1f} GB > {CPU_OFFLOAD_GB:.0f} GB threshold '
                      f'— activations offloading to pinned CPU RAM')
            ...
            for ga in range(GRAD_ACCUM):
                ...
                _offload_ctx = save_on_cpu(pin_memory=True) if _use_cpu_offload else contextlib.nullcontext()
                with _offload_ctx:
                    if USE_LIGER_CE:
                        with _amp_context(device):
