"""
d53_clean_27m — Pure DSQG 27M, NO InterferenceLayer / Kalman EMA / Huygens injection.

Ablation hypothesis:
  condV (Mar 3, 2026) established that Huygens K/V injection disrupts retrieval:
    condV PPL: 52.207 (best) / condV passkey: 36.7% (worst hybrid, vs condM 83.3%)
  condX/condX-v2 (Mar 8) confirmed Huygens degrades Q before full-attention retrieval.
  Paper draft notes: "Huygens injection encodes training-sequence-specific statistics
  that do not generalize at eval time."

  d52 (27M + full physics stack): 0% passkey at ep5.
  This run removes ALL physics additions to test whether they are raising the
  retrieval threshold above 27M.

Architecture:
  D=384, H=8, HD=48, L=8, FFN=1536 (4×D)
  → ~27.278M parameters (all 8 layers are plain TransformerBlock)
  → NO InterferenceLayer, NO Kalman EMA, NO Huygens K/V, NO KdV soliton, NO AGC
  → DSQGAttention UNCHANGED: pos_bias + scale_embed (Q-weighted) + if_gain
     (scale_embed was present in condU 35M pure which showed 42.8% passkey ep3)
  → Same J=44 geometry: [0..40, 128, 384, 1536]
  → Same uniform linspace pos_bias init

Expected result:
  If passkey emerges at ep3 → InterferenceLayer physics raising threshold.
  If still 0% through ep3 → scale is the dominant variable, not architecture.

Run for at least 3 epochs:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_2048_27m_d53_clean.py \\
    2>&1 | tee logs/27m_d53_clean_run.log
"""
...
EXPERIMENT_NAME = 'd53_clean_27m'
SAVE_DIR        = 'checkpoints/d53_clean_27m'
RESULT_FILE     = 'benchmarks/logs/d53_clean_27m_results.json'
...
# -- Uniform init — no θ asymmetry, let model find natural specialization ------
# d50 two-θ misfired: head 0 (α=0.2×1.5=0.30) ended up more globally-open than
# the intended distal head 7 (α=1.0×1.0=1.00). Linspace starting point < distal α.
# d51: clean uniform linspace across all 8 heads, geometry drives specialization.
_WAVELET_COEFFS = [0.4903, 0.8345, 0.2168, -0.1274]   # θ=1.0625 (provenance only)
...
class DSQGAttention(nn.Module):
    ...
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        # Uniform pos_bias initialization — standard linspace across all heads.
        # No θ asymmetry; let geometry ([0..40, 128, 384, 1536]) and training
        # dynamics find the natural head specialization organically.
        # pos_bias[j, h] = -log(1+δⱼ) * alpha_h,  alpha_h ∈ linspace(0.2, 2.0, H)
...
class DSQGTransformer(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads,
        ...
        # d53: ALL layers are plain TransformerBlock — no InterferenceLayer.
        # Ablation: removes Kalman EMA, Huygens K/V injection, KdV soliton, AGC.
        # DSQGAttention (pos_bias + scale_embed + if_gain) unchanged.
        layers = [TransformerBlock(embedding_dim, num_heads, ffn_dim, dropout=dropout)
...
def train():
    ...
    print(f'Uniform pos_bias init: linspace(0.2, 2.0, {NUM_HEADS}) — no θ asymmetry')
    print(f'Wavelet coefficients (provenance only):')
    print(f'  {[round(c,4) for c in _WAVELET_COEFFS]}')
