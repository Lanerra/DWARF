"""
🔬 DWARF Autoresearch — J16D 14M with V=8K Vocabulary

Hypothesis (autoresearcher vocab_size_gradient_tradeoff):
  At 14M (D=256), gradient suppression through the tied LM head is extreme:
    V=32K: suppression = (32000-1-512)/(32000-1) = 98.4%, lr_mult_analytic=63.9
    V=8K:  suppression = (8000-1-512)/(8000-1)  = 93.6%, lr_mult_analytic=16.1
  4× better gradient flow → predicted earlier phase transition (ep1-2 vs ep2-3).

Architecture: D=256, H=8, FFN=1024, L=6 (~14M), J=16 relay offsets
Changes vs J16D fulldata (35M, V=32K):
  EMBEDDING_DIM: 512 → 256  (14M scale)
  FFN_DIM:      2048 → 1024
  VOCAB_SIZE:  32000 → 8192  (KEY CHANGE: new BPE tokenizer, new cache)
Everything else identical: scale_embed_init=0.1, lr_mult=15, MAX_TRAIN_SEQS=121_232.

Run (from /tmp/dwarf-j17d):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 autoresearch/train_j16d_v8k.py \
    > autoresearch/run_j16d_v8k.log 2>&1
"""
...
EMBEDDING_DIM   = 256       # 14M scale (was 512)
NUM_HEADS       = 8
FFN_DIM         = 1024      # 14M scale (was 2048)
NUM_LAYERS      = 6
INTERFERENCE    = 3
FULL_ATTN_LAYER = 5
...
MAX_SEQ_LEN    = 2048
NUM_DOCS       = 100_000
MAX_TRAIN_SEQS = 52_716   # ~67% Chinchilla for 8M — enough to test phase transition timing
MAX_VAL_SEQS   = 5_582

FW_CACHE_FILE  = 'benchmarks/logs/condm_fineweb_edu_doc_cache.json'
TOKENIZER_PATH = 'results/fineweb_v8k_tokenizer.json'   # trained if missing
ENCODED_CACHE  = 'logs/fineweb_encoded_2048_v8k.pt'     # separate from v32k cache
...
# =============================================================================
# MODEL  (identical to train_j16d_fulldata.py — only D/FFN/vocab differ)
# =============================================================================
    ...
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads,
        ...
        for i in range(num_layers):
            if i == full_attn_layer:
                ...
                blocks.append(DSQGBlockV6(embedding_dim, num_heads, ffn_dim, seq_len,
...
def evaluate(model, data, device):
    ...
    vocab = model.out.weight.shape[0]
...
def train():
    ...
    print('=' * 70)
    print('  🔬 DWARF — J16D 14M V=8K Vocabulary Test')
    print('  Hypothesis: V=8K reduces LM head gradient suppression 98.4%→93.6%')
    print('=' * 70)
    ...
    sup_32k = (32000 - 1 - 2*EMBEDDING_DIM) / (32000 - 1) * 100
    sup_8k  = (actual_vocab - 1 - 2*EMBEDDING_DIM) / (actual_vocab - 1) * 100
    print(f'Gradient suppression: V=32K={sup_32k:.1f}%  V={actual_vocab}={sup_8k:.1f}%')
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(CHECKPOINT_DIR, 'j16d_v8k_best.pt'))
            marker = ' *'
    ...
    print(f'description:    J16D V=8K 14M — D={EMBEDDING_DIM}, V={actual_vocab}, '
          f'scale_embed_init={SCALE_EMBED_INIT_VAL}, lr_mult={SCALE_EMBED_LR_MULT}')
