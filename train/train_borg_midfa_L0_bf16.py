...
EXTRACTED_CKPT = "autoresearch/checkpoints/borg_midattn_ep2_full_attn.pt"
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    out_path = os.path.join(checkpoint_dir, f"borg_midfa_L0_ep{epoch}_full_attn.pt")
...
def train(no_warmstart: bool = False):
    ...
    print('=' * 70)
    print('  🔬 DWARF Autoresearch — BORG MID-FA@L0 RUN')
    print('  FullAttn at L0 of 6, warm-started from borg_midattn ep2 FA (partially adapted), UNFROZEN')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}')
    print(f'  D={EMBEDDING_DIM}, H={NUM_HEADS}, L={NUM_LAYERS}, FFN={FFN_DIM}')
    print(f'  IF interval={INTERFERENCE}, Full attn layer={FULL_ATTN_LAYER} (L0 of 6, unfrozen)')
    ...
    best_ckpt_name = f'borg_midfa_L0_best.pt'
    if no_warmstart:
...
if __name__ == '__main__':
    ...
    if args.no_warmstart:
        ...
        print("  Checkpoint: borg_midfa_L0_best.pt\n")
