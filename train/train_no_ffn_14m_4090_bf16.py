"""
🔬 DWARF No-FFN Ablation — Is the FFN load-bearing or redundant?

Hypothesis: If the relay mechanism genuinely carries the memory/retrieval burden,
the FFN (which is a pure nonlinear transform in DWARF's division of labour) may be
entirely redundant. This run removes FFN from ALL blocks — DSQG and FA alike —
replacing each FFN+norm2 with nothing.

Architecture: D=512, H=8 (hd=64), L=8, FFN=0, J=24 (se015 offsets)
  L0:  DSQGBlockNoFFN  IF=False  ← pure DSQG relay, no FFN
  L1:  DSQGBlockNoFFN  IF=True   ← preIF (single layer before FA)
  L2:  FullAttentionBlockNoFFN   ← FA@L2, no FFN
  L3-7: DSQGBlockNoFFN IF=False  ← post-FA relay layers, no FFN

Config:
  - Tokenizer: results/fineweb_tokenizer_8k.json  (8K BPE, FineWeb-Edu)
  - Dataset:   logs/fineweb_edu_encoded_2048_8k.pt (87,484 train seqs)
  - EMA_INIT = 0.0208 (= 1/δ_relay_min = 1/48, same se015 offset set)
  - SCALE_EMBED_INIT = 0.1, LR_MULT = 15.0
  - Batch: BS=128 × GRAD_ACCUM=1 → eff_batch=128 (small vocab → roomy VRAM)
  - Cold start, 3 epochs
  - ~13.6M parameters (embeddings dominate at 8K vocab + D=512)

Baseline comparison:
  - Moonshot-58M FFN=2048:  35.04 PPL, 99.2% passkey  (ar_score = 0.0)
  - FFN=1024 ablation ep1:  38.18 PPL, 93.3% passkey
  - Predicted if FFN truly redundant: PPL within ~5 of FFN=1024 ep1
...
Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_no_ffn_14m_4090_bf16.py \\
    2>&1 | tee logs/run_no_ffn_14m.log
"""
...
# Dataset: 8K-tokenized FineWeb-Edu, 87,484 train seqs
# Chinchilla-optimal for ~13.6M params: 20 × 13.6M / 2048 = 132,812 seqs
# Available: 87,484 seqs = 65.9% Chinchilla — sufficient for directional screen
...
BATCH_SIZE  = 32    # 4090 safe: DSQG kernel at seq=2048 is VRAM-hungry regardless of vocab
GRAD_ACCUM  = 4     # eff_batch=128 preserved
MAX_SEQ_LEN = 2048

TOKENIZER_PATH   = 'results/fineweb_tokenizer_8k.json'
ENCODED_DATASET  = 'logs/fineweb_edu_encoded_2048_8k.pt'
...
def train():
    ...
    print('=' * 70)
    print('  🔬 DWARF No-FFN Ablation — D=512 H=8 L=8 FFN=0, 8K vocab')
    print('  Is the FFN load-bearing, or can the relay carry everything?')
    print('=' * 70)
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(CHECKPOINT_DIR, 'no_ffn_14m_best.pt'))
            marker = ' *'
        ...
        torch.save(model.state_dict(),
                   os.path.join(CHECKPOINT_DIR, f'no_ffn_14m_ep{epoch}_resume.pt'))
        print(f'  Saved resume checkpoint: no_ffn_14m_ep{epoch}_resume.pt')
    ...
    print(f'description: No-FFN ablation — D={EMBEDDING_DIM} H={NUM_HEADS} L={NUM_LAYERS} '
          f'FFN=0 J=24 se015, fineweb_tokenizer_8k, EMA_INIT=1/δ_relay_min')
