"""
🔬 DWARF J13D v3 — L=8 Warm-Start from J13D v2 ep3
J=13 H1+Frobenius-optimal: [1,2,4,5,8,16,32,64,128,256,512,768,1024]
Full warm-start from J13D v2 ep3 checkpoint (FA@L2, L=6 trained from scratch).
L0-5 weights carried forward via strict=False; L6-7 (new DSQG) random-init.
FA@L2 unfrozen — refines against 6 additional post-FA DSQG layers.

Motivation: J13D v2 is the first native FA@L2 trained from scratch on J=13
offsets. L=8 warm-start lets the FA block adapt to deeper post-FA stack
(6 post-FA layers vs 4 in v2) while preserving trained DSQG representations.
Same blueprint as Gen3→se015 (Gen3 full ckpt → Gen5 L=8 warm-start).

Architecture (D=512, H=8, L=8, FFN=2048 → ~45M params with tied embeddings):
  L0:  DSQGBlockV6Physics  IF=False  ← warm from v2 L0
  L1:  DSQGBlockV6Physics  IF=True   ← warm from v2 L1 (preIF)
  L2:  FullAttentionBlock            ← warm from v2 L2, FA@L2 UNFROZEN
  L3-5: DSQGBlockV6Physics IF=False  ← warm from v2 L3-5
  L6-7: DSQGBlockV6Physics IF=False  ← random-init (new layers)

Run (from repo root, on 4090):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_borg_j13d_v3_l8_4090_bf16.py > logs/run_j13d_v3_l8.log 2>&1 &
"""
...
EMA_INIT  = 0.003   # abs()+floor parameterization (EMA dead-zone fix from v2)
EMA_FLOOR = 0.00001
...
EXTRACTED_CKPT = "autoresearch/checkpoints/j13d_v2_30m_ep3_resume.pt"  # Full warm-start from J13D v2
...
VOCAB_SIZE     = 32000
BATCH_SIZE     = 20   # 4090: L=8, FA-compile-only; BS=24 OOM w/ DSQG compile, BS=20 safe headroom
GRAD_ACCUM     = 4    # effective batch = 128; larger model benefits from more stable gradients
MAX_SEQ_LEN    = 2048
NUM_DOCS       = 100_000
MAX_VAL_SEQS   = 5_582
...
def save_resume_checkpoint(model, optimizer, scheduler, epoch, git_hash, checkpoint_dir):
    ...
    out_path = os.path.join(checkpoint_dir, f"j13d_v3_l8_ep{epoch}_resume.pt")
    torch.save({
        "epoch":                epoch,
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":           FFN_DIM,
            "seq_len":           MAX_SEQ_LEN,
            "source_script":     "train/train_borg_j13d_v3_l8_4090_bf16.py",
            "source_layer":      FULL_ATTN_LAYER,
            "num_layers":        NUM_LAYERS,
            "num_offsets":       len(OFFSETS),
            "epoch":             epoch,
            "note": (
                "J13D v3 L=8 warm-start from J13D v2 ep3. "
                f"Epoch {epoch}, FA@L{FULL_ATTN_LAYER}, preIF@L{FULL_ATTN_LAYER-1}."
            ),
        },
    }

    out_path = os.path.join(checkpoint_dir, f"j13d_v3_l8_ep{epoch}_full_attn.pt")
...
def train():
    ...
    print('=' * 70)
    print('  🔬 DWARF J13D v3 — L=8 Warm-Start from J13D v2 ep3')
    print('  J=13 offsets: [1,2,4,5,8,16,32,64,128,256,512,768,1024]')
    print('  Full warm-start: L0-5 from v2, L6-7 random-init, FA@L2 unfrozen.')
    print('=' * 70)
    ...
    best_ckpt_name = 'j13d_v3_l8_best.pt'

    # ── Warm-start from J13D v2 ep3 full checkpoint ───────────────────────────
    if os.path.exists(EXTRACTED_CKPT):
        print(f'\n  Warm-starting from J13D v2: {EXTRACTED_CKPT}')
        ckpt = torch.load(EXTRACTED_CKPT, map_location=device)
        sd = ckpt.get("model_state_dict", ckpt)
        sd = {k.replace("_orig_mod.", ""): v for k, v in sd.items()}
        missing, unexpected = model.load_state_dict(sd, strict=False)
        print(f'  Loaded — missing keys: {len(missing)} (L6-7 new layers), unexpected: {len(unexpected)}')
        print(f'  FA@L{FULL_ATTN_LAYER} UNFROZEN — all {NUM_LAYERS} layers trainable')
        if missing:
            print(f'  New random-init layers: {sorted(set(k.split(".")[1] for k in missing))}')
    else:
    ...
    try:
        for i, block in enumerate(model.blocks):
            if type(block).__name__ == "FullAttentionBlock":
                ...
                print(f"  compiled FullAttentionBlock at layer {i}")
        # Note: compiling DSQG blocks causes graph breaks + Triton OOM at L=8 BS=24;
        # FA-only compile is the proven safe pattern (same as se015, gen5).
        print("  torch.compile applied to FullAttentionBlock(s) only")
    ...
    # Resume mid-run if interrupted
    resume_ckpt = None
    for ep in range(SCREEN_EPOCHS, 0, -1):
        candidate = os.path.join(CHECKPOINT_DIR, f"j13d_v3_l8_ep{ep}_resume.pt")
    ...
    if resume_ckpt:
        ...
        print("  No resume checkpoint found — starting from warm-start\n")
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        for acc_step in range(steps_per_epoch):
            for ga in range(GRAD_ACCUM):
                ...
                x, y  = batch[:, :-1].to(device, non_blocking=True), batch[:, 1:].to(device, non_blocking=True)
    ...
    print(f'description:    J13D v3 L=8 warm-start — {n_params/1e6:.1f}M, '
          f'D={EMBEDDING_DIM}, L={NUM_LAYERS}, FA@L{FULL_ATTN_LAYER}, preIF@L{FULL_ATTN_LAYER-1}, '
          f'J=13 offsets, warm-start from J13D v2 ep3, 4090-tuned kernel')
