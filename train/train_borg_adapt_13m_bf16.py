...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    out_path = os.path.join(checkpoint_dir, f"borg_adapt_13m_ep{epoch}_full_attn.pt")
...
def train(no_warmstart: bool = False):
    ...
    print('=' * 70)
    print('  🔬 DWARF Autoresearch — BORG ADAPTATION RUN')
    print('  FullAttn at L0, warm-started from J26D, adapting to raw embeddings')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}')
    print(f'  D={EMBEDDING_DIM}, H={NUM_HEADS}, L={NUM_LAYERS}, FFN={FFN_DIM}')
    print(f'  IF interval={INTERFERENCE}, Full attn layer={FULL_ATTN_LAYER} (FIRST)')
    ...
    best_ckpt_name = f'borg_adapt_13m_{ckpt_suffix}_best.pt'
    if no_warmstart:
...
if __name__ == '__main__':
    ...
    if args.no_warmstart:
        ...
        print("  Checkpoint: borg_adapt_13m_control_best.pt\n")
