...
Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u benchmarks/train_2048_condV.py \
    2>&1 | tee benchmarks/logs/d41s5_run.log
"""
...
SAVE_DIR    = 'checkpoints/d41s5'
RESULT_FILE = 'benchmarks/logs/d41s5_results.json'

# -- condN offset set ----------------------------------------------------------

_DENSE_LOCAL_W     = 41
_DYADIC_LONG_RANGE = [48, 128, 384, 768, 1536]
_COND_N_OFFSETS    = sorted(set(range(0, _DENSE_LOCAL_W + 1)) |
                             set(_DYADIC_LONG_RANGE))
assert len(_COND_N_OFFSETS) == 47
...
from dsqg_attention_d41s5 import dsqg_attention_v3
...
def main():
    ...
    print('\nApplying torch.compile (mode=default) — first step will be slow...')
