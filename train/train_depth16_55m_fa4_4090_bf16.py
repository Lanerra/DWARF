...
Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_depth16_55m_fa4_4090_bf16.py \\
    > logs/run_depth16_55m_fa4.log 2>&1 &
"""
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":       FFN_DIM,
            "seq_len":       MAX_SEQ_LEN,
            "source_script": "train/train_depth16_55m_fa4_4090_bf16.py",
    ...
    out_path = os.path.join(checkpoint_dir, f"depth16_55m_fa4_ep{epoch}_full_attn.pt")
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(clean_state,
                       os.path.join(CHECKPOINT_DIR, 'depth16_55m_fa4_best.pt'))
            marker = ' *'
        ...
        torch.save({
            ...
            'val_loss': val_loss,
        }, os.path.join(CHECKPOINT_DIR, f'depth16_55m_fa4_ep{epoch}_resume.pt'))
