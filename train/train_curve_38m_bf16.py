"""
🔬 DWARF Clean Curve — 38M BF16 (4090)

Clean curve 38M — D=512, L=6, FFN=2048, V=32K, J24D-int2 physics

Architecture: J24D-int2 + condV physics (EMA + KdV + AGC)
  - V8 kernel with J=24 offsets
  - IF fires at layers where i % 2 == 1: [1, 3]
  - Full attention at layer L-1 (layer 5)
  - BF16, no GradScaler

NOTE: This replaces the existing champion run. Same arch, new tokenizer/dataset.

Run (from /home/dlewis3/Desktop/AI/DWARF):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_curve_38m_bf16.py \
    > logs/run_curve_38m.log 2>&1 &
"""
...
CHECKPOINT_NAME = 'curve_38m_best.pt'
...
def train():
    ...
    print('=' * 70)
    print('  🔬 Clean curve 38M — D=512, L=6, FFN=2048, V=32K, J24D-int2 physics')
    print('=' * 70)
    ...
    print(f'description:    Clean curve 38M — D=512, L=6, FFN=2048, V=32K, J24D-int2 physics')
