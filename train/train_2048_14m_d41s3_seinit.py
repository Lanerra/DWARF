...
Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u benchmarks/train_2048_condV.py \
    2>&1 | tee benchmarks/logs/d41s3_seinit_run.log
"""
...
SAVE_DIR    = 'checkpoints/d41s3_seinit'
RESULT_FILE = 'benchmarks/logs/d41s3_seinit_results.json'

# -- condN offset set ----------------------------------------------------------

_DENSE_LOCAL_W     = 41
_DYADIC_LONG_RANGE = [48, 128, 384]
_COND_N_OFFSETS    = sorted(set(range(0, _DENSE_LOCAL_W + 1)) |
                             set(_DYADIC_LONG_RANGE))
assert len(_COND_N_OFFSETS) == 45
...
from dsqg_attention_d41s3 import dsqg_attention_v3  # reuse same kernel
...
class DSQGAttentionQW(nn.Module):
    ...
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        self.scale_embed = nn.Parameter(torch.full((45, HD), 0.5))  # non-zero init: bootstraps Q-matching before ep3
...
class DSQGBlock(nn.Module):
    ...
    def __init__(self, embedding_dim, num_heads, ffn_dim, seq_len,
        ...
        if interference:
            ...
            self.ema_factor   = nn.Parameter(torch.full((1,), 0.005))  # lower init → longer window from start
...
def main():
    ...
    print('=' * 70)
    print('  d41s3_seinit -- scale_embed non-zero init + lower EMA clamp floor (13M, FineWeb-Edu)')
    print('=' * 70)
