"""
d54_clean_35m — Pure DSQG 35M, NO InterferenceLayer / Kalman EMA / Huygens injection.

Context:
  d53 (27M clean, no IF layers): 0% passkey at ep3. InterferenceLayer was NOT the
  culprit — scale is the gating variable.
  
  condU 35M pure (with IF layers): 42.8% passkey at ep3, 33.3% final.
  condU 35M used D=512, H=8, L=6, FFN=2048 WITH InterferenceLayer at layers 2+5.

  This run uses the SAME dimensions as condU 35M, but removes InterferenceLayer.
  Answers: "Do the physics additions suppress passkey at 35M, or are they neutral?"
  
  Expected outcomes:
  - If passkey > 33.3% (condU 35M pure) → IF layers suppress retrieval even at 35M
  - If passkey ≈ 33.3%                  → IF layers are neutral; scale alone governs
  - If passkey < 33.3%                  → unexpected; requires investigation

Architecture:
  D=512, H=8, HD=64, L=6, FFN=2048 (4×D)
  → ~36.35M parameters (all 6 layers plain TransformerBlock — ~1.35M > condU 35M
     because IF extras are absent; functionally equivalent architecture family)
  → NO InterferenceLayer, NO Kalman EMA, NO Huygens K/V, NO KdV soliton, NO AGC
  → DSQGAttention UNCHANGED: pos_bias + scale_embed + if_gain
  → Same J=44 geometry: [0..40, 128, 384, 1536]
  → Same uniform linspace pos_bias init

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_2048_35m_d54_clean.py \\
    2>&1 | tee logs/35m_d54_clean_run.log
"""
...
EXPERIMENT_NAME = 'd54_clean_35m'
SAVE_DIR        = 'checkpoints/d54_clean_35m'
RESULT_FILE     = 'benchmarks/logs/d54_clean_35m_results.json'
