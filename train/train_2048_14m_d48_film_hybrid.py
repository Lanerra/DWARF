"""
FiLM-Hybrid — FiLM-Conditioned FFN + Full Attention at Layer 5 (14M, 3-Epoch Probe)

FiLM (Feature-wise Linear Modulation) conditioning on the FFN output:
  s = scale_embed.detach().abs().mean()        (scalar, no grad through s)
  gamma_out = (1 + W_gamma * s) * ffn_out      (learned scale)
  beta_out  = W_beta * s                        (learned shift)
  conditioned = gamma_out + beta_out

At init (W_gamma=0, W_beta=0): pure identity. The FFN learns to modulate
...
Architecture: d48 offsets (dense=48, sparse=[96,128,384], J=52)
  FULL_ATTN_LAYER = 5 (hybrid: full attention at layer 5)
  INTERFERENCE = 3 (receiver-chain every 3rd block)

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_2048_14m_d48_film_hybrid.py \
    2>&1 | tee benchmarks/logs/d48_film_hybrid_14m.log
"""
...
EXPERIMENT_NAME = 'd48_film_hybrid_14m'
...
SAVE_DIR    = 'checkpoints/d48_film_hybrid_14m'
RESULT_FILE = 'benchmarks/logs/d48_film_hybrid_14m_results.json'
...
class DSQGBlock(nn.Module):
    def __init__(self, embedding_dim, num_heads, ffn_dim, seq_len,
        ...
        # FiLM conditioning (zero-initialized = identity at start)
        self.film_gamma = nn.Parameter(torch.zeros(embedding_dim))
        self.film_beta = nn.Parameter(torch.zeros(embedding_dim))
    ...
    def forward(self, x):
        ...
        # FiLM: condition FFN output on scale_embed magnitude
        s = self.attn.scale_embed.detach().abs().mean()
        ffn_out = (1.0 + self.film_gamma * s) * ffn_out + self.film_beta * s
        x = x + ffn_out
...
def main():
    ...
    print('=' * 70)
    print(f'  {EXPERIMENT_NAME} -- FiLM-Conditioned FFN + Hybrid (14M, FineWeb-Edu)')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}  '
              f'({torch.cuda.get_device_properties(0).total_memory/1e9:.1f} GB)')
    print(f'  Kernel: dsqg_attention_v3 (d48 offsets)')
    print(f'  FiLM conditioning: gamma/beta zero-init, detached scale_embed signal')
    print(f'  Architecture: INTERFERENCE=3, gate=0, 1 FullAttn @ layer {FULL_ATTN_LAYER}')
