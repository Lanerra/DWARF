"""
🔬 DWARF Autoresearch — BORG-2 DUAL-FA RUN
Frozen borg_midattn FA at L2 + Unfrozen J26D FA at L5
DSQG blocks 0,1,3,4 train from scratch

Architecture (D=512, H=8, L=6, FFN=2048):
  Block 0:  DSQGBlockV6Physics  ← random init, trains
  Block 1:  DSQGBlockV6Physics  ← random init, trains (IF block)
  Block 2:  FullAttentionBlock  ← FROZEN, from borg_midattn_ep2_full_attn.pt
  Block 3:  DSQGBlockV6Physics  ← random init, trains (IF block)
  Block 4:  DSQGBlockV6Physics  ← random init, trains
  Block 5:  FullAttentionBlock  ← UNFROZEN, from j26d_full_attn_extracted.pt

L2 FA was trained at mid-depth receiving processed DSQG residuals.
L5 FA was trained at final position as J26D champion.

Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_borg2_dual_fa_bf16.py \
    > logs/run_borg2_dual_fa.log 2>&1 &
"""
...
FA_L2_CKPT = "autoresearch/checkpoints/borg_midattn_ep2_full_attn.pt"
FA_L5_CKPT = "autoresearch/checkpoints/j26d_full_attn_extracted.pt"
...
class AutoresearchTransformerPhysics(nn.Module):
    def __init__(self, vocab_size, embedding_dim, num_layers, num_heads,
                 ffn_dim, seq_len, full_attn_layers=None, full_attn_layer=None,
                 interference_interval=None, scale_embed_init_val=0.0, dropout=0.1):
        super().__init__()
        if full_attn_layers is None:
            full_attn_layers = [full_attn_layer] if full_attn_layer is not None else [num_layers - 1]
    ...
    def full_attn_parameters(self, layers=None):
        """Yield all parameters from the FullAttentionBlocks at specified layers."""
        if layers is None:
            layers = self.full_attn_layers
        for layer_idx in layers:
            for p in self.blocks[layer_idx].parameters():
                yield p

    def non_full_attn_parameters(self):
        """Yield all parameters except those in the FullAttentionBlocks."""
...
def save_full_attn_checkpoint(model, fa_layer, epoch, checkpoint_dir):
    """Save just the FullAttn block weights for a specific layer."""
    full_attn_block = model.blocks[fa_layer]
    state_dict = {}
    for name, param in full_attn_block.named_parameters():
        state_dict[f"blocks.{fa_layer}.{name}"] = param.data.clone()
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":           FFN_DIM,
            "seq_len":           MAX_SEQ_LEN,
            "source_script":     "train/train_borg2_dual_fa_bf16.py",
            "source_layer":      fa_layer,
            "num_layers":        NUM_LAYERS,
            "epoch":             epoch,
            "note": (
                f"FullAttn block at L{fa_layer} from BORG-2 dual-FA run. "
                f"Epoch {epoch}."
            ),
        },
    }

    out_path = os.path.join(checkpoint_dir, f"borg2_dual_fa_L{fa_layer}_ep{epoch}_full_attn.pt")
    torch.save(payload, out_path)
    print(f"  Saved FullAttn L{fa_layer} checkpoint: {out_path}")
...
def train():
    ...
    print('=' * 70)
    print('  🔬 DWARF Autoresearch — BORG-2 DUAL-FA RUN')
    print('  Frozen borg_midattn FA at L2 + Unfrozen J26D FA at L5')
    print('  DSQG blocks 0,1,3,4 train from scratch')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}')
    print(f'  D={EMBEDDING_DIM}, H={NUM_HEADS}, L={NUM_LAYERS}, FFN={FFN_DIM}')
    print(f'  IF interval={INTERFERENCE}, Full attn layers={FULL_ATTN_LAYERS}')
    print(f'  Frozen FA layers={FREEZE_FA_LAYERS}')
    ...
    model = AutoresearchTransformerPhysics(
        ...
        seq_len=MAX_SEQ_LEN, full_attn_layers=FULL_ATTN_LAYERS,
        interference_interval=INTERFERENCE,
        scale_embed_init_val=SCALE_EMBED_INIT_VAL,
    ).to(device)

    # ── Warm-start FullAttn blocks from extracted weights ──────────────────────
    print(f'\n  Loading dual FA warm-start weights...')

    if os.path.exists(FA_L2_CKPT):
        print(f'  Loading L2 FA from {FA_L2_CKPT}...')
        ckpt_l2 = torch.load(FA_L2_CKPT, map_location="cpu")
        fa_l2_weights = ckpt_l2["full_attn_block"]
        remapped_l2 = {}
        for k, v in fa_l2_weights.items():
            new_key = k.replace("blocks.2._orig_mod.", "blocks.2.")
            remapped_l2[new_key] = v
        missing, unexpected = model.load_state_dict(remapped_l2, strict=False)
        print(f'  L2 FA: loaded {len(remapped_l2)} weights, missing {len(missing)}')
        if unexpected:
            print(f'  WARNING L2 unexpected: {unexpected}')
    else:
        print(f'  WARNING: {FA_L2_CKPT} not found — L2 FA random init')

    if os.path.exists(FA_L5_CKPT):
        print(f'  Loading L5 FA from {FA_L5_CKPT}...')
        ckpt_l5 = torch.load(FA_L5_CKPT, map_location="cpu")
        fa_l5_weights = ckpt_l5["full_attn_block"]
        remapped_l5 = {}
        for k, v in fa_l5_weights.items():
            new_key = k.replace("blocks.5._orig_mod.", "blocks.5.")
            remapped_l5[new_key] = v
        missing, unexpected = model.load_state_dict(remapped_l5, strict=False)
        print(f'  L5 FA: loaded {len(remapped_l5)} weights, missing {len(missing)}')
        if unexpected:
            print(f'  WARNING L5 unexpected: {unexpected}')
    else:
        print(f'  WARNING: {FA_L5_CKPT} not found — L5 FA random init')

    for fa_layer in FREEZE_FA_LAYERS:
        for p in model.blocks[fa_layer].parameters():
            p.requires_grad = False
    print(f'  L2 FA frozen. L5 FA unfrozen (will refine).')
    ...
    non_scale_embed_params = [p for p in model.non_scale_embed_parameters() if p.requires_grad]
    scale_embed_params     = [p for p in scale_embed_params if p.requires_grad]
    ...
    passkey_results = {}
    ppl_results     = {}
    best_ckpt_name = 'borg2_dual_fa_best.pt'
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        for fa_layer in FULL_ATTN_LAYERS:
            save_full_attn_checkpoint(model, fa_layer, epoch, CHECKPOINT_DIR)
    ...
    print(f'description:    BORG-2 Dual-FA — {n_params/1e6:.1f}M, L2 frozen midattn FA + L5 unfrozen J26D FA, '
          f'D={EMBEDDING_DIM}, L={NUM_LAYERS}')
