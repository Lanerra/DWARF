...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    out_path = os.path.join(checkpoint_dir, f"borg_midattn_ep{epoch}_full_attn.pt")
...
def train(no_warmstart: bool = False):
    ...
    print(f'  IF interval={INTERFERENCE}, Full attn layer={FULL_ATTN_LAYER} (MIDDLE, L2 of 5)')
    ...
    best_ckpt_name = f'borg_midattn_best.pt'
    if no_warmstart:
...
if __name__ == '__main__':
    ...
    if args.no_warmstart:
        ...
        print("  Checkpoint: borg_midattn_best.pt\n")
