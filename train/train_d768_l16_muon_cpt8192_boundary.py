"""
DWARF D=768 L=16 Triadic J=96 — HISA/DSQG hybrid v2 L3, Bible-Muon 8192 boundary-packed CPT variant.

CPT target: resume the scaled D768/L16 Muon staggered-MOVT scratch checkpoint and adapt it to
8192-token boundary-packed StarCoderData / ArXiv / original-replay / LongPack data.
This is the ~124M-class continuation counterpart to the D768/L16 40/30/20/10 scratch run.
...
Architecture: D=768, H=12 (hd=64), L=16, FFN=2048, tied lm_head
  ...
  HISA block: HierarchicalSparseAttentionV15HISA(C=128 default, top_k=4, HISA_m=64 default)
...
NUM_CHUNKS       = int(os.environ.get('DWARF_NUM_CHUNKS', '128'))
...
EMA_INIT  = 0.020833
EMA_FLOOR = 0.00001
LR        = float(os.environ.get('DWARF_LR', '1e-4'))
...
LR_WARMUP_STEPS = int(os.environ.get('DWARF_LR_WARMUP_STEPS', '250'))
...
MAX_SEQ_LEN    = int(os.environ.get('DWARF_SEQ_LEN', '8192'))
MAX_VAL_SEQS   = int(os.environ.get('DWARF_MAX_VAL_SEQS', '4096'))
...
PASSKEY_DISTANCES    = _parse_int_list_env(
    'DWARF_PASSKEY_DISTANCES',
    [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 1536,
     2048, 3072, 4096, 5120, 6144, 7168, 7680],
)
...
CHECKPOINT_DIR    = os.environ.get('DWARF_CHECKPOINT_DIR', 'autoresearch/checkpoints/cpt8192_muon_boundary')
_CKPT_BASE_DEFAULT = 'd768_l16_muon_cpt8192_boundary_new_dataset_staggered_movt' if os.getenv('DWARF_STAGGER_MOVT_PLANES', '1') == '1' else 'd768_l16_muon_cpt8192_boundary_new_dataset'
...
DATASET_PATH = os.environ.get('DWARF_DATASET', 'datasets/cpt_8192_starcoder35_arxiv35_orig20_longabc10_boundary_150k.pt')
DEFAULT_RESUME_PATH = os.environ.get('DWARF_DEFAULT_RESUME', 'autoresearch/checkpoints/d768_l16_muon_40_30_20_10_staggered_movt_ep1.pt')
...
def _adamw_cls():
    if _BNB_AVAILABLE:
        return getattr(bnb.optim, 'AdamW8bit', bnb.optim.AdamW8bit)
...
def _streamed_linear_ce_loss(hidden: torch.Tensor,
    ...
    """Compute tied final-projection CE, optionally masked for padded tails."""
    ...
    grad_h = torch.zeros_like(h) if grad_denom is not None else None

    for s in range(0, n_rows, chunk_rows):
        e = min(s + chunk_rows, n_rows)
        h_chunk = h[s:e]
        mask_chunk = None if flat_mask is None else flat_mask[s:e]
        if mask_chunk is not None and not bool(mask_chunk.any()):
            continue
        ...
        if mask_chunk is None:
            ...
            loss_vec = F.cross_entropy(logits.float(), y[s:e], reduction='none')
            loss_sum = (loss_vec * mask_chunk.to(dtype=loss_vec.dtype)).sum()
            del loss_vec
        total_loss = total_loss + loss_sum.detach()
        if grad_denom is not None:
            (loss_sum / float(max(grad_denom, 1.0))).backward()
...
def train():
    ...
    print('  CPT: boundary-packed 8192 StarCoderData / ArXiv / original replay / LongPack mix. D768/L16 continuation target.')
    print('=' * 70)
    ...
    if train_data.size(1) != MAX_SEQ_LEN or val_data.size(1) != MAX_SEQ_LEN:
        raise ValueError(
            f'Dataset seq_len train={train_data.size(1)} val={val_data.size(1)} does not match MAX_SEQ_LEN={MAX_SEQ_LEN}. '
            'Set DWARF_SEQ_LEN to the encoded row length.'
        )
    train_loss_mask = _cache.get('train_loss_mask')
    val_loss_mask = _cache.get('val_loss_mask')
    ...
    if USE_LIGER_CE and (not bool(train_loss_mask.all()) or not bool(val_loss_mask.all())):
        raise RuntimeError('Liger fused CE does not support boundary loss masks; run with DWARF_LIGER=0.')
    ...
    print(
        ...
        f'train_pad={1 - train_real / train_slots:.2%} val_pad={1 - val_real / val_slots:.2%}'
    )
    ...
    resume_path = os.getenv('DWARF_RESUME', DEFAULT_RESUME_PATH)
    ...
    if resume_path and os.path.isfile(resume_path):
        ...
        if 'model_state_dict' in ckpt:
            ...
            skip_opt = os.getenv('DWARF_SKIP_OPT', '1') == '1'
            if not skip_opt:
                try:
                    optimizer.load_state_dict(ckpt['optimizer_state_dict'])
                except (KeyError, ValueError, RuntimeError) as _oe:
            ...
            if 'scheduler_state_dict' in ckpt and not skip_opt:
                scheduler.load_state_dict(ckpt['scheduler_state_dict'])
        else:
            model.load_state_dict(ckpt, strict=False)
        print(f'  Resumed from {resume_path} (starting epoch {start_epoch})')
    elif resume_path:
        raise FileNotFoundError(f'DWARF_RESUME/default checkpoint not found: {resume_path}')
    ...
    model_ref = _unwrap_compiled_module(model)
    tokens_per_step = BATCH_SIZE * GRAD_ACCUM * (MAX_SEQ_LEN - 1)  # nominal slots; logs ignore masked padding
    ...
    for epoch in range(start_epoch, SCREEN_EPOCHS + 1):
        ...
        for acc_step in range(steps_per_epoch):
            ...
            for idx_start in micro_starts:
                batch_idx = indices[idx_start:idx_start + BATCH_SIZE]
                if len(batch_idx):
                    total_rows_this_accum += int(train_loss_mask[batch_idx, 1:].sum().item())
            total_rows_this_accum = max(total_rows_this_accum, 1)

            for idx_start in micro_starts:
                batch_idx = indices[idx_start:idx_start + BATCH_SIZE]
                batch = train_data[batch_idx]
                batch_mask = train_loss_mask[batch_idx]
                ...
                if USE_LIGER_CE:
                    ...
                    if n_rows <= 0:
                        del batch, batch_mask, x, y, target_mask
                        continue
                    with _amp_context(device):
                        hidden = model.forward_hidden(x)
                    total_loss, n_valid = _streamed_linear_ce_loss(
                        hidden, y, model_ref.out.weight,
                        chunk_rows=CE_CHUNK,
    ...
    print('\n' + '=' * 70)
    print(f'  DSR + R_PLANES={R_PLANES} Bible-Muon CPT8192 Boundary Summary (D{EMBEDDING_DIM}-L{NUM_LAYERS}, tied lm_head)')
    print('=' * 70)
