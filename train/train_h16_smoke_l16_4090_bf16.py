"""
🚀 DWARF H=16 Smoke Test — D=512, H=16 (hd=32), L=16, FFN=1024, FA@L4, cold-start

Architecture: D=512, H=16 (hd=32), L=16, FFN=1024, J=24 (se015 offsets), TIED lm_head
...
Smoke test: H=16 (hd=32) vs validated H=8 (hd=64) at D=512.
  Relay/residual ratio = (J=24 × hd=32) / D=512 = 1.5× (same as D=1024/H=16 validated config)
  Kernel accepts arbitrary HD via constexpr; BLOCK_N=128 at HD<=64 branch applies.
  1 epoch only — checking relay activation (scale_embed |max| vs 2.0 threshold) and passkey.
  Reference: h16_smoke_fa4 (H=8, hd=64) — same L,D,FFN,FA placement, different H only.

Key question: does hd=32 change relay activation dynamics vs hd=64?
...
Run (from repo root):
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/train_h16_smoke_fa4_4090_bf16.py \\
    > logs/run_h16_smoke_fa4.log 2>&1 &
"""
...
EMBEDDING_DIM    = 512
NUM_HEADS        = 16         # hd = 512/16 = 32  ← smoke test for unvalidated h_d
...
def save_full_attn_checkpoint(model, epoch, git_hash, checkpoint_dir):
    ...
    payload = {
        ...
        "config": {
            ...
            "ffn_dim":       FFN_DIM,
            "seq_len":       MAX_SEQ_LEN,
            "source_script": "train/train_h16_smoke_fa4_4090_bf16.py",
            ...
            "git_hash":      git_hash,
            "note": (
                f"H16-Smoke-FA4: D={EMBEDDING_DIM} H={NUM_HEADS} L={NUM_LAYERS} "
                f"FFN={FFN_DIM} J={len(OFFSETS)} FA@L{FULL_ATTN_LAYER} "
                f"preIF@L{FULL_ATTN_LAYER-1}. Epoch {epoch}/3. Cold start."
            ),
        },
    }
    out_path = os.path.join(checkpoint_dir, f"h16_smoke_fa4_ep{epoch}_full_attn.pt")
...
def train():
    ...
    print('=' * 70)
    print('  🚀 DWARF H=16 Smoke FA@L4 — D=512 H=16 hd=32 L=16 FFN=1024 J=24, cold start')
    ...
    for epoch in range(1, SCREEN_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(clean_state,
                       os.path.join(CHECKPOINT_DIR, 'h16_smoke_fa4_best.pt'))
            marker = ' *'
        ...
        torch.save({
            ...
            'val_loss': val_loss,
        }, os.path.join(CHECKPOINT_DIR, f'h16_smoke_fa4_ep{epoch}_resume.pt'))
    ...
    print(f'description: H16-Smoke-FA4 D={EMBEDDING_DIM} H={NUM_HEADS} hd=64 L={NUM_LAYERS} '
