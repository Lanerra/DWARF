...
_SPARSE_LIST = [48, 128, 384]
ALL_OFFSETS  = list(range(42)) + _SPARSE_LIST
assert len(ALL_OFFSETS) == 45
...
class _DSQGFnV3(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, pos_bias, scale_embed):
        ...
        assert pos_bias.shape    == (45, H)
        assert scale_embed.shape == (45, HD)
...
class DSQGAttentionV3(nn.Module):
    ...
    def __init__(self, embedding_dim, num_heads, seq_len=2048,
        ...
        self.scale_embed = nn.Parameter(torch.zeros(45, HD))
...
def run_tests(device='cuda'):
    ...
    for B, H, N, HD, lbl in cfgs:
        ...
        pb = torch.randn(45, H,  device=device, dtype=torch.float32) * 0.5
        se = torch.randn(45, HD, device=device, dtype=torch.float32) * 0.05
    ...
    v  = torch.randn(B,H,N,HD, device=device, dtype=torch.bfloat16) * 0.1
    pb = torch.randn(45, H, device=device, dtype=torch.float32) * 0.5
    se_zero = torch.zeros(45, HD, device=device, dtype=torch.float32)
