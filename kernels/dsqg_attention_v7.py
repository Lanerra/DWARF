"""
DSQG Attention V7 — J=20 relay-optimal offset set (J20_D)
==========================================================

Same mechanisms as V6 (MOVT + QK-OVT + NPCI) but with the J20_D offset set:

    ALL_OFFSETS = [1,2,3,4,6,8,13,15,16,21,23,28,48,64,96,192,384,512,768,1024]

V6 used J=16 (8 small + 8 large). V7 uses J=20 (10 small + 10 large).

Key changes from V6:
- ALL_OFFSETS: J=16 → J=20 (J20_D set with finer local coverage)
- J_SMALL=10: indices 0-9 (δ≤21) — local offsets, no MOVT
- J_LARGE=10: indices 10-19 (δ≥23) — distal offsets, MOVT applied
- Forward/backward: static_range(J) with 20-element delta tuple
- pos_bias shape: [20, H]  (was [16, H])
- scale_embed shape: [20, HD]  (was [16, HD])
- phase_base/gain shape: [10, H, 2]  (was [8, H, 2])

Usage:
  from dsqg_attention_v7 import DSQGAttentionV7, npci_rotate

Testing:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 kernels/dsqg_attention_v7.py
"""
...
# J20_D relay-optimal offset set
ALL_OFFSETS = [1, 2, 3, 4, 6, 8, 13, 15, 16, 21, 23, 28, 48, 64, 96, 192, 384, 512, 768, 1024]
J         = len(ALL_OFFSETS)   # 20
J_SMALL   = 10                 # indices 0-9: δ≤21 — local, no MOVT
J_LARGE   = 10                 # indices 10-19: δ≥23 — distal, MOVT applied
...
# ─────────────────────────────────────────────────────────────────────────────
# Forward Kernel V7 — single static_range(J) loop, direct loads for all offsets
# ─────────────────────────────────────────────────────────────────────────────
...
def _fwd_v7(
    ...
    # ── All 20 offsets — direct indexed loads ──────────────────────────────
    for i in tl.static_range(20):
        delta = (1, 2, 3, 4, 6, 8, 13, 15, 16, 21, 23, 28, 48, 64, 96, 192, 384, 512, 768, 1024)[i]
        kp    = ns - delta
        val   = (kp >= 0) & nm
...
# ─────────────────────────────────────────────────────────────────────────────
# D computation — unchanged from V6
# ─────────────────────────────────────────────────────────────────────────────
...
class _DSQGFnV7(torch.autograd.Function):
    @staticmethod
...
def dsqg_attention_v7(q, k, v, pos_bias, scale_embed,
                      phase_base, phase_gain, y_pre, z_pre):
    """
    q, k, v:       [B, H, N, HD]   bfloat16
    pos_bias:      [20, H]          float32
    scale_embed:   [20, HD]         float32
...
class DSQGAttentionV7(nn.Module):
    """
    DSQG V7: J=20 relay-optimal offsets + MOVT(r=2) + QK-OVT + NPCI.

    Extends V6 (J=16) to J=20 with finer local coverage (δ=2,6,8,16 added).
    J_SMALL=10 (δ≤21, no MOVT), J_LARGE=10 (δ≥23, MOVT applied).
    ...
    def attn_summary(self):
        with torch.no_grad():
            pb   = self.pos_bias.detach().cpu()       # [20, H]
            se   = self.scale_embed.detach().cpu()    # [20, HD]
...
def _reference_v7(q, k, v, pos_bias, scale_embed,
                  phase_base, phase_gain, y_pre, z_pre):
    """Pure PyTorch reference. Slow — for testing only.
    phase_base/gain: [J_LARGE=10, H, 2] — large offsets only (indices 10-19).
    """
    B, H, N, HD = q.shape
    sc   = HD ** -0.5
    ...
    gi   = MAX_DELTA - off[None, :] + ni[:, None]   # [N, 20]
    Ka   = kp[:, :, gi, :]                           # [B, H, N, 20, HD]
    Va   = vp[:, :, gi, :]                           # [B, H, N, 20, HD]

    s = (q.float().unsqueeze(3) * Ka).sum(-1) * sc   # [B, H, N, 20]
...
def run_tests(device='cuda'):
    print("=" * 70)
    print("DSQG V7 — Correctness Tests (J=20 J20_D + MOVT + QK-OVT + NPCI)")
    print("=" * 70)
    ...
    for B, H, N, HD, lbl in cfgs:
        ...
        ref = _reference_v7(q, k, v, pb, se, phb, phg, y, z)
        out = dsqg_attention_v7(q.clone(), k.clone(), v.clone(), pb, se, phb, phg, y, z)
    ...
    out1  = dsqg_attention_v7(q.clone(), k.clone(), v.clone(), pb, se, phb_z, phg_z, y_z, z_z)
    ref1  = _reference_v7(q, k, v, pb, se, phb_z, phg_z, y_z, z_z)
