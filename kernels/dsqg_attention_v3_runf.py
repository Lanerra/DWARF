"""
DSQG Attention V3-RunF — Dense 1-15 + GAP 16-31 + Octave Blocks 32-512
========================================================================

Derived from dsqg_attention_v3_rune.py. Key changes vs Run E:
  1. Offset set: dense 1-15 replaces dense 1-39 (removes gap-filling 16-31)
  2. J=63 (vs J=79) — ~20% faster per epoch
  3. Gap 16-31 is intentionally EMPTY (commitment pressure zone)

Design principle — "cliff edge at octave boundaries":
  Run E's "distraction by density" finding: dense 1-39 gives excellent PPL
  but prevents hard commitment to specific retrieval offsets. The model finds
  40 soft alternatives near every octave boundary and never needs to commit
  sharply to any one (resulting in passkey plateau at ~20% per distance).

  Run F restores the "cliff edge" structure from Run C: dense 1-15 recovers
  the natural-language PPL value (Run C showed dense 1-15 ≈ V3 PPL), then
  the gap 16-31 ensures there are NO soft alternatives between the local zone
  and the first octave block [32-39]. The model must commit to [32-39] for
  d=32 retrieval — no compromise available.

  The octave blocks [64-71][128-135][256-263][512-519] are kept from Run D/E,
  extending the same cliff-edge structure to all five octave distances.

  Rust verification (offset_optimizer::run_f_analysis) confirms:
  - δ_eff directly covered for d=1,2,4,8,32,64,128,256,512 (9/12 direct)
  - d=16: δ_eff=23 in gap — multi-hop only (same weakness as Run C; acceptable)
  - d=1024/1536: multi-hop synthesis (proven to reach 60% in Run D)
  - Short-range density (δ=1-31): 15 offsets — matches Run C exactly
  - Run F path-count score: 6.91× Run C, 4.83× Run D
  - Gap 16-31: CONFIRMED EMPTY (commitment pressure intact)

Offset set (J=63 including δ=0):
  Base:        {0}                              (1)  — self-attention component
  Dense 1-15:  {1,2,...,15}                    (15)  — NL modeling + d=1-8 δ_eff
  GAP:         (16-31 intentionally empty)      (0)  — commitment pressure zone
  Block d≈32:  {32,33,...,39}                   (8)  — δ_eff for d=32
...
Hypothesis: Run F = Run C PPL (~52-53) + Run D long-range passkey (60%+ by ep7)
  - dense 1-15 matches Run C's PPL recovery (confirmed: 52.857 PPL)
  - cliff edge at [32-39] reproduces Run C's d=32 commitment (ep2 onset expected)
  - octave blocks at 64-512 extend to all five long-range distances (Run D showed 60%)
  - path-count advantage vs Run C: 207× at d=1536

Comparison:
  Run C (J=40, linspace):  52.857 PPL, 45.0% passkey  (d=32:60%, d=64-512:0/40%)
  Run D (J=53, near-zero): 55.488 PPL, 35.0% passkey  (d=64/256/1024:60%, short-range:20%)
  Run E (J=79, near-zero): ~52 PPL,  ~20%  passkey    (plateau — density distraction)
  Run F (J=63, near-zero): ~52 PPL,  ~60%? passkey    (cliff edge + all octave blocks)

Init: near-zero pos_bias (zeros) — same as Run D and Run E.
...
# <<< RUNF: dense 1-15 + GAP 16-31 + octave blocks at 32-512, J=63
_BASE   = [0]
_DENSE  = list(range(1, 16))        # dense 1-15: NL modeling + d=1-8 δ_eff
# GAP 16-31: intentionally empty — commitment pressure zone
_D32    = list(range(32, 40))       # cliff-edge block 1: δ_eff for d=32
_D64    = list(range(64, 72))       # dense octave 2: δ_eff for d=64
_D128   = list(range(128, 136))     # dense octave 3: δ_eff for d=128
_D256   = list(range(256, 264))     # dense octave 4: δ_eff for d=256
_D512   = list(range(512, 520))     # dense octave 5: δ_eff for d=512
_TAIL   = [768, 1024, 1536]
ALL_OFFSETS = sorted(_BASE + _DENSE + _D32 + [48] + _D64 + [96] +
                     _D128 + [192] + _D256 + [384] + _D512 + _TAIL)
assert len(ALL_OFFSETS) == 63, f"Expected 63 offsets, got {len(ALL_OFFSETS)}"
# Verify the gap 16-31 is empty
assert all(x not in ALL_OFFSETS for x in range(16, 32)), "Gap 16-31 must be empty in Run F"
_MAX_OFFSET = 1536   # unchanged — same KV buffer size as V3/RunD/RunE
...
def _fwd_runf(
    ...
    # <<< RUNF: static_range(63) + dense-1-15 + gap-16-31 + octave-block offset tuple
...
class _DSQGFnRunF(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, pos_bias, scale_embed):
        B, H, N, HD = q.shape
        J = pos_bias.shape[0]   # 63
        ...
        _fwd_runf[(B*H, triton.cdiv(N, BN))](
...
def dsqg_attention_v3_runf(q, k, v, pos_bias, scale_embed):
    """
    q, k, v:      [B, H, N, HD]  bfloat16
    pos_bias:     [63, H]         float32  — near-zero (zeros) init recommended
    scale_embed:  [63, HD]        float32
    Returns:      [B, H, N, HD]  same dtype as input
    """
    orig_dtype = q.dtype
    if orig_dtype != torch.bfloat16:
        q, k, v = q.bfloat16(), k.bfloat16(), v.bfloat16()
    out = _DSQGFnRunF.apply(q, k, v, pos_bias.float(), scale_embed.float())
...
def _reference_runf(q, k, v, pos_bias, scale_embed):
    B, H, N, HD = q.shape
    sc  = HD ** -0.5
...
def run_smoke_test(device='cuda'):
    J = len(ALL_OFFSETS)
    print(f"V3-RunF smoke test: J={J}, max_offset={_MAX_OFFSET}")
    print(f"  Dense 1-15:  {list(range(1,16))}")
    print(f"  GAP 16-31:   (empty — commitment pressure zone)")
    print(f"  Block d≈32:  {list(range(32,40))}")
    ...
    # Forward
    ref = _reference_runf(q.detach(), k.detach(), v.detach(), pb, se)
    out = dsqg_attention_v3_runf(q.detach().clone(), k.detach().clone(),
    ...
    dsqg_attention_v3_runf(qt, kt2, vt, pb, se).sum().backward()
