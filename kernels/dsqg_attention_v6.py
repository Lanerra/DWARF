"""
DSQG Attention V6 — J=16 relay-optimal offset set (J16_D)
==========================================================

Same mechanisms as V5 (MOVT + QK-OVT + NPCI) but with the J16_D offset set
identified by analyze_j.py (Mar 11 2026):

    ALL_OFFSETS = [1,3,4,13,15,21,23,28,48,64,96,192,384,512,768,1024]

V5 used J=44 (dense δ=0..32 + 11 sparse). V6 uses J=16 (8 small + 8 large).

Key changes from V5:
- ALL_OFFSETS: J=44 → J=16 (J16_D relay-optimal set, max_hops=2)
- Forward: two-loop structure (dense advance + sparse) → single static_range(16)
  All offsets use direct indexed K/V loads (no block-pointer advance trick).
- MOVT phase rotation: applied to large offsets only (δ≥48, indices 8-15).
  phase_base/gain shape: [8, H, 2]  (was [11, H, 2])
- Backward dq/dkdv: static_range(16) with clean if/else for phase (no OOB reads)
- pos_bias shape: [16, H]  (was [44, H])
- scale_embed shape: [16, HD]  (was [44, HD])

J16_D relay properties (from analyze_j.py):
  full_dense=YES (every n∈[0,1536] reachable as sum of offsets)
  max_hops=2  (best among all J=16 candidates; others are max_hops=3)
  j_score=-616.9
...
Testing:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 kernels/dsqg_attention_v6.py
"""
...
# J16_D relay-optimal offset set (max_hops=2, full dense [0,1536] coverage)
ALL_OFFSETS = [1, 3, 4, 13, 15, 21, 23, 28, 48, 64, 96, 192, 384, 512, 768, 1024]
J         = len(ALL_OFFSETS)   # 16
J_SMALL   = 8                  # indices 0-7: δ={1,3,4,13,15,21,23,28} — no MOVT
J_LARGE   = 8                  # indices 8-15: δ={48,64,96,192,384,512,768,1024} — MOVT
assert J == 16 and J_SMALL + J_LARGE == J
...
# ─────────────────────────────────────────────────────────────────────────────
# Forward Kernel V6 — single static_range(16) loop, direct loads for all offsets
# ─────────────────────────────────────────────────────────────────────────────
...
def _fwd_v6(
    ...
    # ── All 16 offsets — direct indexed loads ──────────────────────────────
    for i in tl.static_range(16):
        ...
        mn    = tl.maximum(mi, s)
        # Guard: when mi==-inf (no valid entries yet), mi-mn = -inf-(-inf) = NaN.
        # Use tl.where to safely zero-out the correction in that case.
        cor   = tl.where(mi > float('-inf'), tl.exp(mi - mn), tl.zeros_like(mi))
        # Guard: when val==False, s==-inf and s-mn could be NaN if mn==-inf too.
        ...
        if i < 8:
            # Small offsets (δ≤28): no MOVT rotation
            acc = acc * cor[:,None] + p[:,None] * vt
        else:
            # Large offsets (δ≥48): MOVT + QK-OVT, pi = i - 8
            pi  = i - 8
...
# ─────────────────────────────────────────────────────────────────────────────
# D computation — unchanged from V5
# ─────────────────────────────────────────────────────────────────────────────
...
def _bwd_dq_v6(
    ...
    for i in tl.static_range(16):
        ...
        if i < 8:
            # Small offsets — no MOVT, no dy_pre contribution
            ...
            # Large offsets — MOVT rotation, pi = i - 8
            pi  = i - 8
            z0  = tl.load(zb + kp*stride_zn + 0, mask=val, other=0.0)
            z1  = tl.load(zb + kp*stride_zn + 1, mask=val, other=0.0)
...
def _bwd_dkdv_v6(
    ...
    for i in tl.static_range(16):
        ...
        if i < 8:
            # Small offsets — no rotation
            dot_rv = tl.sum(don * vt, axis=1)
            ds_v   = alpha * (dot_rv - Dn)
            dk    += ds_v[:,None] * qn * sc
            dv    += alpha[:,None] * don
        else:
            # Large offsets — MOVT, pi = i - 8
            pi  = i - 8
            pb0 = tl.load(PHASE_BASE + pi * stride_phi + h * stride_phh + 0)
            pb1 = tl.load(PHASE_BASE + pi * stride_phi + h * stride_phh + 1)
            pg0 = tl.load(PHASE_GAIN + pi * stride_pgi + h * stride_pgh + 0)
            ...
            # Buffer: unique slot [bh, blk, pi*2+m] — no atomic contention
...
class _DSQGFnV6(torch.autograd.Function):
    @staticmethod
    ...
    def backward(ctx, dout):
        ...
        blocks_n = (N + BN - 1) // BN
        _dev     = q.device
        # Phase buffer: J_LARGE=8 large offsets × 2 planes per block
        ...
        def _reduce_phase_buf(buf):
            # buf: [B*H, blocks_n, J_LARGE*2] → [J_LARGE, H, 2]
...
def dsqg_attention_v6(q, k, v, pos_bias, scale_embed,
                      phase_base, phase_gain, y_pre, z_pre):
    """
...
class DSQGAttentionV6(nn.Module):
    """
    DSQG V6: J=16 relay-optimal offsets + MOVT(r=2) + QK-OVT + NPCI.

    Identical mechanisms to V5 but with J16_D offset set (16 offsets vs 44).
    Expected: similar or better passkey (max_hops=2 relay), significantly
    faster kernel (63% fewer offset iterations).

    Parameters owned:
      pos_bias      [16, H]    global frequency prior
      scale_embed   [16, HD]   Q-matched-filter (zero-init)
    ...
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        # MOVT: large offsets only (δ≥48) — J_LARGE=8 offsets × 2 planes
    ...
    def attn_summary(self):
        with torch.no_grad():
            pb   = self.pos_bias.detach().cpu()       # [16, H]
            se   = self.scale_embed.detach().cpu()    # [16, HD]
        ...
        return {
            ...
            'phase_base_local_mean':     0.0,  # small offsets have no phase
...
def _reference_v6(q, k, v, pos_bias, scale_embed,
                  phase_base, phase_gain, y_pre, z_pre):
    """Pure PyTorch reference. Slow — for testing only.
    phase_base/gain: [J_LARGE=8, H, 2] — large offsets only (indices 8-15).
    """
    B, H, N, HD = q.shape
    sc   = HD ** -0.5
    ...
    gi   = MAX_DELTA - off[None, :] + ni[:, None]   # [N, 16]
    Ka   = kp[:, :, gi, :]                           # [B, H, N, 16, HD]
    Va   = vp[:, :, gi, :]                           # [B, H, N, 16, HD]

    s = (q.float().unsqueeze(3) * Ka).sum(-1) * sc   # [B, H, N, 16]
    ...
    a  = F.softmax(s, dim=-1)                        # [B, H, N, 16]
    # Positions with no valid keys (e.g. n=0 since min offset=1) produce NaN
    # from softmax of all-inf. Replace with 0 to match Triton kernel behaviour.
    a  = torch.nan_to_num(a, nan=0.0)

    # MOVT on large offsets (indices 8-15)
...
def run_tests(device='cuda'):
    print("=" * 70)
    print("DSQG V6 — Correctness Tests (J=16 J16_D + MOVT + QK-OVT + NPCI)")
    print("=" * 70)
    ...
    for B, H, N, HD, lbl in cfgs:
        ...
        ref = _reference_v6(q, k, v, pb, se, phb, phg, y, z)
        out = dsqg_attention_v6(q.clone(), k.clone(), v.clone(), pb, se, phb, phg, y, z)
        fe  = (ref.float() - out.float()).abs().max().item()
        ok  = fe < 0.05
        if not ok: ok_all = False
        print(f"  {lbl:22s}  fwd_err={fe:.4f}  {'PASS ✓' if ok else 'FAIL ✗'}")

    # Zero-init phase → same as no-phase result
    print()
    ...
    out1  = dsqg_attention_v6(q.clone(), k.clone(), v.clone(), pb, se, phb_z, phg_z, y_z, z_z)
    ref1  = _reference_v6(q, k, v, pb, se, phb_z, phg_z, y_z, z_z)
