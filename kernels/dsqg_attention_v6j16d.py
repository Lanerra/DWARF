"""
DSQG Attention V6J16D — V6 kernel mechanics with J16_D relay-optimal offsets
=============================================================================

Identical to V6 (MOVT + QK-OVT + NPCI) with the J16_D offset set:
...
Usage:
  from dsqg_attention_v6j16d import DSQGAttentionV6J16D, npci_rotate

Testing:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 kernels/dsqg_attention_v6j16d.py
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Forward Kernel — single static_range(16) loop, direct loads for all offsets
# ─────────────────────────────────────────────────────────────────────────────
...
class _DSQGFnV6J16D(torch.autograd.Function):
    @staticmethod
...
def dsqg_attention_v6j16d(q, k, v, pos_bias, scale_embed,
                          phase_base, phase_gain, y_pre, z_pre):
    """
    q, k, v:       [B, H, N, HD]   bfloat16
    pos_bias:      [16, H]          float32
    scale_embed:   [16, HD]         float32
    phase_base:    [8, H, 2]        float32
    phase_gain:    [8, H, 2]        float32
...
class DSQGAttentionV6J16D(nn.Module):
    ...
    def forward(self, x, kv_inject=None):
        ...
        out = dsqg_attention_v6j16d(q, k, v,
...
def _reference_v6j16d(q, k, v, pos_bias, scale_embed,
                      phase_base, phase_gain, y_pre, z_pre):
    B, H, N, HD = q.shape
    sc   = HD ** -0.5
...
def run_tests(device='cuda'):
    print("=" * 70)
    print("DSQG V6J16D — Correctness Tests (J=16 J16_D + MOVT + QK-OVT + NPCI)")
    print("=" * 70)
    ...
    for B, H, N, HD, lbl in cfgs:
        ...
        ref = _reference_v6j16d(q, k, v, pb, se, phb, phg, y, z)
        out = dsqg_attention_v6j16d(q.clone(), k.clone(), v.clone(), pb, se, phb, phg, y, z)
    ...
    out1  = dsqg_attention_v6j16d(q.clone(), k.clone(), v.clone(), pb, se, phb_z, phg_z, y_z, z_z)
    ref1  = _reference_v6j16d(q, k, v, pb, se, phb_z, phg_z, y_z, z_z)
