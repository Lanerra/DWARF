"""
DSQG Attention V8 H100 — J=24 relay-optimal offset set, H100/sm_90 optimised
==========================================================

Same mechanisms as V8_4090 (MOVT + QK-OVT + NPCI), J24_D offset set.
...
class _DSQGFnV8(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, pos_bias, scale_embed,
        ...
        _sm90 = (_cc[0] == 9 and _cc[1] == 0) or _cc[0] > 9   # H100 / GH200
        _sm89 = (_cc[0] == 8 and _cc[1] == 9)                   # RTX 4090 Ada

        # H100 (sm90): deeper pipeline (num_stages=3), larger L1, full tensor-core width
        # num_stages=3: software-pipelines 3 blocks ahead in the Triton scheduler
        #               ~20-30% speedup on H100 for memory-bound DSQG kernels
        # BLOCK_N=128 at HD=64: fills 128×64=8K elem accumulator (32KB) — fits H100 SRAM
        if HD <= 64:
