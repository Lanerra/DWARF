"""
condU 35M Pure DSQG — Q-Weighted Scale Gains, Scaling Validation (FineWeb-Edu)

Pure-DSQG variant of condU 35M: all 6 layers are DSQG (no full attention).
Scales condU 13M (pure DSQG, D=256) to 35M (D=512) while preserving the full
condU physics stack:
  ...
  - INTERFERENCE=3, FULL_ATTN_LAYER=-1 (pure DSQG)

PURPOSE
-------
Intermediate scaling data point between condU 13M (52.206 PPL) and a
planned condU 85M run. Allows:
  1. Pure-DSQG scaling: condU 13M (52.206 PPL, 43.3% passkey, pure DSQG)
     → condU 35M pure (this script) — does PPL advantage scale without
     full attention?
  2. Isolate the contribution of full attention: compare against the hybrid
     condU 35M (train_2048_27m_condU.py) which has FULL at layer 5.
  3. Check IF gain evolution at larger HD (50 vs 32): does the
     global>local differentiation strengthen with more head capacity?

ARCHITECTURE
  D=512, H=8, d_head=64, L=6, FFN=2048 (~35M params)
  FULL_ATTN_LAYER=-1 → all layers DSQG; layer types:
    ['DSQG', 'DSQG', 'DSQG+INT', 'DSQG', 'DSQG', 'DSQG+INT']
  Layers 0-4: condU DSQGAttentionQW (V3 kernel, Huygens injection)
              interference at layers 2 and 5 (INTERFERENCE=3)
  Layer   5:  Full O(N²) causal attention (gate=0)
  ~35M parameters; direct pure-DSQG comparison to hybrid condU 35M

HYPERPARAMETERS
  B=8, GA=4 (local RTX 4090; same effective batch as condU 13M)
  LR=3e-4, cosine, AdamW, 10 epochs, FineWeb-Edu 100K docs

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u benchmarks/train_2048_27m_condU.py \\
    2>&1 | tee benchmarks/logs/condU_27m_run.log
"""
...
EMBEDDING_DIM = 512      # 13M: 256 → 35M: 512  (HD=64, power-of-2, Triton-safe)
NUM_LAYERS    = 6
NUM_HEADS     = 8        # d_head = 64
FFN_DIM       = 2048     # 4 × EMBEDDING_DIM
INTERFERENCE  = 3
FULL_ATTN_LAYER = -1               # pure DSQG — no full attention layer
...
SAVE_DIR    = 'checkpoints/2048_condU_35m_pure_checkpoints'
RESULT_FILE = 'benchmarks/logs/condU_35m_pure_results.json'
...
# D=512, H=8 → HD=64 (power-of-2): Triton kernel runs natively with zero waste.
...
class DSQGAttentionQW(nn.Module):
    """
    condU DSQG attention — identical to condU 13M, scaled to D=400 (HD=50).

    Physics parameters:
      pos_bias    [44, H]   — global learned frequency prior
    ...
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        self.scale_embed = nn.Parameter(torch.zeros(44, HD))  # zero init = pure pos_bias
    ...
    def forward(self, x, kv_inject=None):
        ...
        # V3 kernel or Python fallback depending on HD
...
class DSQGBlock(nn.Module):
    """
    condU DSQGBlock with Huygens K/V injection (identical to condU 13M).
    No additional Kalman/KdV/AGC modifications (those were condV).
    """
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                ...
                nn.init.normal_(m.weight, 0, 0.02)
        # Enforce gate=0 on all gate_proj layers
        for m in self.modules():
...
# ── Data utilities (identical to condU 13M) ───────────────────────────────────
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
    ...
    # Separate physics parameters for lower LR (they have their own learning dynamics)
    physics_params  = set()
    for block in model.blocks:
        if isinstance(block, DSQGBlock):
            physics_params.update(id(p) for p in block.attn.pos_bias.unsqueeze(0))
            physics_params.update(id(p) for p in [block.attn.pos_bias,
                                                   block.attn.scale_embed,
                                                   block.attn.if_gain])
    param_groups = [
        {'params': [p for p in model.parameters()
                    if id(p) not in physics_params and p.requires_grad],
         'lr': LR, 'weight_decay': 0.1},
        {'params': [p for n, p in model.named_parameters()
                    if any(hasattr(b.attn, 'pos_bias') and
                           id(p) in (id(b.attn.pos_bias), id(b.attn.scale_embed),
                                     id(b.attn.if_gain))
                           for b in model.blocks if isinstance(b, DSQGBlock))],
         'lr': LR * 0.1, 'weight_decay': 0.0},
    ]
    # Flatten: just use one group for simplicity (physics params are few)
    ...
    print(f'\n  condU 35M Pure DSQG TEST: PPL {test_ppl:.3f} | Loss {test_loss:.4f}')
    print(f'  condM 27M reference: 44.500 PPL | delta = {test_ppl - 44.500:+.3f}')
    print(f'  condU 13M reference: 52.206 PPL | delta = {test_ppl - 52.206:+.3f}')
    ...
    print(f'  condU 13M reference: 43.3% passkey')
    ...
    results = {
        'experiment':              'condU_35m_pure_dsqg',
        'kernel':                  'dsqg_attention_v3',
        'architecture':            f'condU pure DSQG: 6xDSQGQW (no full attention)',
        ...
        'condu_13m_ref_ppl':       52.206,
        'condu_13m_ref_passkey':   0.433,
    }
...
def main():
    ...
    print('=' * 70)
    print('  condU 35M Pure DSQG — Q-Weighted Scale Gains + IF Amplifier + Huygens K/V')
    print(f'  D={EMBEDDING_DIM}, H={NUM_HEADS}, d_head={EMBEDDING_DIM//NUM_HEADS}, '
          f'L={NUM_LAYERS}, FFN={FFN_DIM}  [NO full attention]')
    print(f'  Pure DSQG: FULL_ATTN_LAYER={FULL_ATTN_LAYER} (disabled)')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}  '
              f'({torch.cuda.get_device_properties(0).total_memory/1e9:.1f} GB)')
    print(f'  Kernel: dsqg_attention_v3 (HD={EMBEDDING_DIM//NUM_HEADS}, BLOCK_HD={EMBEDDING_DIM//NUM_HEADS}, scale_embed [44,{EMBEDDING_DIM//NUM_HEADS}])')
    print(f'  Physics: QW scale gains + IF amplifier + Huygens K/V injection')
    print(f'  Architecture: INTERFERENCE={INTERFERENCE}, gate=0, '
          f'pure DSQG (no FullAttn)')
    ...
    # torch.compile fuses ops and reduces kernel launch overhead — typically
    # 1.5-2.5× faster on Ampere/Ada without any code changes.
