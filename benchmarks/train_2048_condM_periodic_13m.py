"""
DWARF condM-periodic 13.1M -- 3:1 Repeating Hybrid

Architecture: [DSQG, DSQG, DSQG, Full, DSQG, DSQG, DSQG, Full]
  D=224, H=8, d_head=28, L=8, FFN=896
  Full attention at layers 3 and 7 (0-indexed)
  ~13.06M parameters (vs condM 13M: 13.97M -- within 7%)

Purpose
-------
  First iso-scale test of the 3:1 periodic hybrid. Direct comparison with:
    condM 13M (layer5): 13.97M params, test PPL 54.529
    condM 13M (layer3): 13.97M params, test PPL 54.480
    Standard 13M:       test PPL 64.07

  Hypothesis: two co-trained preprocessor->attention pairs give better PPL
  than one 5-layer DSQG chain, at essentially the same parameter count.

  Each 3-DSQG-block learns to preprocess for its own full attention layer:
    Block 1 (layers 0-3): handles local coherence + medium-range patterns
    Block 2 (layers 4-7): handles global retrieval + long-range patterns

  Expected PPL: 51-54 (conservative) or sub-50 (optimistic).

  Interference at DSQG layers 2 and 5 (i%3==2, not in full_attn_layers).

Run on local RTX 4090:
  CUDA_VISIBLE_DEVICES=0 python3 -u benchmarks/train_2048_condM_periodic_13m.py \\
    2>&1 | tee benchmarks/logs/condM_periodic_13m_run.log
"""
...
EMBEDDING_DIM  = 224
NUM_LAYERS     = 8        # [DSQG, DSQG, DSQG, Full, DSQG, DSQG, DSQG, Full]
NUM_HEADS      = 8        # d_head = 28
FFN_DIM        = 896      # 4 x D
INTERFERENCE   = 3

FULL_ATTN_LAYERS = {3, 7}   # full attention at layers 3 and 7 (0-indexed)
CHECKPOINT_DIR   = '2048_condM_periodic_13m_checkpoints'
...
# ---- Attention and block classes (identical to 6M script) --------------------
...
class DSQGAttentionN(nn.Module):
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        delta_vals = torch.tensor([math.log(1.0 + d) for d in _COND_N_OFFSETS], dtype=torch.float32)
...
class CondMPeriodicTransformer(nn.Module):
    def __init__(self, vocab_size, D, L, H, FFN_dim, seq_len,
        ...
        super().__init__()
        if full_attn_layers is None: full_attn_layers = {L - 1}
...
# ---- Data utilities (identical to 6M) ----------------------------------------
...
def train(model, train_data, val_data, test_data, tokenizer, save_dir, device):
    ...
    print(f'\n  condM-periodic 13M TEST: PPL {test_ppl:.3f}')
    print(f'\n  ======= Comparison =======')
    print(f'  {"Standard 13M":<40} {"64.07":>8}')
    print(f'  {"condM 13M (layer5, 5 DSQG)":<40} {"54.529":>8}')
    print(f'  {"condM 13M (layer3, 3+2 DSQG)":<40} {"54.480":>8}')
    print(f'  {"condM-periodic 13M [3:1:3:1] (this)":<40} {test_ppl:>8.3f}')
    delta = test_ppl - 54.529
    print(f'  vs condM 13M (layer5): {delta:+.3f} PPL '
          f'({"better" if delta < 0 else "worse"} with {model.param_count():,} params '
          f'vs 13,984,480)')

    return {'test_ppl': test_ppl, 'best_val_ppl': best_val_ppl,
            'best_epoch': best_epoch, 'total_time_s': time.time() - t0,
            'architecture': '[DSQG×3, Full, DSQG×3, Full]',
            'full_attn_layers': sorted(FULL_ATTN_LAYERS),
            'n_params': model.param_count(),
            'condM_13m_layer5_ppl': 54.529,
            'standard_13m_ppl': 64.07}


def main():
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    print('=' * 70)
    print('  condM-periodic 13.1M -- [DSQG×3, Full, DSQG×3, Full]')
    ...
    inter_layers = [i for i in range(NUM_LAYERS)
                    if i not in FULL_ATTN_LAYERS and i % INTERFERENCE == INTERFERENCE - 1]
    ...
    out_path = os.path.join(_script_dir, 'condM_periodic_13m_results.json')
