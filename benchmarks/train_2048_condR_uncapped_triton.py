"""
condR-Uncapped — condR architecture with FineWeb-Edu uncapped (13M)

Identical architecture to condR (capped), but with no sequence cap on the
FineWeb-Edu dataset. Tests whether condR's faster passkey emergence combines
with the breadth advantage of uncapped FineWeb-Edu (more unique long documents).

Capped condR result (for comparison):
  - 43.3% passkey at epoch 7 (d=2:80% — highest single-distance on FW-Edu)
  - 40.0% at epoch 6 — first run to exceed the 38.3% FW-Edu ceiling
  - PPL better than condQ throughout

Uncapped dataset size:
  - ~212,458 train sequences/epoch (4x the capped 52,716)
  - Chinchilla point at epoch ~0.64
  - Each epoch ~24 min on RTX 4090; 5 epochs = ~2 hours
  - Extend NUM_EPOCHS if curve still ascending at epoch 5

Architecture (identical to condR — do not modify):
  - 5 DSQG layers + 1 FullAttentionBlock (layer 5)
  - INTERFERENCE = 2 (layers 1 and 3 get interference)
  - Gate from out_flat (experiment B)
  - Gate bias 1.0 (experiment C)
  - Zero-init gate weights (experiment A)
  - LR warmup 200 steps (experiment E)
  - Gradient checkpointing on FullAttentionBlock (experiment F)
  - SDPA dropout RETAINED (condQ showed removing it kills d>=512)
  - Triton DSQGAttentionN_Fused kernel

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u benchmarks/train_2048_condR_uncapped_triton.py \
    2>&1 | tee benchmarks/logs/condr_uncapped_run.log
"""
...
NUM_EPOCHS      = 5   # uncapped: each epoch ~4x longer; extend if still ascending
BATCH_SIZE      = 8
GRAD_ACCUM      = 4
LR              = 3e-4
MAX_SEQ_LEN     = 2048
...
# MAX_TRAIN_SEQS: removed — uncapped run uses all available sequences
...
SAVE_DIR    = 'checkpoints/condr_uncapped'
RESULT_FILE = 'benchmarks/logs/condr_uncapped_results.json'
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
    ...
    best_epoch        = 0
    best_passkey_mean = 0.0
    best_passkey_epoch = 0
    t0                = time.time()
    per_epoch_results = []
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for acc_step in range(steps_per_epoch):
            ...
            if step % 200 == 0:
                lr_now = scheduler.get_last_lr()[0]  # actual LR (no display bug)
        ...
        if pk_mean > best_passkey_mean:
            best_passkey_mean  = pk_mean
            best_passkey_epoch = epoch
            torch.save(model.state_dict(),
                       os.path.join(SAVE_DIR, 'best_passkey.pt'))
            print(f'  *** New best passkey: {pk_mean*100:.1f}% at epoch {epoch} ***')
    ...
    # ── Final evaluation — PPL-optimal checkpoint ────────────────────────────────
    model.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'best.pt'),
                                     weights_only=True))
    test_loss = evaluate(model, test_data, BATCH_SIZE, device)
    test_ppl  = math.exp(min(test_loss, 20))
    print(f'\n  condR-Uncapped TEST (PPL-optimal, ep{best_epoch}): PPL {test_ppl:.3f} | Loss {test_loss:.4f}')

    # ── Also evaluate best-passkey checkpoint ────────────────────────────────────
    pk_opt_path = os.path.join(SAVE_DIR, 'best_passkey.pt')
    if os.path.exists(pk_opt_path) and best_passkey_epoch != best_epoch:
        model.load_state_dict(torch.load(pk_opt_path, weights_only=True))
        pk_test_loss = evaluate(model, test_data, BATCH_SIZE, device)
        pk_test_ppl  = math.exp(min(pk_test_loss, 20))
        print(f'  condR-Uncapped TEST (passkey-optimal, ep{best_passkey_epoch}): PPL {pk_test_ppl:.3f}')
        print(f'  PPL-vs-passkey trade-off: {test_ppl:.3f} vs {pk_test_ppl:.3f} PPL')
        # Restore PPL-optimal for temperature sweep
        model.load_state_dict(torch.load(os.path.join(SAVE_DIR, 'best.pt'),
                                         weights_only=True))
    ...
    print('  ' + '  '.join(parts))
    print(f'  condM reference: 83.3% passkey | condR-capped ep7: 43.3%')

    ss = model.attn_summary()

    results = {
        'experiment':        'condr_uncapped',
...
def main():
    ...
    print('=' * 70)
    print('  condR-Uncapped — condR architecture, FineWeb-Edu uncapped (13M)')
    print('=' * 70)
    ...
    print(f'  Uncapped: using all {len(train_data):,} train sequences')
