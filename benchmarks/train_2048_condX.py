...
from causal_ema_native import causal_ema_kdv, causal_ema_dyn
...
class DSQGBlock(nn.Module):
    ...
    def __init__(self, embedding_dim, num_heads, ffn_dim, seq_len,
        ...
        if interference:
            ...
            # condX: content-selective gate — learns which tokens are "passkey-like"
            # and deserve a strong field imprint (gate≈1) vs background (gate≈0)
            # Init bias=-3 → sigmoid≈0.047: near-zero initially, opens during training
            self.content_gate_proj = nn.Linear(embedding_dim, 1, bias=True)
    ...
    def forward(self, x):
        ...
        if self.interference:
            ...
            B, N, D = xi.shape
            H, HD   = self.num_heads, self.head_dim
            content_gate = torch.sigmoid(self.content_gate_proj(xi))  # (B,N,1)
            pool    = causal_ema_dyn(xi, content_gate, self.ema_factor, self.kdv_alpha)
    ...
    def block_summary(self):
        ...
        return {
            ...
            'gate_bias':   self.content_gate_proj.bias.item(),
            'gate_sigmoid': torch.sigmoid(self.content_gate_proj.bias).item(),
        }
...
class CondXTransformer(nn.Module):
    ...
    def _init_weights(self):
        ...
        for m in self.modules():
            ...
            if hasattr(m, 'content_gate_proj') and isinstance(m.content_gate_proj, nn.Linear):
                nn.init.zeros_(m.content_gate_proj.weight)
                nn.init.constant_(m.content_gate_proj.bias, -3.0)
    ...
    for n, p in model.named_parameters():
        ...
        if pname in physics_names:
            physics_params.append(p)
              ...
              (pname == 'bias' and 'content_gate_proj' in mname) or
