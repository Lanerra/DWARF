"""
condM 13M — Chinchilla-Repeated Emergence Experiment
=====================================================
Identical to train_2048_condM_layer_ablation_triton.py (full_layer=5) with two
additions: per-epoch checkpoint saving and per-epoch passkey retrieval eval.

Purpose: track when passkey capability emerges across epochs on repeated data.
Chinchilla ref: 13.98M params × 20 = 280M tokens; epoch ≈ 2.6 at 108M tok/epoch.

Usage:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u benchmarks/train_2048_condm_chinchilla_repeated.py \\
    2>&1 | tee benchmarks/logs/condm_chinchilla_repeated.log
"""
...
# Chinchilla-Repeated config — full_layer always 5 (condM canonical)
FULL_ATTN_LAYER   = 5
SAVE_DIR          = 'checkpoints/condm_chinchilla_repeated'
CONDM_13M_PARAMS  = 13_980_000
CHINCHILLA_TOKENS = 20 * CONDM_13M_PARAMS   # ~280M tokens

# Passkey eval constants (exact format from eval_suite.py)
...
def train(model, train_data, val_data, test_data, tokenizer,
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # Per-epoch checkpoint (always, not just best)
        torch.save(model.state_dict(),
                   os.path.join(save_dir, f'epoch_{epoch:02d}.pt'))

        tok_per_epoch = len(train_data) * MAX_SEQ_LEN
        chin_ep       = CHINCHILLA_TOKENS / tok_per_epoch
        pct           = epoch / chin_ep * 100
        chin_flag     = ' ← ~CHINCHILLA' if abs(epoch - chin_ep) < 0.6 else f' ({pct:.0f}%C)'
        print(f'Ep {epoch}/{NUM_EPOCHS} | Train {train_loss:.4f} '
              f'| Val {val_loss:.4f} PPL {val_ppl:.1f}{marker} | {elapsed:.0f}s{chin_flag}')
        ...
        # Passkey retrieval eval (per epoch)
        print('  Passkey...')
        pk      = passkey_accuracy(model, tokenizer, device)
        pk_mean = sum(pk.values()) / len(pk)
        pk_hits = sum(1 for v in pk.values() if v > 0.5)
        print(f'  mean={pk_mean*100:.1f}%  ({pk_hits}/{len(pk)} distances >50%)')
        print('  ' + '  '.join(f'd={d}:{v*100:.0f}%' for d, v in sorted(pk.items())))

        if not hasattr(train, '_history'): train._history = []
        train._history.append({
            'epoch': epoch, 'val_ppl': val_ppl,
            'chinchilla_pct': epoch / (CHINCHILLA_TOKENS / (len(train_data) * MAX_SEQ_LEN)) * 100,
            'passkey_mean': pk_mean,
            'passkey_by_d': {str(k): v for k, v in pk.items()},
            'elapsed_s': elapsed,
        })
        sys.stdout.flush()
    ...
    # ── Summary table ──────────────────────────────────────────────────────────
    history = getattr(train, '_history', [])
    chin_ep = CHINCHILLA_TOKENS / (len(train_data) * MAX_SEQ_LEN)
    if history:
        print('\n' + '='*72)
        print('  condM 13M — Chinchilla-Repeated Emergence')
        print(f'  Chinchilla epoch ≈ {chin_ep:.1f} | Reference: 10-ep = 54.529 PPL / 83.3% passkey')
        print('='*72)
        print(f'  {"Ep":>3}  {"Tokens":>8}  {"%Chin":>6}  {"ValPPL":>8}  {"Passkey":>8}')
        for r in history:
            cf = ' ←' if abs(r['epoch'] - chin_ep) < 0.6 else '  '
            tok = r['epoch'] * len(train_data) * MAX_SEQ_LEN
            print(f'  {r["epoch"]:>3}  {tok/1e6:>7.0f}M  '
                  f'{r["chinchilla_pct"]:>5.0f}%  {r["val_ppl"]:>8.1f}  '
                  f'{r["passkey_mean"]*100:>7.1f}%{cf}')
        print('='*72)
        out = 'benchmarks/logs/condm_chinchilla_repeated_results.json'
        with open(out, 'w') as f:
            json.dump({'experiment': 'condm_chinchilla_repeated',
                       'chinchilla_epoch': chin_ep, 'per_epoch': history,
                       'final_test_ppl': test_ppl}, f, indent=2)
        print(f'  Results → {out}')
...
def main():
    ...
    print('=' * 70)
    print('  condM 13M — Chinchilla-Repeated Emergence Experiment')
    print('=' * 70)
    if torch.cuda.is_available():
        print(f'  GPU: {torch.cuda.get_device_name(0)}  '
              f'({torch.cuda.get_device_properties(0).total_memory/1e9:.1f} GB)')
    print(f'  Architecture: 5 DSQG → 1 full attn (layer {FULL_ATTN_LAYER}) — condM canonical')
    print(f'  Chinchilla target: {CHINCHILLA_TOKENS/1e6:.0f}M tokens (20 × {CONDM_13M_PARAMS/1e6:.2f}M params)')

    os.makedirs('benchmarks/logs', exist_ok=True)
    os.makedirs(SAVE_DIR, exist_ok=True)

    ckpt_dir_name = SAVE_DIR
    ...
    # Per-epoch JSON already saved inside train(); final results here:
    results_path = f'benchmarks/logs/condm_chinchilla_repeated_final.json'
